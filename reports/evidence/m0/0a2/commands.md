...
# F-2(총계와 단위별 합이 어긋남)는 계산 오류가 아니라 **두 실행의 출력을 섞어 붙인**
# 것이었다. 스크립트 안의 assert로는 그것을 막을 수 없다 — 같은 실행 안에서 두 값은
# 정의상 같기 때문이다(항진식). 그래서 가드는 assert가 아니라 **출력 형태**다:
# 총계·단위별·합·소계를 인접한 세 줄로 함께 찍어, 서로 다른 실행에서 떼어다 붙이면
# 눈에 보이게 한다. 복사할 때 세 줄을 통째로 옮기는 것이 재발 방지책이다.
tot_c = sum(cand.values())
print(f"=== 광역 후보(열거 동반 셈) : {tot_c}건 ===")
print("  단위별:", dict(cand), "| 합:", tot_c)
...
=== 광역 후보(열거 동반 셈) : 489건 ===
  단위별: {'줄': 38, '행': 33, '종': 19, '건': 329, '항목': 12, '곳': 22, '개': 36} | 합: 489
  항목·종·곳 소계: 53
```
...
> 위는 **기준 커밋 한 번의 실행**으로 교체했고, 스크립트가 **총계·단위별·합·소계를
> 인접한 세 줄로 함께 찍어** 다시 섞이지 않게 했다.
>
> **가드는 `assert`가 아니다**(verifier F6-4 정정). 처음에 넣었던 합·총계 일치 `assert`는
> **항진식**이었다 — 같은 실행 안에서 두 값은 정의상 같으므로 F-2의 원인(**두 실행의
> 출력을 섞어 붙이기**)을 막지 못한다. 스크립트 안의 어떤 검사로도 못 막는 종류이므로
> **assert를 걷어내고 그 사실을 스크립트 주석에 적었다.** 실제 재발 방지책은 **출력
> 형태**다 — 세 줄이 붙어 있어 서로 다른 실행에서 떼어다 붙이면 눈에 보인다.
>
...
**추가 발견은 없다.** 광역 후보 중 `항목`·`종`·`곳` 단위 **53건**(기준 커밋 실측,
위 `소계`)**을 전수로 읽었고**, #1 외에
...
> (실물은 **OPS-03의 `V2 제약`**). 미갱신이 아니라 `e06e62a`(운영자 결정 반영)의
...
| 분류 `후속` | **거짓 포인터** — 근거(전달 의미 확정 + pull 확정)는 정합하나 "아래 `V2 제약`"이 **NOTI-05에 없는 bullet**을 가리켰다(verifier F-1). **OPS-03의 `V2 제약`**으로 명시했고 `경계` bullet("outbox 메커니즘 일반은 OPS-03이 소유")과 정합한다 |
...
| # | 처리 |
| --- | --- |
| **F-1** | NOTI-05 `분류`가 **NOTI-05에 없는 `V2 제약`**을 가리켰다 → **OPS-03의 `V2 제약`**으로 명시. 미갱신이 아니라 `e06e62a`의 재분류가 **신설한 잘못된 포인터**다. R13-4의 "한 bullet뿐"·`분류` 행 "정합"·`scope.md`의 같은 전칭을 실제 상태로 고쳤다 |
...
```
$ python3 numsweep.py
추출된 수치 인용: 217건 | 정본 불일치: 134건 (이력 서술 포함 — 판정 대상)
$ python3 enumsweep.py
추출 16건 | 불일치 7건
=== 광역 후보(열거 동반 셈) : 489건 ===
  항목·종·곳 소계: 53
...
### R13-8. Codex 8차 완결 2차 — verifier F6-1~F6-4 (2026-08-27)

재검증 `not-ready`(medium 2 · low 2). **F-2~F-5의 실질은 전부 통과** — F-2 진단 타당·값
전부 재현·고정점 성립, F-4 해소, F-5 판단 타당, **범위 밖 근거 ③도 표본 확인에서 오류 0**.

#### F6-1 — 줄 번호 자기참조를 **없앴다**

`:2345`가 2줄 어긋났다. **원인은 F-1 수정 자신**이다 — `:1846-1850`에 +2줄을 넣어 아래를
전부 밀었고, `:2345`는 verifier가 `ec9e3c8`에서 잰 값을 옮긴 것이다. **형태 5를 자기
편집에 대해 반복**했다.

**줄 수를 맞춰 고치는 대신 줄 번호를 뺐다.** 근거:

1. **정보를 더하지 않는다.** `OPS-03의 V2 제약`은 **이름만으로 유일하게 지시된다** —
   `### OPS-03 ` 블록 **1개**, 그 안의 `- **V2 제약**` **1개**(문서 전체에는 18개가
   있으므로 유일성을 만드는 것은 "OPS-03의"라는 소유자 지정이지 줄 번호가 아니다).
2. **줄 번호는 편집마다 썩는다.** 이 문서는 계속 편집되고, 이번 라운드가 정확히 그것으로
   걸렸다. 줄 수 불변으로 고치면 **다음 편집에서 또 깨진다.**
3. **빼면 재발이 불가능해진다.** 이름 참조는 대상이 사라지지 않는 한 유효하고, 사라지면
   그건 F-1과 같은 부류로 다시 잡힌다.

**전수 확인**: 문서 안 `` `:NNNN` `` 형식 참조는 `capability-map.md`에 4건이고
`:1848`(=이번 건)만 **자기참조**, 나머지 `:1457`·`:1927`·`:3172`는 전부 **legacy 문서
참조**(`:240`·`:266`·`:240`)임을 직접 열어 확인했다. 전파 지점 4곳(`capability-map.md`
본문 · `commands.md` 두 곳 · `scope.md`)에서 `:2345`를 전부 제거했다. **참조로서의
`:2345`는 0건**이며, 남은 매치는 **이 절이 제거 사실을 설명하며 인용하는 것뿐**이다
(자기참조 — `R9-3`·`R10-3`에 적은 것과 같은 부류).

#### F6-2 — "고쳤다"가 거짓이었다

F-1 처리에서 **세 지점 중 둘만 고치고** `scope.md`의 같은 전칭("미갱신 서술은 그 한
bullet뿐이었고")을 두었다. **이번 라운드가 성문화한 형태 6의 거짓 전칭이 살아 있었고,
처리했다는 진술 자체가 거짓**이었다.

`scope.md` 원문 지점에 `> 정정 (verifier F-1)` 블록을 넣어 `commands.md`와 처리 수준을
맞췄다. §10.1 형태 6에 **재발 3회**로 남겼다 — **"고쳤다"는 진술도 전칭이다. 적기 전에
그 지점을 열어 확인한다.**

#### F6-3 — `scope.md` 원문 지점에 정정 포인터

F-3·F-5를 `commands.md`에만 반영하고 `scope.md` 원문(`35건을 전수로`·`막았다`)은 그대로
뒀다. 새 절이 처리를 적었으나 **원문 지점에 포인터가 없어** 두 파일의 처리 수준이 달랐다.
이 저장소가 이미 쓰는 방식(`0ce2e4c` "원문 지점에 정정 절 포인터 append")대로 **취소선 +
정정 포인터**를 원문에 달았다.

#### F6-4 — 항진식 `assert`를 걷어냈다

넣었던 `assert tot_c == sum(dict(cand).values())`는 **같은 값을 자기 자신과 비교**하는
항진식이라 가드가 아니었다. **F-2의 원인은 계산이 아니라 전사(두 실행의 출력을 섞어
붙이기)이므로 스크립트 안의 어떤 검사로도 막을 수 없다.**

→ **assert를 제거하고 그 사실을 스크립트 주석에 적었다.** 실제 가드는 **출력 형태**다 —
총계·단위별·합·소계를 **인접한 세 줄로 함께** 찍어, 서로 다른 실행에서 떼어다 붙이면
눈에 보이게 한다. **"고정했다"는 주장을 "출력 형태로 줄였다"로 낮췄다.**

#### "고쳤다"고 적은 지점의 실제 확인

F6-2가 그 실패였으므로 이번엔 **적기 전에 전부 열어 확인**했다.

| 주장 | 확인 |
| --- | --- |
| `:2345` 제거 4곳 | 참조로서 **0건**. `grep -rn ':2345'`의 잔여 매치는 **이 R13-8 절이 제거 사실을 설명하며 인용하는 것**뿐이고 `capability-map.md`·`scope.md`에는 **0건** |
| `scope.md` 전칭 정정 | `:760` 원문에 취소선 없이 서술 교체 + `> 정정` 블록 확인 |
| F-3·F-5 원문 포인터 | `scope.md`의 `35건`·`막았다` 두 줄에 취소선 + 정정 확인 |
| `assert` 제거 | 인라인 본문(R13-1)에 **`assert` 문 없음** 확인 — 남은 두 매치는 **왜 assert로 막을 수 없는지 설명하는 주석**이다 |
