...
- **C-3.1** — **위 출력이 낸 매치는 전부** ① **취소선 안**(원본 보존) ② **정정 블록이
  무엇을 고쳤는지 설명하는 산문** ③ **provenance 표가 이 정정을 등재한 행**
  ④ **§13 정정문이 원문을 인용한 자리** 중 하나이며 **살아 있는 단정은 없다.**
  **좌표를 여기 옮겨 적지 않는다** — 어느 줄인지는 위 블록이 낸다(편집마다 밀린다).
  §13 **밖**에 살아 있는 단정은 **C-3.4가 따로 찍는다**(`scope.md` **O-2**).
