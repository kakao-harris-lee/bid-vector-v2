...
- **판정 근거의 출처** — 두 「남는다」의 판정 기준은 **0B가 세운 것**이다. 0B verifier
  `F-c`가 *"「legacy 서술이니 그대로」로는 부족하다 — **귀속 없이 적으면 이 문서의 확정
  서술로 읽힌다**"*로 판정하고 `R-BASIS-03`·`R-BASIS-06`에 **`OPEN-REG-05` 유보**를 달았다.
  **`observed`로 귀속하는 것만으로는 부족하고 그 `OPEN`을 지목해야 한다**는 기준이며,
  두 항목 다 그 지목이 없다.
