...
| §6 NOTI(투찰률) | **`observed`** — *"(관찰된 회귀)"*로 귀속 | **그렇다 — 방향을 말한다**(*"과세 공고에서 하한 여유가 **부풀어 보인다**"*). 그러려면 두 금액의 **대소 관계**가 정해져야 하고 그것은 `OPEN-REG-05` 소유인데 **그 귀속이 없다** | **남는다** |
| §6 NOTI(acceptance) | 표시 요구 | **아니다** — *"서로 다른 **행으로 표시된다**"*는 **표시**에 대한 것이고 차이의 크기·방향을 말하지 않는다 | **빠진다** |
| §7 SET(SET-09 F-4) | **`observed`** — 표 제목이 「관찰된 실패·회귀」 | **그렇다 — 크기를 정량화한다**(*"과세 공고에서 **약 10%** 어긋난다"*). **0B ledger `R-BASIS-06`이 같은 주장에 ※를 달아 `OPEN-REG-05` 소유로 돌린 그 문장**인데 **여기엔 그 귀속이 없다** | **남는다** |
...
- **판정 근거의 출처** — 두 「남는다」의 판정 기준은 **0B가 세운 것**이다. 0B verifier
  ...
  **기준은 `observed`로 귀속하는 것만으로는 부족하고 그 `OPEN`을 지목해야 한다**는 것이며,
  두 항목 다 그 지목이 없다.

- **`O-2` 판정 요약** — **남는 것은 `observed` 층에서 두 금액의 차이를 정량화·방향화하면서
  `OPEN-REG-05`를 지목하지 않은 항목**이고 **§6 NOTI(투찰률)와 §7 SET(SET-09 F-4)**이 그것이다.
...
쪼개** 기록한다. 그렇지 않으면 이 절을 쓸 때마다 결과가 늘어 **고정점이 아니다**.

```
### 실행 시점 HEAD = a264d6f (이 라운드의 편집이 든 작업 트리)
### 마커로 집는다 — 이 절 자신의 서술을 세지 않기 위해서다(고정점).

$ MARK='재판정으로 이 절의'; grep -rn "⚠ 수정 라운드 10의 출처 층 $MARK" reports/evidence/m0/0a3/ | cut -c1-64
reports/evidence/m0/0a3/scope.md:443:> **⚠ 수정 라운드 10의 출처 층 재판정으로
reports/evidence/m0/0a3/scope.md:527:> **⚠ 수정 라운드 10의 출처 층 재판정으로
reports/evidence/m0/0a3/checklist.md:421:> **⚠ 수정 라운드 10의 출처 층 재

$ MARK2='재판정으로 이 귀속은'; grep -rn "⚠ 수정 라운드 10의 출처 층 $MARK2" reports/evidence/m0/0a3/ | cut -c1-64
reports/evidence/m0/0a3/checklist.md:365:| **H-1** (high) | C-3.

$ grep -n '^## 갱신 이력 — 수정 라운드 [45] ' reports/evidence/m0/0a3/scope.md | cut -c1-56
436:## 갱신 이력 — 수정 라운드 4 (verifier `H-1`~`H-4`)
507:## 갱신 이력 — 수정 라운드 5 (verifier `P-1` · 게이트 안 1건)
$ grep -n '^## 1[12]\. 수정 라운드' reports/evidence/m0/0a3/checklist.md | cut -c1-56
358:## 11. 수정 라운드 4 — verifier `H-1`~`H-4`
397:## 12. 수정 라운드 5 — verifier `P-1` (게이트 안 1건)
```

**판정** — **네 자리 전부 라운드별 이력 절 안에 있다.** 위 두 grep 이 낸 줄 번호가
아래 두 grep 이 낸 절 머리보다 **뒤**다: `scope.md`는 **라운드 4 이력**과 **라운드 5
이력**, `checklist.md`는 **§11(라운드 4)**과 **§12(라운드 5)**. **이력 본문은 고쳐 쓰지
않았고 표시만 달았다.**

**앞 라운드들이 달아 둔 다른 표시**(`scope.md` 라운드 7 · `checklist.md` §10 `G-2` ·
§14 `M-2` · `O-2` 행)는 **이 라운드가 만든 것이 아니라** 여기 세지 않는다 —
다른 마커를 쓴다.
