...
전부 저장소 루트에서 실행한다. **재실행일을 여기 열거하지 않는다** — 라운드마다 낡고, 감사자가
확인할 것은 「언제 돌렸는가」가 아니라 「지금 돌려도 같은가」다. 어느 라운드가 무엇을 건드렸는지는
`git log -- fixtures/ 'reports/evidence/m0/0e/fixtures-*.md'` 가 낸다.
**분류·계약 라운드는 기대값·입력 파일을 건드리지 않았다** — 바꾼 것은 manifest 의 분류·출처·계약
뿐이다. 그 사실을 산문이 세지 않고 `git log --oneline 14686db..HEAD -- fixtures/expected
fixtures/input` 가 낸다(그 목록은 짧고, 라운드 이름을 읽으면 어느 것이 데이터 수정이었는지 보인다).
