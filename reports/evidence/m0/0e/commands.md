...
| **C-12b** | 강등 뒤 **낡은 현재형 `authoritative` 주장** — manifest 전수. `/tmp/fxvenv/bin/python -c "import yaml,io,re;f='fixtures/manifest.yaml';m=yaml.safe_load(io.open(f,encoding='utf-8'));c={x['id']:x['classification'] for x in m['cases']};I=sorted(c,key=len,reverse=True);S=re.compile(r'classification_policy\.authoritative\|물음:\|답:\|열릴 당시\|이전의 상태\|내려갔\|비었\|였다\|2026-09-02 에');print('불일치',[(i,d) for i,l in enumerate(io.open(f,encoding='utf-8').read().splitlines(),1) if 'authoritative' in l and not S.search(l) for d in I if d in l and c[d]!='authoritative'])"` | 0 | **불일치 0.** 산문이 어떤 case 를 현재형으로 `authoritative` 라 말하는데 그 case 의 실제 `classification` 이 아닌 자리를 찾는다. **제외 셋** — `classification_policy.authoritative`(절 이름) · 결정 **축어**(`물음:`/`답:` — 그 시점의 원문이라 고쳐선 안 된다) · **과거형 표지**(「열릴 당시」·「내려갔다」 류). **이 클래스는 세 번째다**(B10 medium #2 · r19 F-11 · B11) — 강등이 서사를 낡게 만드는 자리라 라운드마다 돌린다. **이 검사가 못 보는 것**: 같은 줄 안의 동시 출현만 재므로 문단에 걸친 주장은 사람이 읽는다 |
...
**preflight 명령**(`C-13`·`C-14`·`C-16`·`C-17` 이 쓴다. 정본은 `.claude/skills/codex-review-gate/SKILL.md` 이고 여기 있는 것은
**각 라운드의 실행 기록**이다). ②는 총 행 수를 같은 핸들로 받고(0 이면 못 연 것이다), ③의 `codex --version` 은 **지금 PATH 가 고르는** 버전이라 **실행된 버전의 정본은 머리글**이다.
