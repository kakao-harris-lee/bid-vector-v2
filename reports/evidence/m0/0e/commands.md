...
| **C-12b** | 강등 뒤 **낡은 덮개 주장** — manifest 전수, **문단 단위**. **⚠ 실행 가능한 형태가 아직 없다** — 정의는 아래에 전부 적었으나 표 행에 인라인할 수 없다(정규식 교체(`or`)의 세로줄이 표 칸을 깨고 루프에 줄바꿈이 필요하다). **커밋된 스크립트 자리를 정하는 것이 남았다**(B10 medium #3 과 같은 처리) | 0 | **수정 전(`78b98ee`) 13 → HEAD 8.** Codex 가 든 넷 중 **문면에 온전한 id 로 적힌 셋**(`verdict-004`·`money-basis-001`·`-006`)과 `money-basis-005` 가 닫혔다. **정의** — 블록은 **빈 줄로 가르고**, **주장형 동사**(`authoritative` 로 남·고정·덮·유지 / 그대로 `authoritative` / `authoritative` 이고 / 덮인다 / 덮는다 / 고정한다 / 고정하고 / 계속 고정)와 case id 가 **한 블록의 ±200자** 안에 함께 있으면 센다. **제외**는 과거형 표지(열릴 당시 / 이전의 상태 / 내려갔 / 비었 / 였다 / 이던). **못 보는 것 둘** — ① **축약 인용**(`money-basis-001`·`` `004` `` 처럼 뒤를 번호로만 적은 자리)은 id 문자열이 없어 잡히지 않는다(Codex 가 든 `money-basis-004` 가 그 자리다) ② 무해/유해를 기계가 가르지 못한다 — **남는 8 은 축어·강등 기록·축이 그 case 를 강등 이유로 드는 자리·이미 ⚠ 로 층을 밝힌 자리**이고 **0 을 목표로 두지 않는다** |
