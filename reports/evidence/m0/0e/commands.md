...
| **C-7b** | 같은 목록 \| `grep -E '^(fixtures/\|docs/discovery/(data-dictionary\|regression-ledger)\|reports/evidence/m0/0[a-d])'` | **0** | **매치는 이 레인의 계약·분류 축 파일들**이고 **여기 열거하지 않는다 — 명령이 낸다**(열거하면 접촉이 늘 때마다 낡는다). `in_scope` 파일과 그 축 파일을 한 커밋에 담은 **혼합 커밋**(부류 A)이 있어 pathspec 이 두 레인에 걸친다. **선언된 예외**이고 부류·검출 명령·판정 규칙은 표 아래 **「`C-7b` 매치의 처리」**가 갖는다. **⚠ 이 검사는 이 레인의 `fixtures/` 접촉을 전수로 재지 못한다** — `C-0` 이 `in_scope` 경로로 레인을 뽑으므로 **그 축 파일만 건드린 커밋(부류 B)은 입력에 들지 않는다.** 같은 절이 그 부류의 검출을 갖는다. `data-dictionary.md` · `regression-ledger.md` · 앞 여섯 slice evidence 는 여전히 **무접촉** |
...
| **C-12b** | 강등 뒤 **낡은 덮개 주장** — manifest 전수, **문단 단위**. `python3 fixtures/tools/manifest_prose_consistency.py [manifest 경로]` (인자를 주면 옛 트리의 manifest 로 그날 판정을 재현한다 — 기대값·입력은 강등에서 바뀌지 않는다) | 0 | **수정 전(`78b98ee`) 13 → HEAD 8**(case id 종류). **셈 단위 둘을 병기한다** — 스크립트는 `불일치(case id)` 와 `불일치(블록,case)` 를 함께 내고 후자는 **18 → 11** 이다. **같은 집합의 다른 단위**이고 적중 목록은 같다. **정의** — 블록은 **빈 줄로 가르고**, **주장형 동사**(`authoritative` 로 남·고정·덮·유지 / 그대로 `authoritative` / `authoritative` 이고 / 덮인다 / 덮는다 / 고정한다 / 고정하고 / 계속 고정)와 case id 가 **한 블록의 ±200자** 안에 함께 있으면 센다. **제외**는 과거형 표지(열릴 당시 / 이전의 상태 / 내려갔 / 비었 / 였다 / 이던). **못 보는 것 둘** — ① **축약 인용**(`money-basis-001`·`` `004` `` 처럼 뒤를 번호로만 적은 자리)은 id 문자열이 없어 잡히지 않는다(Codex 가 든 `money-basis-004` 가 그 자리다) ② 무해/유해를 기계가 가르지 못한다 — **남는 8 은 축어·강등 기록·축이 그 case 를 강등 이유로 드는 자리·이미 ⚠ 로 층을 밝힌 자리**이고 **0 을 목표로 두지 않는다** |
...
| **C-24** | `codex-review-gate` **preflight** — 리뷰 B15(`approve`) | 0 | ① repo 흔적 **3**(B14 와 같다) ② 매치 **0** / 총 **683행** — **5라운드 연속 정체**(B11~B15). 배치 정지로 읽되 **매치 0 이라 독립성은 성립** ③ **`0.151.0`** ④ 누출 줄 기준 **4** · **developer 구간 0**(경계 = codex 첫 출력 **149** 행) · **프롬프트 누출 grep 0**(B14 수정 논거 0자) ⑤ **방출 2 — 종말이 `approve` 인 첫 라운드**다. **150** 행 `approve`(findings 0 · **cmds 0**)와 **9308** 행 `approve`(findings **1** · cmds **13**)이고, **저장본은 9308 과 일치·150 과 불일치**함을 양방향으로 확인했다. **스킬의 「`commands_run` 빈약한 approve = 미완 실행」 규칙이 실제로 갈라야 했던 첫 사례**이고 **0 대 13** 으로 명확히 갈렸다 — 조기 출력을 저장했으면 **findings 를 잃은 approve** 를 등재했을 것이다 ⑥ **계수 판별자의 네 번째 오탐 표면 — 이 evidence 자신이다.** codex 가 `commands.md` 를 뜨면서 **`C-23` 행**이 그대로 찍혔는데, 그 행은 세 번째 표면을 설명하느라 `{"verdict":"Eligible"…}` 를 인용하고 판별자 이름 `reviewed_base` 를 함께 적는다 — **한 줄에 둘이 있어 형태 판별자를 통과한다.** **판별자의 실패 형태를 evidence 에 적는 행위가 그 실패의 새 사례를 만든다.** ⑦ **그래서 판별자를 실측으로 다시 갈랐다** — B13·B14·B15 에서 **앞머리 앵커**(앞줄이 `codex`)는 **3·3·2** 로 매번 옳았고 **형태 판별자 단독**은 **4·4·4** 로 매번 과다였다(CLI 후행 echo 를 늘 포함하고, B15 에선 위 `C-23` 행까지 셌다). **둘의 교집합은 앵커와 같다.** **정본은 스킬**(`45add86`)이고 그 문면은 형태 판별자를 든다 — **이 실측은 하네스 레인에 보고했다** ⑧ **hooks** 이벤트 8 · 스크립트 1 · stdout **0 바이트** ⑨ **제한 ③의 성격** — legacy object-id 대조 미실행은 clean worktree 에 legacy checkout 이 **의도적으로** 없어서다(격리 설계의 대가이고 전 라운드 `residual_risks` 에 일관 기록). 저장소 안의 index 해시·manifest 참조 집합·pinned SHA 선언의 **상호 일치까지는 확인**됐다 |
| **C-23** | `codex-review-gate` **preflight** — 리뷰 B14 | 0 | ① repo 흔적 **3**(B13 은 6 · B12 는 4 — **값이 라운드마다 움직인다**. 조건은 값의 고정이 아니라 **그 양을 기록한다**이고, 스냅숏으로만 읽는다) ② 매치 **0** / 총 **683행** — **4라운드 연속 정체**(B11~B14). 배치 정지로 읽되 **매치 0 이라 독립성은 어느 쪽이든 성립** ③ **`0.151.0`** ④ 누출 줄 기준 **14** · **developer 구간 0**(경계 = codex 첫 출력 **147** 행) · **프롬프트 정합** — 누출 grep 1건은 B5 이래의 색인 줄로 무해, **B13 수정 논거 누출 0** ⑤ **방출 3 · 조기 `approve` 2** — **148**(findings 0 · **cmds 0**)·**4329**(cmds 1) 행 · **11416** 행 종말 `request_changes`(findings 1 · cmds 12)이고 **저장본과 일치**한다. **11421 행은 CLI echo 라 제외** ⑥ **계수 판별자의 세 번째 오탐 표면 — 도메인 스키마의 `verdict` 필드.** codex 가 `license-*` 기대값을 그대로 뜨면서 `{"verdict":"Eligible"…}` 류가 **8줄** 찍혔다. **다만 이 라운드에서 두 판별자 모두 그것을 걸러냈다** — 형태 판별자는 `reviewed_base` 동반을 요구해서(그 8줄은 0건), 앞머리 앵커는 앞줄이 `codex` 가 아니라 `### license-00N expected` 라서다. **표면은 실재하나 죽은 카운터는 없었다** — 정본은 스킬(`45add86`) ⑦ **hooks** 이벤트 8 · 스크립트 1 · stdout **0 바이트** |
| **C-22** | `codex-review-gate` **preflight** — 리뷰 B13 | 0 | ① repo 흔적 **6**(B12 는 4 — `~/.codex` 는 라운드 사이에도 바뀐다. 값이 아니라 **그 양을 기록한다**가 조건이다) ② 매치 **0** / 총 **683행** — **3라운드 연속 정체**(B11~B13). 리뷰 레인은 이제 **배치 정지**로 읽고, **매치 0 이라 독립성은 어느 쪽이든 성립**한다 ③ **`0.151.0`** ④ 누출 줄 기준 **11** · 발생 **16** · **developer 구간 0**(경계 = codex 첫 출력 **145** 행) ⑤ **방출 3 · 조기 `approve` 2** — **2774**(**25%**)·**9999**(**89%**) 행, 각 findings 0 · cmds 1 · **11221** 행 종말 `request_changes`(findings 2 · cmds 19)이고 **저장본과 일치**한다. **11226 행은 `tokens used` 뒤 CLI echo 라 제외**. **이 라운드가 「위치 무관」의 제대로 된 근거다** — 둘 다 `codex` 표지가 앞선 **진짜 방출**이고 하나는 **실행 89% 지점**이다. B10 에서 같은 명제를 들었다가 그 자리가 `jq` 의 파일 읽기 출력이라 철회했는데, **성문화된 계수 규칙 아래서 이번에 진짜 방출로 복원됐다** ⑥ **hooks** 이벤트 8 · 스크립트 1 · 실행 실측 3경로 stdout **0 바이트** ⑦ **프롬프트 정합** — B12 수정 논거 누출 **0**, 머리 축어 동일 |
| **C-21** | `codex-review-gate` **preflight** — 리뷰 B12 | 0 | ① repo 흔적 **4** ② 매치 **0** / 총 **683행** — **정체가 이어진다**(674 → 683 → 683 → 683). 리뷰 레인 판단은 *"일시 정지보다 배치 정지 쪽으로 기움"* 이나 **매치 0 이라 독립성은 어느 쪽이든 성립**한다 — 계속 관측한다 ③ **`0.151.0`** ④ **누출 줄 기준 19 · 발생 27(역대 최대)** · **developer 구간 0**(경계 = codex 첫 출력 **143** 행). **range 가 스킬 파일과 `CLAUDE.md` 이력을 둘 다 담아 생긴 예상된 false-positive 바닥**이고 **판독 규칙 없이는 오탐 중단됐을 라운드**다 ⑤ **방출 3 · 조기 `approve` 2**(**144**·**2749** 행, 각 findings 0 · cmds 1) · **10520** 행 종말 `request_changes`(findings 2 · cmds 15)이고 **저장본과 일치**한다. **10525 행은 `tokens used` 뒤의 CLI echo 라 제외**. **한 라운드 조기 approve 2회는 B10 에 이어 두 번째 — 단발이 아니다** ⑥ **hooks** 이벤트 8 · 스크립트 1 · 실행 실측 3경로 stdout **0 바이트**(`ORCA_*` unset) |
...
**`C-7b` 매치의 처리 — 이 레인의 계약·분류 축 파일 접촉을 선언된 예외로 둔다.**
...
- **무엇이 잡히는가.** `8701882`(「`milestone-1.md` 인용 좌표를 `:12` 에서 `:10` 으로 — 두 자리」)이
  `milestone-0.md` 와 그 축 파일을 한 커밋에 담아 두 레인의 pathspec 에 동시에 걸린다.
...
- **`C-7b` 가 구조적으로 보지 못한다.** `C-0` 이 `in_scope` 경로로 레인을 뽑으므로 그 축 파일만 건드린
  커밋은 입력에 아예 없다 — 매치는 A 부류가 건드린 경로뿐이고, **B 부류의 검출은 위 블록의 마지막 명령**이 한다.
- **왜 이 레인이 manifest 를 고치는가.** 접촉의 축이 **이 레인이 세운 계약·분류 축**이라서다 —
  계약과 그 계약이 무엇을 golden 으로 잠그는가의 판정. **범위는 커밋마다 다르고 그 커밋 메시지가 갖는다.**
