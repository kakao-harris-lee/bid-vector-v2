...
| **C-4** | `git diff 14686db c490b5a -- 'v2-지침서.md' milestone-1.md milestone-0.md` | 0 | 개정 셋의 전후 문면. Q1 `3.x`→`4.x`+4.x 재확인 항목 · Q2 유효기간 축 제거+미검증 노출 유지 · Q3 산출물 행 삭제+`ADR 0009` (d) 사유 |
| **C-5a** | `git diff -U0 14686db c490b5a -- docs/adr/ \| grep -cE '^@@'` | 0 | **11** = 아홉 파일의 3행 + `ADR 0001` §5 의 두 hunk. 그 밖은 무접촉 |
...
| **C-8** | `for f in reports/evidence/m0/*/codex-review-*.json; do python3 -c "import json,sys;d=json.load(open(sys.argv[1]));print(d['verdict'],d['reviewed_base'][:8],d['reviewed_head'][:8])" "$f"; done` | 0 | verdict 30건. 각 slice 마지막: 0A `request_changes` `6c6b3a2a` · 0A2 `f790e193` · 0A3 `20f09baf` · 0B `7701556c` · 0C `7cbdc9bb` · 0D `df056259` (뒤 다섯 `approve`) |
| **C-9a** | `git log --oneline 6c6b3a2..6af7019 -- docs/discovery/capability-map.md` | 0 | 커밋 **4개** (`0b0c8aa`·`23b9c2a`·`0581e97`·`d140f92`) — 0A 라운드 4~6 |
| **C-9b** | `git diff --stat 6c6b3a2 6af7019 -- docs/discovery/capability-map.md` | 0 | **31 삽입 / 14 삭제** — 어느 리뷰 range 에도 들지 않은 delta |
...
| **C-10** | `git diff --numstat 14686db c490b5a -- 'v2-지침서.md' milestone-0.md milestone-1.md docs/adr docs/discovery/capability-map.md \| awk '{a+=$1;d+=$2} END{print a-d}'` · `wc -l reports/evidence/m0/0e/{scope,commands,checklist}.md` | 0 | 산출물 순증 **429** · evidence 합계 **300** — **게이트 통과**(evidence ≤ 산출물) |
