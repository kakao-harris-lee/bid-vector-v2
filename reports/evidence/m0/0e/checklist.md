...
**registry 밖의 본문 서술도 해소 상태에 맞췄다** — `OPEN-OPS-07` 이 §12·§14 의 해소와 달리
**OPS-21 본문**·**§13 인계 표**에 현재형 「활성이다」·「기입 대기」로 남아 있었다(Codex B5 medium).
**3.x 전제의 조사 결과는 역사적 전제로 보존**했고 남은 4.x 재확인의 담당이 그 `OPEN` 의 미해소분이
아니라 **M1 1A** 임을 명시했다. **§12 만 파싱하는 `C-3` 은 이 모순을 못 본다** — `C-12` 가 그 자리다.
