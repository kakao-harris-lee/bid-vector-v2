...
**그 에스컬레이션에 운영자가 답했다 — 4문 4답, 물음과 답을 그대로 옮긴다(대신 판단하지 않는다).
물음은 이 레인이 세운 가족 정의 그대로이고, 네 답 모두 「유지」다 — 분류를 바꾼 답은 없다.**
...
**그래서 내려간 넷과 남은 가족들이 한 잣대의 양면이다** — 이름을 `verifies` 가 **잠그면** 내려가고
(`verdict-001`·`002`·`floor-applicability-001`, 그리고 기대값이 같은 어휘를 잠근 `verdict-003`),
**나르기만 하면** 남는다. **남는 것이 승인된 것은 아니다** — 그 이름들은 어휘 승인 대상이며 case
단위 비준과 다른 축이다. 가족별 근거와 강등선 문면은 위 정본이 갖는다.
