...
head_sha: ba2e4f9   # 이 갱신 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | **출력 블록 전부를 `ba2e4f9` 트리에서 다시 떴다** — 실제로 바뀐 것은 아홉이다. 선언 SHA를 옮기고 Codex verdict JSON을 evidence 패키지에 등재한다. `head_sha`를 `ba2e4f9`로 옮긴다 |
