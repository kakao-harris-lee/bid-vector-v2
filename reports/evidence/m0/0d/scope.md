...
head_sha: ba2e4f9   # 산출물이 고정된 커밋 = commands.md 의 선언 SHA. 아래 절 참조
in_scope:
...
| 이 커밋 | **`head_sha` 규약을 커밋 관계에서 떼어냈다.** *"직전 커밋"*은 뒤 편집과 0C 레인의 개입으로 **두 번 낡았다.** 규약을 **「산출물이 고정된 커밋 = `commands.md`의 선언 SHA」**로 바꿨다 — **관계가 아니라 SHA를 지목하므로 이 파일을 다시 써도 옮겨지지 않는다.** 값은 `ba2e4f9`로 불변이다 |
