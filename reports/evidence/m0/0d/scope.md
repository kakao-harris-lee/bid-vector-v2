...
head_sha: ba2e4f9   # 이 파일을 쓴 커밋의 직전 **0D** 커밋. 「head_sha와 range」 절 참조
in_scope:
...
커밋의 직전 0D 커밋**이다 — **`HEAD^`가 아니다.** 이 브랜치는 다른 slice와 공유되므로
`HEAD^`가 그 slice의 커밋일 수 있다. **선별자는 커밋 제목의 `m0-0d` 태그**이며 C-2가 같은
선별로 경로를 잰다.

그 뒤에 오는 0D 커밋은 이 값을 갱신하지 않는다 — 그 경로가 전부 `in_scope`(`docs/adr/`
또는 `reports/evidence/m0/0d/`) 안이라 **"in_scope 밖 경로가 없다"는 판정이 뒤 커밋에
낡지 않기 때문**이다. 확인하는 명령은 `commands.md` **C-2**다.
...
| 이 커밋 | **`head_sha` 규약이 공유 브랜치에서 거짓이 됐다** — *"직전 커밋"*이라 적었는데 `HEAD^`가 0C 레인의 커밋이다. 규약을 **「이 파일을 쓴 커밋의 직전 0D 커밋」**으로 고쳤다(선별자는 C-2와 같은 `m0-0d` 태그). **값은 `ba2e4f9`로 불변**이다 |
