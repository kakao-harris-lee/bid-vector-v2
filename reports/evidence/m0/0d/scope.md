...
head_sha: f207412   # 산출물이 고정된 커밋 = commands.md 의 선언 SHA. 아래 절 참조
in_scope:
...
**커밋 관계로 정의하지 않는다** — 앞선 라운드에 `직전 커밋`으로 적었다가 두 번 낡았다
(뒤 편집이 들어올 때 한 번, 0C 레인이 사이에 커밋했을 때 한 번). **관계는 낡고 SHA는
낡지 않는다.**

그래서 **이 파일을 다시 쓰는 커밋은 이 값을 옮기지 않는다** — 그 커밋이 바꾸는 것은
이력과 규약이지 산출물이 아니고, `commands.md`의 출력 블록도 그 SHA의 트리에서 그대로
다시 뜬다(**C-11**). **값이 옮겨지는 것은 출력 블록을 다시 뜰 때뿐이다.**
...
| 이 커밋 | **출력 블록 전부를 선언 SHA `f207412` 트리에서 다시 떴다** — 실제로 바뀐 것은 다섯(C-1 · C-2 · C-7 · C-8 · C-10)이다. **C-12를 C-11 앞으로 옮겼다** — 뒤에 두면 C-11이 자기 출력 길이를 바꿀 때마다 C-12의 기록 좌표가 낡는다. 선언 SHA와 `head_sha`를 그 값으로 맞추고, C-7의 판정 줄에서 낡은 셈을 걷었다 |

**이력 절은 자기가 속한 커밋의 diff나 파일 목록을 주장하지 않는다.** 행 라벨은
**이미 커밋된 SHA**이며, 그 커밋이 무엇을 바꿨는지는 `git show --stat --format='' <SHA>`에
맡긴다. **예외는 마지막 행 하나** — 라운드의 마지막 변경이 이 파일과 같은 커밋에 실릴 때
`이 커밋`으로 적는다. 그 행도 **무엇을 바꿨는지만 적고 diff나 파일 목록을 주장하지 않으며**,
다음 편집에서 SHA로 확정된다.
