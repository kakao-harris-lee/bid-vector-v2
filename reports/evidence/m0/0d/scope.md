...
head_sha: 122ec21   # 이 갱신 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | **이 표의 마지막 행 라벨과 아래 footer가 어긋나 있었다** — footer가 *"이미 커밋된 SHA만 지목한다"*고 적는데 마지막 행이 `이 커밋`이었다. footer를 실제 관례에 맞췄고 그 행을 `122ec21`로 확정했다. `head_sha`를 `122ec21`로 옮긴다 |
