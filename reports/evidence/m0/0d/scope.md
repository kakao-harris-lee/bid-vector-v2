...
head_sha: 6d6e38b   # 이 갱신 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | **A8 회귀 정정.** `79700e1`이 신설한 C-11의 하네스가 명령 문자열을 56자로 자르면서 **자른 자리가 공백이면 후행 공백이 남았고**, 그 출력이 기록되면서 `git diff --check`가 3건을 잡았다. 하네스의 출력 한 줄에 `.rstrip()`을 넣어 원인을 없애고 C-11 출력을 다시 떴다 — **검사를 늘리면 그 검사 자신이 검사 대상이 된다**는 것을 이 자리가 보인다 |
