...
head_sha: 779266f   # 이 갱신 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
6. **`legacy-reference-map.md`를 만들지 않았다.** 그 파일의 존재 여부 자체가
...
| `779266f` | **F-2 · F-3 · F-5 · F-7 · F-8.** C-3이 스캐너의 3줄을, C-5.4가 `DATABASE_URL` 6행 중 3행을 뺐다 — 둘 다 복원(마스킹 유지). C-9.4의 후행 공백을 `paste`로 없앴고, C-9.5의 「glob 순서」 주장을 `sort`로 대체했으며, C-1 주석의 라운드 이력을 이 파일 소유로 되돌렸다. **C-9.6·C-10을 신설**해 `F-4`·`F-1`이 요구한 전수 훑기의 자리를 만들었다 |
