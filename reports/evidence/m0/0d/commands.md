...
**출력 블록은 `2a46bd4` 트리에서 뜬 것이다** — **예외는 아래 표의 「재현 래칫」 열이
`제외`인 블록들이고, 그 열의 값을 사람이 정하지 않는다.** C-11의 스크립트가 명령 블록의
문면에서 정하며, **무엇이 어느 사유로 빠졌는지는 C-11 출력이 쌍마다 낸다.**
커밋 안의 블록은 자기 커밋 트리에서 뜰 수 없다 — 그래서 선언 SHA를 **`2a46bd4`로 그대로
...
| --- | --- | --- | --- |
| C-1 · C-2 · C-3 · C-3n · C-4 · C-7 · C-7n · C-8 · C-9 · C-10 | 이 저장소 | **`2a46bd4`** | 대조 |
| C-5.1 ~ C-5.5 | legacy `bid-vector`만 | **`ed4b06c`** (이 저장소의 어느 HEAD에서도 같다) | 대조 |
| C-6 · C-6n | **양쪽** — `citescan.py`가 `docs/adr`(저장소)와 legacy를 함께 읽는다 | **`2a46bd4` + `ed4b06c`** | 대조 |
| C-12.1 · C-12.2 | **양쪽** — 리뷰 레인 산출물(`_workspace/`)·`~/.codex` 실물과 **등재된 verdict JSON**(이 저장소) | **`2a46bd4`** + 저장소 밖 실물 | **제외 `EXT`** — 트리의 함수가 아니다 |
...
```
2a46bd41ed5157388f03468507ad198c2d106d06
tracked_dirty=0
```
...
모두 그렇다. **구조가 막는 것이 아니다** — 새 블록을 더할 때 이 판정에 기대지 말고
문면에 토큰을 넣어야 한다.
...
**판정**: `diff=0`이면 **위 출력이 `replayed=`로 센 쌍이 선언 SHA `2a46bd4`의 트리에서
