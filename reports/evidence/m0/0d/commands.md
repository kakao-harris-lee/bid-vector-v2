...
**출력 블록은 `d96d38b` 트리에서 뜬 것이다** — **예외는 아래 표의 「재현 래칫」 열이
`제외`인 블록들이고, 그 열의 값을 사람이 정하지 않는다.** C-11의 스크립트가 명령 블록의
문면에서 정하며, **무엇이 어느 사유로 빠졌는지는 C-11 출력이 쌍마다 낸다.**
커밋 안의 블록은 자기 커밋 트리에서 뜰 수 없다 — 그래서 선언 SHA를 **`d96d38b`로 그대로
...
| --- | --- | --- | --- |
| C-1 · C-2 · C-3 · C-3n · C-4 · C-7 · C-7n · C-8 · C-9 · C-10 | 이 저장소 | **`d96d38b`** | 대조 |
| C-5.1 ~ C-5.5 | legacy `bid-vector`만 | **`ed4b06c`** (이 저장소의 어느 HEAD에서도 같다) | 대조 |
| C-6 · C-6n | **양쪽** — `citescan.py`가 `docs/adr`(저장소)와 legacy를 함께 읽는다 | **`d96d38b` + `ed4b06c`** | 대조 |
| C-12.1 · C-12.2 | **양쪽** — 리뷰 레인 산출물(`_workspace/`)·`~/.codex` 실물과 **등재된 verdict JSON**(이 저장소) | **`d96d38b`** + 저장소 밖 실물 | **제외 `EXT`** — 트리의 함수가 아니다 |
...
```
d96d38b59a969be02303ef3f98a3beec1149450e
tracked_dirty=0
```
...
**`EXT`는 명령 문면에서 나온다** — 충분조건은 **위 표의 토큰이 블록 문면에 있는 것**이지
「저장소 밖을 읽는가」가 아니다. 토큰이 문면에 들어오면 그것만으로 빠지고 빠지면 그것만으로
대조로 돌아온다.

**그래서 축과 판정이 같지 않다.** 위 「제외의 축」은 *"트리를 고정하면 답도 고정되는가"*인데
이 판정은 그것을 **토큰 세 개로 근사**한다 — **저장소 밖 상태를 그 토큰 없이 읽는 블록은
빠지지 않고 대조 대상으로 남는다.** 절대 경로(`cat /etc/hosts`·`ls /Users/…/.codex/`) ·
네트워크(`curl …`) · 상위 디렉터리 저장소(`git -C ../bid-vector …`) · 시점 값(`date -u`)이
모두 그렇다. **그런 블록이 오늘 없다는 것은 실측된 사실이고 구조가 막는 것이 아니다** —
새 블록을 더할 때 이 판정에 기대지 말고 문면에 토큰을 넣어야 한다.

빠진 쌍은 `SKIP <코드> <근거 토큰>` 행으로 남고 사유별 수는 마지막 줄이 낸다 — **출력
자신이 무엇을 왜 뺐는지 말한다.**
...
**판정**: `diff=0`이면 **위 출력이 `replayed=`로 센 쌍이 선언 SHA `d96d38b`의 트리에서
