...
**출력 블록은 `1e641bb` 트리에서 뜬 것이다** — **예외는 아래 표의 「재현 래칫」 열이
`제외`인 블록들이고, 그 열의 값을 사람이 정하지 않는다.** C-11의 스크립트가 명령 블록의
문면에서 정하며, **무엇이 어느 사유로 빠졌는지는 C-11 출력이 쌍마다 낸다.**
커밋 안의 블록은 자기 커밋 트리에서 뜰 수 없다 — 그래서 선언 SHA를 **`1e641bb`로 그대로
...
| --- | --- | --- | --- |
| C-1 · C-2 · C-3 · C-3n · C-4 · C-7 · C-7n · C-8 · C-9 · C-10 | 이 저장소 | **`1e641bb`** | 대조 |
| C-5.1 ~ C-5.5 | legacy `bid-vector`만 | **`ed4b06c`** (이 저장소의 어느 HEAD에서도 같다) | 대조 |
| C-6 · C-6n | **양쪽** — `citescan.py`가 `docs/adr`(저장소)와 legacy를 함께 읽는다 | **`1e641bb` + `ed4b06c`** | 대조 |
| C-12.1 · C-12.2 | **양쪽** — 리뷰 레인 산출물(`_workspace/`)·`~/.codex` 실물과 **등재된 verdict JSON**(이 저장소) | **`1e641bb`** + 저장소 밖 실물 | **제외 `EXT`** — 트리의 함수가 아니다 |
...
— 같은 규약을 다른 파일에 다시 적지 않고 여기를 가리킨다.
...
```
1e641bb72594d71ed4104bc39029051999fdef52
tracked_dirty=0
```
...
```
pair  0 L61    OK                                 git rev-parse HEAD
pair  1 L82    OK                                 T0=$(mktemp -d)
pair  2 L138   OK                                 T=$(mktemp -d)
pair  3 L165   OK                                 # adrscan.py 는 C-3 블록이 만든 "$T" 의 것을 그대로 쓴다 —
pair  4 L188   OK                                 git log --format='%H %s' 998dc21..HEAD | gre
pair  5 L222   OK                                 cd bid-vector
pair  6 L291   OK                                 cd bid-vector
pair  7 L308   OK                                 awk -F'\t' '$4>50 {printf "%s:%d-%d\t%d\t%s\
pair  8 L362   OK                                 cd bid-vector
pair  9 L433   OK                                 cd bid-vector
pair 10 L504   OK                                 cd bid-vector
pair 11 L554   OK                                 cd bid-vector
pair 12 L587   OK                                 cd bid-vector
pair 13 L626   OK                                 cd bid-vector
pair 14 L735   OK                                 T=$(mktemp -d)
pair 15 L790   OK                                 # citescan.py 는 C-6 블록이 만든 "$T" 의 것을 그대로 쓴다
pair 16 L885   OK                                 T=$(mktemp -d)
pair 17 L927   OK                                 # ops07scan.py 는 C-7 블록이 만든 "$T" 의 것을 그대로 쓴다
pair 18 L946   OK                                 xargs grep -nEi 'api[_-]?key|secret|token|pa
pair 19 L970   OK                                 grep -ci 'react' docs/discovery/capability-m
pair 20 L982   OK                                 grep -h '^### Slice' milestone-*.md | wc -l
pair 21 L997   OK                                 grep -ho 'OPEN-[A-Z]\{2,4\}-[0-9]\{2\}' docs
pair 22 L1035  OK                                 grep -ho 'OPEN-ADR-[0-9]\{2\}' docs/adr/*.md
pair 23 L1060  OK                                 grep -n 'OPEN-[A-Z]\{2,4\}-[0-9]\{2\}' docs/
pair 24 L1095  OK                                 T=$(mktemp -d)
pair 25 L1127  OK                                 echo "-- ADR 안의 OPS-13 전수"
pair 26 L1188  SKIP EXT <_workspace/>             echo "-- 실행 메타 · 출처: _workspace/m0-0d/codex.
pair 27 L1229  SKIP EXT <~/>                      echo "-- preflight ① consolidate 이후 이 저장소 흔적
pair 28 L1356  SKIP SELF <==0D-BLOCK-HARNESS==>   # ==0D-BLOCK-HARNESS== — 이 쌍은 대조 대상에서 빠진다 (S
pairs_total=29 replayed=26 skipped_SELF=1 skipped_EXT=2 diff=0
```

**판정**: `diff=0`이면 **위 출력이 `replayed=`로 센 쌍이 선언 SHA `1e641bb`의 트리에서
