...
- 작성: 라운드 0 → 수정 라운드 1 → 수정 라운드 2(2026-08-22, Codex 재리뷰 `request_changes` 대응) → 수정 라운드 3(2026-08-26, verifier `not-ready` M-1 + low 3건 대응) → 수정 라운드 4(2026-08-26, Codex 3차 `request_changes` high 1건 대응) → **수정 라운드 5(2026-08-26, verifier `not-ready` H-1·M-1·M-2·L-1·L-2 대응)**
- 산출물: `docs/discovery/capability-map.md` (2,624줄 — 라운드 5 기준)
...
> 독립 재현**됐고"였다. **그 재현 주장은 사실이 아니어서 철회한다**(§10.3 · `commands.md`
...
| 축 | 정의 | 결과 |
| --- | --- | --- |
| **F1** (신규) | 조건부 보유 블록마다 (조건부 묶음 본문 + 인용 OPEN의 §12 결정문) = 쟁점 어휘를 만들고, **같은 블록의 무조건 항목**과 토큰 중첩 3개 이상을 전수 적출 | 7블록에서 7건 적출, 전부 결정 무관. ~~수정 전 STR-08 항목이 중첩 5로 걸려 Codex 정본 finding을 독립 재현~~ → **라운드 5에서 철회**(§10.3). 실측 결과 수정 전 STR-08 항목은 **2 / 2**로 임계 3 미만이라 **적출되지 않는다** |
...
| # | severity | 처리 | 산출물 변화 |
| --- | --- | --- | --- |
| **H-1** | **high** | F1의 "수정 전 STR-08을 중첩 5로 적출해 정본 finding을 독립 재현했다"를 **처리 방식 (a)로 정정** — 같은 스크립트를 수정 전 문서(`6c6b3a2`)에 **실제로 실행**했다. 실측 **2 / 2**(임계 3 미만, **적출 안 됨**)이며 기록된 5는 수정 **후** 값이었다. **주장을 철회**하고 F1의 성격을 triage 필터로 정정. §10.1 채택 근거 (2)도 철회하고 (1)·(3)만으로 재작성 | 산출물 변화 없음(evidence만) |
...
| **L-2** | low | F1·F2'·X **스크립트 전문**을 `commands.md` G2에 기입. 두 스크립트 모두 문서 경로를 인자로 받아 임의 리비전에 재실행 가능 | commands.md |
...
**결론은 바뀌지 않는다.** Codex 정본이 high라는 채택 판단은 근거 (1)(1·2차와 같은 레인)과
(3)(유리한 판정을 고르면 심판 레인이 무의미)만으로 유지되고, STR-08 모순의 검증
...
2. **X는 M-1을 적출하지 못한다.** 버그 수정 후에도 NOTI-02 `경계` × `OPEN-STR-08`은
   실측 **2**(수정 후 3)다. 임계를 2로 내리면 적출이 **437건**이 되어 읽을 수 있는 양을
   벗어나고, 그 안에서도 M-1은 최하위라 잡음과 구별되지 않는다. **verifier는 축이 아니라
   읽어서 M-1을 찾았다.**
