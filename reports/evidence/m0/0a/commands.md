...
2. **X는 M-1을 적출하지 못한다.** 버그를 고친 뒤에도 NOTI-02 `경계` × `OPEN-STR-08`의
   ...
   > **⚠ 아래 "기준 리비전" 문단과 표 헤더, 그리고 그 뒤의 "재구현 차이" 문단은 틀렸다.**
   > **이 파일 맨 끝의 「G4 기준 리비전 정정 (verifier 라운드 6 H-2)」 절을 함께 읽어라.**
   > 요약: 수정 전 열의 기준은 `23b9c2a`가 아니라 **`0b0c8aa`**이고, verifier가 낸
   > 15·36은 재구현 차이가 아니라 **G2 스크립트와 일치하는 정확한 값**이었다.
   > (0A는 동결이므로 원문을 재작성하지 않고 이 포인터만 단다 — 0A2 slice 계약.)
...
## 2026-08-26 — **G4 기준 리비전 정정 (verifier 라운드 6 H-2)** · slice 0A2에서 append

> **0A는 `6af7019`에서 동결됐다.** 이 절은 위 G4 본문을 **재작성하지 않고 append로**
> 정정한다. 0A2 slice 계약(`reports/evidence/m0/0a2/scope.md`)이 이 파일을 "H-2 정정 주석
> 한정, 재작성 금지, append 방식"으로 in_scope에 넣었다.

### 무엇이 틀렸나

라운드 6이 L-7에 대응해 G4 임계 표에 기준 리비전을 기입했는데, **그 기입 자체가 H-1과
같은 계열의 미검증 기재였다.** verifier가 evidence의 `sweep_r5.py`를 코드 블록에서 기계
추출하고 임계 상수 한 줄만 파라미터화해 실측했다.

```
리비전     임계5  임계4  임계3  임계2
23b9c2a     15     36    109    437
0b0c8aa     16     37    109    437
0581e97     15     37    110    440
```

| G4가 적은 것 | 실측 | 판정 |
| --- | --- | --- |
| "수정 전 = `23b9c2a`"이고 그 열이 16/37/109/437 | `23b9c2a`는 **15/36**/109/437 | **틀림.** 16/37은 `0b0c8aa` 값이다 |
| "`0b0c8aa`도 4행 전부 같은 값이다" | 임계 5·4가 다르다(16 vs 15, 37 vs 36) | **틀림** |
| "verifier가 낸 15·36은 재구현 차이. G2 스크립트로는 16·37" | G2 스크립트가 `23b9c2a`에서 **15·36**을 낸다 | **틀림.** 구현 차이가 아니라 **리비전 오표기**이며 verifier 값이 G2 스크립트 값과 일치한다 |

### 정정

1. **표의 "수정 전" 열 기준은 `0b0c8aa`다** — `23b9c2a`가 아니다.
2. `23b9c2a`(M-1·M-2 직전)에서는 임계 5·4가 **15·36**이다. 두 리비전의 차이는 **라운드 4의
   ML-03 수정**이며(`23b9c2a`가 ML-03 `legacy 형태 처리`를 재서술해 X 축 적출 1건이
   사라진다), 그래서 정확히 1씩 어긋난다.
3. **"재구현 차이" 문단을 철회한다.** verifier의 재구현은 G2 스크립트와 **일치했고**,
   어긋난 것은 구현이 아니라 **리비전 표기**였다. 외부의 정확한 관측을 결함으로 되돌려
   적은 것이므로 H-1의 처리 원칙("완화하지 않고 실측으로 교체")에 정면으로 어긋났다.

### 왜 이 정정이 필요한가 — 계열 기록

H-1은 "어느 리비전에서 측정했는지 확인하지 않고 **재현 주장**을 기입"이었다. H-2는
"어느 리비전에서 측정했는지 확인하지 않고 **리비전 이름**을 기입"이다. **같은 뿌리이며,
그 계열을 닫으려던 수정이 그 계열을 재생산했다** — 같은 문단이 "H-1이 정확히 이
계열이었으므로 명시한다"고 적고 있었다.

**논지는 어느 기준에서도 불변이다** — M-1은 임계 2에서만 잡히고 그때 적출이 437~440건이라
사람이 읽을 수 있는 양을 벗어난다. 바뀐 것은 숫자에 붙은 **리비전 이름**뿐이고, 축이
triage 필터라는 결론(G4·checklist §11.3)은 그대로다.

- exit: 0 (verifier 리포트 §3.2의 실측을 그대로 전재. 0A2는 이 표를 재측정하지 않았다 —
  0A evidence가 동결이므로 verifier의 독립 실측을 인용하는 것이 옳다)
