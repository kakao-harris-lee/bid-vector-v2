...
> **사전 §11이 닫는 넷 가운데 셋(`OPEN-REG-05`·`OPEN-QUAL-10`·`OPEN-QUAL-11`)이
> 여기서 「활성」으로 찍힌다.** 두 상류 파일의 행을 **고치지 않았기 때문**이며
> (out_of_scope), 사전 §11이 그 셋을 **결정 근거와 함께 등재**해 닫는다.
> 넷째 `OPEN-QUAL-06`은 **분해 방식이 2026-08-26에 이미 등재돼** §12.2에 있고,
> 2026-08-28의 **U-5가 남은 목록을 닫는다** — 사전 §3.2.3이 그 자리다.
> 상류 registry 갱신은 별도 slice의 몫이다(`regression-ledger.md` §10.3).

> **위 「활성」 열다섯 가운데 열둘은 사전이 소유자만 밝히고 정의를 붙이지 않은 것들이다**
> — 어느 자리에서 어떻게 다뤘는지는 `checklist.md` **A5**가 항목별로 적는다.
