...
- **낱말만이 아니라 그 옆의 값 자리까지 가려서 낸다.** 어휘 갈래가 매치하는 것은 **낱말**이라
  ...
  `낱말 [:=] 값`(낱말이 따옴표로 닫혀 있어도 좋다 — json 꼴)과 `낱말 바로 뒤에 붙는 값`
  까지 늘렸다. 구분자는 남기고 낱말 자리와 값 자리를 **따로** 가리므로 꼴은 읽을 수 있다.
  ① 그 꼴로 걸린 값은 **이 파일에 실리지 않고** ② **이 출력 자신이 다음 라운드의 매치가
  되지 않는다** — 재귀가 여기서 끊긴다.
...
> **가림이 덮는 것은 `낱말 [:=] 값`(낱말이 따옴표로 닫혀 있어도 좋다)과 낱말 바로 뒤에
> 붙는 값이다.** 그 꼴을 벗어난 자리(예로 값이 낱말보다 앞에 오는 줄)까지 덮는다고
> 주장하지 않는다. **덮는다고 적은 꼴에 대해서는 산문이 아니라 블록이 잰다** — 표본
> 일곱의 값 조각이 통째로든 여덟 자 이상 이어서든 출력에 남으면 `FAIL`이다.
