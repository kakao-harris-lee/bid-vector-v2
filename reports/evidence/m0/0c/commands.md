...
**C-1 ~ C-9의 블록은 `ef999de`**(**이 evidence가 좌표로 고정한 커밋**이며 `scope.md` yaml의
...
선언 SHA `ef999de`. 원본은 `_workspace/`(gitignore 대상)라 이 블록은 그 디렉터리가
있는 작업 트리에서만 재현된다.
...
```
for c in $(git log --format='%H' 2b05684..ef999de \
...
다시 돌리려면 위 두 자리의 `ef999de`를 HEAD로 바꾼다.
...
git diff --check 2b05684..ef999de | sed 's/[[:space:]]\{1,\}$/<후행공백>/'
echo "base_sha 기준 지적: $(git diff --check 2b05684..ef999de | wc -l | tr -d ' ')"
echo "review_base 기준 지적: $(git diff --check aff62ab..ef999de | wc -l | tr -d ' ')"
```
...
`MASKS`가 한 줄씩 밝힌다. 선언 SHA `ef999de`.
...
활성으로 복원", "임계 자체는 미결"이라 적는 행들이 그렇다. 선언 SHA `ef999de`.
...
`bid-vector` symlink가 있어야 재현된다. 선언 SHA `ef999de`.
...
docs/discovery/data-dictionary.md — 매치 0줄
    ...
    reports/evidence/m0/0c/commands.md:1635: ## C-8 · <가림 6자> 스캔
    reports/evidence/m0/0c/commands.md:1669: PATS = [("비밀 어휘", re.compile(r"(?i)(api[_-]?key|<가림 6자>|<가림 5자>|<가림 8자>|<가림 7자>|begin (rsa|ec|openssh))")),
    reports/evidence/m0/0c/commands.md:1670:         ("식별자",   re.compile(r"([0-9]{3}-[0-9]{2}-[0-9]{5}|<가림 7자>|telegram[_-]?id|@[A-Za-z0-9_]{5,}bot)"))]
    reports/evidence/m0/0c/commands.md:1681: PROBES = [("어휘 + 따옴표 값",  '<가림 7자> = <가림 19자>'),
    reports/evidence/m0/0c/commands.md:1682:           ("어휘 뒤 바로 값",   'Authorization: <가림 7자><가림 15자>'),
    reports/evidence/m0/0c/commands.md:1683:           ("쉼표 든 값",       'db_<가림 8자>=<가림 16자>'),
    reports/evidence/m0/0c/commands.md:1684:           ("어휘 + 콜론 값",    'slack_<가림 5자>: <가림 16자>'),
    reports/evidence/m0/0c/commands.md:1685:           ("따옴표 든 낱말",    '"<가림 7자>": <가림 17자>'),
    reports/evidence/m0/0c/commands.md:1686:           ("값 형태 — 번호",    '<가림 12자>'),
    reports/evidence/m0/0c/commands.md:1687:           ("값 형태 — 개인키",  '-----<가림 9자> PRIVATE KEY-----')]
    reports/evidence/m0/0c/commands.md:1814: > **공백으로 끝나는 어휘**(`PATS`에서는 `<가림 7자>` 하나뿐이다)일 때만 재고, 값이 **영숫자로
    reports/evidence/m0/0c/commands.md:1816: > 한 조각이어서 낱말 뒤에 구분자가 바로 오지 않는 줄**(`<가림 6자>_key = …` ·
    reports/evidence/m0/0c/commands.md:1817: > `client_<가림 6자>_id=…` · `<가림 5자>s = […]`)이나 값이 낱말보다 앞에 오는 줄 — 까지 덮는다고
...
**대상 파일** — **이 파일 자신**(읽는 체크아웃의 내용). **실행 트리** — `ef999de`
(C-1 ~ C-9가 선언한 SHA).
...
# 대상 파일 = 이 파일 자신(읽는 체크아웃의 내용) · 실행 트리 = ef999de (블록들이 선언한 SHA)
...
git worktree add --detach "$WT" ef999de >/dev/null 2>&1
...
환경 전제 — bid-vector: 있음 · _workspace: 있음
  ...
  축어 일치            for c in $(git log --format='%H' 2b05684..ef999de \
...
git worktree add --detach "$CT" ef999de >/dev/null 2>&1
