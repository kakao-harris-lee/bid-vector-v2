...
**C-1 ~ C-9의 블록은 `e67d177`**(**이 evidence가 좌표로 고정한 커밋**이며 `scope.md` yaml의
`head_sha`와 같은 값이다)의 트리를 선언한다 — 그 SHA를 체크아웃한 worktree에서 재현된다.
**이 자리는 순서를 말하지 않는다** — 「브랜치의 head」도 「마지막 커밋」도 아니다. 그렇게
적으면 **그 뒤에 커밋이 하나 붙는 순간 낡는다**(실제로 그랬다). 규약의 문면은 `scope.md`의
「`head_sha`와 range」 절에 있고 이 자리는 그것을 그대로 쓴다.
...
선언 SHA `e67d177`. 원본은 `_workspace/`(gitignore 대상)라 이 블록은 그 디렉터리가
있는 작업 트리에서만 재현된다.

**사본은 원본의 축어 복제가 아니다** — Codex 리뷰 finding **E**가 U-3의 소유 `OPEN`
오기를 지적했고, **결정 기록이므로 원 표기를 지우지 않고 취소선과 ⚠ 정정으로** 덧댔다.
**그 차이가 무엇인지도 몇 자리인지도 산문이 세지 않는다 — 아래 diff가 그대로 낸다.**
(앞서 이 자리가 「한 자리에서 다르다」라 세었고, 그 셈은 diff가 내는 것보다 좁았다.)

**경계는 행 번호가 아니라 날짜 머리로 잡는다.** 앞서 이 블록이 양쪽을 **행 번호로**
잘랐고, 사본의 머리가 길어지자 **머리 줄이 비교 대상 안으로 새어** 실재하지 않는 차이를
냈다. 원본 쪽도 마찬가지였다 — 「파일 끝까지」라 적어 두었으나 **원본은 그 뒤로 자라
2026-08-29 묶음이 붙었다.** **어느 쪽이 자라도 낡지 않는 경계로 바꿨다.**

```
# 경계를 **행 번호가 아니라 날짜 머리로** 잡는다 — 어느 쪽 파일이 자라도 낡지 않는다.
# 꼬리의 빈 줄과 구분선은 양쪽에서 같은 규칙으로 떼어 낸다(그 차이는 이 블록이 재지 않는다).
# diff 가 낸 줄을 그대로 실으면 이 파일이 후행 공백을 갖는다 — 표지로 바꿔 싣는다.
seg() {
  awk '/^## 2026-08-28 · U-1 결정/{f=1} /^## 2026-08-29 /{f=0} f' "$1" \
  | awk '{a[NR]=$0} END{n=NR; while(n>0 && (a[n]=="" || a[n]=="---")) n--;
                        for(i=1;i<=n;i++) print a[i]}'
}
diff <(seg _workspace/m0-open-decisions/decisions-log.md) \
     <(seg reports/evidence/m0/0c/decisions-2026-08-28.md) \
  | sed 's/[[:space:]]\{1,\}$/<후행공백>/'
...
---
> ~~`OPEN-REG-05`가 닫히고~~ `O-2` 귀속과 fixture 유보가 함께 풀린다.
> (⚠ U-1b 절의 정정 — **닫히지 않는다.** 과세 처리 축만 답한다.)
44,45c45,46
< U-1과 합쳐 **`OPEN-REG-05`("기초금액과 추정가격의 과세 처리 — 두 금액의 차이가 무엇으로
< 이루어지는가")가 닫힌다.**
---
> U-1과 합쳐 ~~**`OPEN-REG-05`("기초금액과 추정가격의 과세 처리 — 두 금액의 차이가 무엇으로
> 이루어지는가")가 닫힌다.**~~ → **그 `OPEN`의 과세 처리 축에 답한다.** ⚠ 아래 정정
47,51d47
< > **⚠ `A-35`·`A-36`·`A-37`.** 이 문단의 「닫힌다」는 이 파일 안에서 **더 이상 유효하지
< > 않다.** 정본 `regression-ledger.md:1345` 가 `OPEN-REG-05` 를 **활성**으로 적고
< > `docs/adr/0002:185,202` 도 활성 `OPEN` 으로 든다. U-1·U-1b 가 닫은 것은 **VAT 성분
< > 하나**이고, **차이의 잔차 성분**과 **저장 값의 행별 과세 처리**가 남는다.
<<후행공백>
95c91
< | `OPEN-REG-05` | **해소** — 0C가 근거와 함께 등재 |
---
> | `OPEN-REG-05` | ~~**해소** — 0C가 근거와 함께 등재~~ → **과세 처리 축만 답한다. `OPEN`은 활성** ⚠ 아래 정정 |
100,102c96,115
< > **⚠ `A-37`.** 위 표의 `OPEN-REG-05` **「해소」**는 정본과 어긋난다 — 정본 기준
< > **미해소**다. `R-BASIS-01` 유보가 사유만 바뀌고 유지되는 것이 그 `OPEN` 의 실질 효력이
< > 그대로임을 보여 준다. 나머지 금액 셋의 과세는 0C 가 **`OPEN-DIC-04`** 로 신설했다.
---
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 4 high #2 · 「귀속 정본표」 `A-35`·`A-36`·`A-37`)** —
> > 위 표와 이 절이 **`OPEN-REG-05`가 닫힌다**고 적었으나 **닫히지 않는다.**
> >
> > - **원 질문**(상태 정본 `regression-ledger.md` §9): *"기초금액과 추정가격의 과세 처리 —
> >   **두 금액의 차이가 무엇으로 이루어지는가**"*. **같은 행이 *"basis가 다르다는 사실은
> >   `OPEN-STR-01`로 확정됐으나 차이의 크기·과세 처리는 결정도 측정도 없다"*고 적는다.**
> > - **U-1·U-1b가 답한 것**: **두 금액의 과세 처리**(추정가격 `Exclusive` · 기초금액
> >   `Inclusive`). 이 절이 스스로 *"`OPEN-REG-05`는 두 질문을 묶고 있고 이 결정은 그중
> >   하나만 닫는다"*고 적어 두었다 — **그 자기 제한이 옳았고 아래 결론이 그것을 덮었다.**
> > - **남는 축 둘**: ① **두 금액 차이의 잔차 성분**(사정률 등) — **잔차 측정**이
> >   선행해야 하고 그 측정은 없다. ② **저장 값의 행별 실제 과세 처리** — 정의가 정해져도
> >   legacy 저장 값이 그 정의를 따른다는 근거가 없다. **`R-BASIS-01` fixture 유보가 사유만
> >   바뀌고 유지되는 것**이 그 `OPEN`의 실질 효력이 그대로임을 보여 준다.
> >
> > **`OPEN-REG-05`는 활성이다.** `docs/adr/0002:185`가 그것을 **이 ADR이 선점하지 않는 활성
> > `OPEN`**으로 들고 `:202`가 *"닫힌 뒤에 만든다"*로 적는 것과 이제 한목소리다.
> > **나머지 금액 셋**(`AllocatedBudget`·`YegaAmount`·`AwardAmount`)의 과세 처리는 이 결정이
> > 다루지 않으며 **`OPEN-DIC-04`**(활성)가 받았다.
> > **U-1·U-1b의 결정 내용은 바뀌지 않는다.** 원 표기는 지우지 않고 취소선으로 남긴다 —
> > **이 파일은 결정 기록이다.**
109c122
< | **U-3** | **`TenderOutcome` aggregate — current는 event stream의 fold 결과** | `OPEN-SET-05` |
---
> | **U-3** | **`TenderOutcome` aggregate — current는 event stream의 fold 결과** | ~~`OPEN-SET-05`~~ → ~~**`OPEN-SET-04`**~~ → **닫는 활성 `OPEN` 없음** ⚠ 아래 정정 둘 |
111c124
< | **U-8** | **`lmtGrpNo` = 요건 묶음** (그룹 간 OR / 그룹 내 AND) | `OPEN-QUAL-11` |
---
> | **U-8** | **`lmtGrpNo` = 요건 묶음** (그룹 간 OR / 그룹 내 AND) | ~~`OPEN-QUAL-11`~~ → **닫는 `OPEN` 없음** ⚠ 아래 정정 |
113,118d125
< > **⚠ `A-38`·`A-40`·`A-47`.** 위 표의 세 귀속이 정본과 어긋난다.
< > **U-2 → `OPEN-QUAL-10`**: 게시 요건 축이 남는다(`부분`).
< > **U-3 → `OPEN-SET-05`**: **오귀속** — SET-05 는 재공고 대사 대상 선택 규칙이라 무관하다.
< > 뒤의 정정 절이 `OPEN-SET-04` 로 갈아끼웠으나 **그것도 오귀속**이다(`A-41`).
< > **U-8 → `OPEN-QUAL-11`**: **오귀속** — 실제 소유자는 이미 닫힌 `OPEN-QUAL-01`.
<<후행공백>
129a137,160
...
> >
> > **⚠ 이 정정이 갈아끼운 대상도 오귀속이다 (2026-08-30, 「귀속 정본표」 `A-41`).**
> > **U-3이 닫는 활성 `OPEN`은 없다.**
> >
> > - **`OPEN-SET-04`의 원 질문**(`capability-map.md` §12): 이벤트 **재관측 횟수가
> >   사용자·운영에 필요한 정보인가** — 곧 **노출 여부**다. **아래 U-3 본문 자신이
> >   *"이 결정에 종속되나 별개다"*라 적고 *"0C가 재관측 횟수를 별도 필드로 두되 노출은
> >   정하지 않는다"*로 끝난다.** 종속은 귀속이 아니다.
> > - **U-3이 정한 것**: aggregate 경계와 **fold로의 재정의**. 그것은
> >   `capability-map.md:2190`의 **SET-09 `F-7` 「V2 제약」**이 이미 확정해 둔 것과 같다 —
> >   *"current snapshot을 event stream에서 재계산(fold)하는 구조로 둔다"*. **새로 닫는
> >   질문이 아니다.**
> > - **멱등 키와 재관측 키의 구성**은 U-3도 `OPEN-SET-04`도 정하지 않았고
> >   **`OPEN-DIC-07`**(활성, 사전 §9)이 받았다.
> >
> > **`OPEN-SET-04`와 `OPEN-SET-05` 둘 다 활성으로 남는다.** **정정이 새 오귀속을 만든
> > 자리다** — 갈아끼울 때도 그 `OPEN`의 원문 질문을 열어 대조한다.
><후행공백>
143,147d173
< > **⚠ `A-41` 의 근거가 이 문단이다.** 여기서 장부 자신이 `OPEN-SET-04` 를 **「종속이되
< > 별개」**로 정확히 적었다. 그런데 뒤의 정정 절이 U-3 의 소유 `OPEN` 을 `OPEN-SET-04` 로
< > 갈아끼웠다 — **이 문단과 모순된다.** 정본 판정: **U-3 이 닫는 활성 `OPEN` 은 없다.**
< > 멱등 키의 **구성**은 0C 가 **`OPEN-DIC-07`** 로 신설했다.
<<후행공백>
172c198,199
< **`OPEN-QUAL-11`은 해소**된다. **`OPEN-QUAL-09`(시공능력 미달 처리)는 별개로 유지**한다 —
---
> **`OPEN-QUAL-11`은 해소**된다.(⚠ 아래 정정 — **해소되지 않는다.** U-8은 그 질문에 답하지
> 않는다.) **`OPEN-QUAL-09`(시공능력 미달 처리)는 별개로 유지**한다 —
175,178d201
< > **⚠ `A-47`.** 「`OPEN-QUAL-11` 은 해소된다」는 **오귀속**이다. QUAL-11 은 허용업종의
< > **단독/결합 충족**을 묻고 U-8 은 그 (a)/(b) 어느 쪽도 고르지 않는다. 뒤의 정정 절이
< > 이것을 바로잡았고(`A-49`), 정본 판정도 그 정정을 지지한다.
<<후행공백>
181a205,231
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 3 high #1 · 운영자 정정)** — 위 표의 U-8 행과 이 절이
> > 소유 `OPEN`을 `OPEN-QUAL-11`로 적고 *"해소된다"*로 닫았으나 **그 귀속이 틀렸다.**
> > `capability-map.md` §12에서 그 id가 묻는 것은 **`permsnIndstrytyList`(허용업종)의 결합
> > 규칙 — 단독 충족인가 결합 충족인가**이고 선택지는 **(a) 허용업종만 보유해도 그 그룹
> > 충족 / (b) 제한 면허를 보유한 상태에서만 대체 경로로 인정**이다. **U-8이 정한 것은
> > `lmtGrpNo` 그룹 간 OR / 그룹 내 AND이며 (a)/(b) 어느 쪽도 고르지 않는다 — 다른
> > 질문이다.**
> > **U-8의 결정 내용은 바뀌지 않는다** — 바뀌는 것은 그것이 어느 `OPEN`을 닫는가뿐이고,
> > **U-8이 닫는 활성 `OPEN`은 없다**~~(그 결합 축을 소유한 `OPEN`이 따로 없었다)~~
> > (⚠ 아래 재정정 — 그 괄호가 거짓이다).
> > **`OPEN-QUAL-11`은 활성으로 되돌린다.** **운영자 판정(2026-08-30)**: 조달청 문서에
> > 근거가 없어 지금 답하지 않고 **M1 자격 slice(1C)가 실제 공고로 관측해 닫는다.**
> > 원 표기는 지우지 않고 취소선으로 남긴다 — **이 파일은 결정 기록이다.**
> >
> > **⚠ 재정정 (2026-08-30, 같은 날 · 운영자 정정)** — 위 괄호의 부재 주장
> > *"그 결합 축을 소유한 `OPEN`이 따로 없었다"*가 **거짓이다.**
> > **`OPEN-QUAL-01`이 그 축을 소유했고 운영자 결정 2026-08-26으로 이미 해소됐다** —
> > `capability-map.md` §12.2의 그 행이 결정을 U-8과 **같은 문장**(**(a)** 그룹 간 OR /
> > 그룹 내 AND)으로 적고 근거로 **조달청 OpenAPI 참고자료**를 들며, §12.1이 그것을
> > 해소로 등재한다. **결론은 그대로 선다 — 소유자가 없어서가 아니라 소유자가 이미
> > 닫혀 있어서다.** **U-8은 `OPEN-QUAL-01`의 확정을 2026-08-28에 재확인한 것**으로 읽는
> > 것이 실물에 맞고, 사전 §3.2.2가 그것을 사전의 언어로 적는다.
> > **같은 형태의 세 번째다** — ① U-3 → `OPEN-SET-05`(실제 ~~`OPEN-SET-04`~~ → **닫는 활성
> > `OPEN` 없음**, ⚠ U-3 절의 재정정) ② U-8 →
> > `OPEN-QUAL-11` · U-2/U-2b → `OPEN-QUAL-10` 전체 ③ **이번 — 「소유자가 없다」를 확인
> > 없이 단정.** **부재 주장도 그 자리를 열어 재고 나서 적는다.**
><후행공백>
191,195d240
< > **⚠ `A-44`.** 「신규 (미등록)」 주장은 **참으로 확인됐다** — 네 정본 전수 스윕에서 정책
< > version 의 **식별 방식**을 묻는 활성 행이 없다. **다만 이 절이 `:463-464` 에서 요구한
< > 반례가 실제로 나왔고 장부에 되돌아오지 않았다** — 0C 가 **`OPEN-DIC-01`**(면허·지역 축이
< > 정책 version 을 싣는가, 활성)로 등록했다. 「장부에 처음 들어오는 것」 절 참조.
<<후행공백>
241a287,289
> **「전건 완료」는 「물을 것을 다 물었다」이지 「잔여 미결이 없다」가 아니다** —
> 아래 마지막 절의 ⚠ 블록이 그 뜻을 적는다.
><후행공백>
244,246c292,294
...
---
> | **U-2b** | **시공능력평가금액은 직전 연도 공시값** | ~~`OPEN-QUAL-10` → **해소**~~ → **보유액 축의 기준 시점만 답함 · `OPEN`은 활성** ⚠ 아래 정정 |
> | **U-5** | **`UncertainReason` 네 값으로 확정** | ~~`OPEN-QUAL-06` → **해소**~~ → **이미 닫혀 있던 결정의 하류 실행** ⚠ 아래 정정 |
> | **U-9** | **"66% 오염" 인용 금지 유지. V2가 재정의·재측정** | ~~`OPEN-NUM-01` → 인용 금지로 닫음~~ → **`OPEN-NUM-01` 유지** ⚠ 아래 정정 |
248,253d295
< > **⚠ `A-39`·`A-43`·`A-50`.** 위 표의 세 귀속이 전부 정본과 어긋난다.
< > **U-2b → `OPEN-QUAL-10` 해소**: `부분` — 게시 요건 축과 보유액 축의 `unit` 이 남는다.
< > **U-5 → `OPEN-QUAL-06` 해소**: **오귀속** — 소유자는 맞으나 **2026-08-26 에 이미 닫혔다.**
< > **U-9 → `OPEN-NUM-01` 인용 금지로 닫음**: **오귀속** — 「인용 금지」는 그 `OPEN` 이 이미
< > 담고 있는 **잠정 처리**이지 답이 아니다. 답은 **측정 방법·모수·측정일**이다.
<<후행공백>
258c300
< **`OPEN-QUAL-10`이 닫힌다.**
---
> ~~**`OPEN-QUAL-10`이 닫힌다.**~~(⚠ 아래 정정 — **닫히지 않는다.** 두 결정이 답한 것은 **보유액 축의 과세 처리와 기준 시점**뿐이다.)
260,263d301
< > **⚠ `A-39`.** 「`OPEN-QUAL-10` 이 닫힌다」는 정본과 어긋난다 — **게시 요건 축**
< > (`cnstrtnAbltyEvlAmtList`)이 남고, 보유액 축 안에서도 **`unit`** 을 두 결정 어느 쪽도
< > 정하지 않았다(`data-dictionary.md:1551,1554`). 갱신 주기·시행 구간은 **`OPEN-DIC-02`**.
<<후행공백>
270a309,323
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 3 high #2 · 운영자 정정)** — 위 표의 U-2b 행과 이 절이
> > **`OPEN-QUAL-10`이 닫힌다**고 적었으나 **절반만 닫힌다.**
> > `capability-map.md` §12의 그 행은 **공고 게시 요건(`cnstrtnAbltyEvlAmtList`)과 운영자
> > 보유액 양쪽**의 unit·basis·과세·기준 시점을 묻고, 같은 행이 *"조달청 문서는 이 필드에
> > 단위·과세를 명시하지 않는다"*고 적는다 — **그 사실이 이 `OPEN`의 존재 이유다.**
> > **U-2·U-2b가 정한 것은 운영자 보유액 축뿐이고 공고 게시 요건 축은 정해지지 않았다.**
> > **그 보유액 축 안에서도 `unit`은 두 결정이 정한 것이 아니다** — U-2는 과세 처리를,
> > U-2b는 기준 시점을 정했고 **단위를 정한 결정은 없다**(이 파일 자신이 legacy에
> > *"「원」이라는 주석만"* 있다고 적는다). 보유액의 단위를 정하는 것은 **V2가 직접 받는
> > 입력의 계약**이다(`Money`, `v2-지침서.md` §4.1) — 근거는 사전 §11의 문단이 댄다.
> > **`OPEN-QUAL-10`은 게시 요건 축에 대해 활성으로 되돌린다.** 지금 답할 근거가 없다 —
> > **관측으로만** 알 수 있고 **M1이 게시값을 실제로 수집한 뒤** 닫힌다(사전 §5.5 · §13.4).
> > **U-2·U-2b의 결정 내용은 바뀌지 않는다.** 원 표기는 지우지 않고 취소선으로 남긴다 —
> > **이 파일은 결정 기록이다.**
><후행공백>
284a338,353
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 4 · 「귀속 정본표」 `A-43`)** — 위 표의 U-5 행이
> > 소유 `OPEN`을 *"`OPEN-QUAL-06` → **해소**"*로 적었다. **소유자는 맞으나 그 `OPEN`은
> > U-5보다 먼저 닫혀 있었다.**
> >
> > - **원 질문**(`capability-map.md` §12): `Uncertain` 판정을 **분해할 것인가.**
> > - **답한 것**: **운영자 결정 2026-08-26.** §12.1이 그 행을 취소선으로 해소 등재하고
> >   §12.2가 상태 **`확정`**(추가 작업 없음)으로 적는다.
> > - **U-5가 한 것**: 그 확정이 요구한 *"사유 enum은 legacy evidence 문자열 이식이 아니라
> >   도메인 명세가 먼저 정한다"*의 **실행**. **값 목록은 그 `OPEN`의 답이 아니다** — 같은
> >   §12.2 행이 *"scout 노트의 4분류는 **후보이지 확정 목록이 아니다**"*라 적어 그 축을
> >   **답에서 명시적으로 제외**한다.
> >
> > **한 번 닫힌 것을 두 번 세지 않는다.** **U-8 → `OPEN-QUAL-01`(⚠ U-8 절의 재정정)과 같은
> > 형태이며 이 파일에서 두 번째다.** **U-5의 결정 내용과 사전 §3.2.3의 등재는 그대로
> > 유효하다.** 원 표기는 지우지 않고 취소선으로 남긴다 — **이 파일은 결정 기록이다.**
><후행공백>
295a365,379
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 3 medium)** — 위 표의 U-9 행이 소유 `OPEN`을
> > *"`OPEN-NUM-01` → **인용 금지로 닫음**"*이라 적었고, **같은 파일의 「유지되는 OPEN」
> > 절은 그것을 유지되는 `OPEN`으로 분류한다. 한 파일 안에서 서로 모순이었다.**
> >
> > **정본은 `capability-map.md`다.** 그 파일 §12의 `OPEN-NUM-01` 행은 결정 필요 사항을
> > *"「저장된 기초금액의 66%가 진짜가 아님」 주장"*으로 적고, 선택지 칸에
> > *"측정 방법·모수·측정일 부재. 5곳에 문자열 복제. **재측정 전 인용 금지**"*라 적는다.
> > **「인용 금지」는 그 `OPEN`이 이미 담고 있는 잠정 처리이지 그 `OPEN`의 답이 아니다** —
> > 답은 **측정 방법·모수·측정일**이고 그것은 **재측정으로만** 나온다.
> >
> > **따라서 `OPEN-NUM-01`은 유지된다.** U-9가 한 것은 **그 잠정 처리를 그대로 두기로
> > 확정한 것**이고, **`OPEN`을 닫은 것이 아니다.** 「유지되는 OPEN」 절과 사전 §7.4가
> > 맞고 이 행이 틀렸다. `OPEN-REG-04`(ledger §9)도 같다.
> > 원 표기는 지우지 않고 취소선으로 남긴다 — **이 파일은 결정 기록이다.**
><후행공백>
300,303c384,390
< > **⚠ 「전건 완료」는 「잔여 없음」이 아니다.** 이 제목은 **운영자에게 물을 것을 다
< > 물었다**는 뜻이며, 결과적으로 참이었다. 그러나 **같은 U-묶음이 활성 `OPEN` 을 일곱 개
< > 만들었다**(`OPEN-DIC-01`~`07`, 0C 등록). 「잔여 없음」으로 읽으면 **거짓**이다.
< > 「장부에 처음 들어오는 것」 절 참조.
---
> > **⚠ 「전건 완료」가 뜻하는 것 (2026-08-30 · 「귀속 정본표」 전수 반영)** —
> > **「운영자에게 물어야 할 것을 다 물었다」**는 뜻이다. **「잔여 미결이 없다」는 뜻이
> > 아니다.** 0C가 사용자 입력을 더 기다리지 않고 착수한 판단은 옳았으나, **같은 U-묶음이
> > 활성 `OPEN`을 남겼고**(`OPEN-DIC-NN`, 사전 §9) **해소한 활성
> > `OPEN`은 없다**(아래 「해소되는 OPEN」의 ⚠ 정정). **물을 것을 다 묻는 것과 남은 미결이
> > 없는 것은 다르다.** **셈은 이 주석이 내지 않는다** — `commands.md` **C-6.2**·**C-6.3**이
> > 낸다. 앞서 이 자리가 셈을 적었고 **`OPEN`이 늘 때마다 낡았다.**
308c395
< | U-1b | 기초금액 **부가세 포함** → **`OPEN-REG-05` 해소** |
---
> | U-1b | 기초금액 **부가세 포함** → ~~**`OPEN-REG-05` 해소**~~ **과세 처리 축만 답함 · `OPEN`은 활성** ⚠ U-1b 절의 정정 |
310c397
< | U-2b | **직전 연도 공시값** → **`OPEN-QUAL-10` 해소** |
---
> | U-2b | **직전 연도 공시값** → ~~**`OPEN-QUAL-10` 해소**~~ **보유액 축의 기준 시점만 답함 · `OPEN`은 활성** ⚠ U-2b 절의 정정 |
313c400
< | U-5 | `UncertainReason` **네 값 확정** → **`OPEN-QUAL-06` 해소** |
---
> | U-5 | `UncertainReason` **네 값 확정** → ~~**`OPEN-QUAL-06` 해소**~~ **이미 닫혀 있던 결정의 하류 실행** ⚠ U-5 절의 정정 |
316,317c403,404
< | U-7 | 면허 유효기간 **다루지 않음** → `OPEN-QUAL-05` 알려진 제한 |
< | U-8 | `lmtGrpNo` = **요건 묶음** → **`OPEN-QUAL-11` 해소** |
---
> | U-7 | 면허 유효기간 **다루지 않음** → `OPEN-QUAL-05` 알려진 제한 (**활성 유지** — 종료 조건인 `v2-지침서.md` §4.2 개정이 집행 유예다) |
> | U-8 | `lmtGrpNo` = **요건 묶음** → ~~**`OPEN-QUAL-11` 해소**~~ **닫는 `OPEN` 없음** ⚠ U-8 절의 정정 |
321,325c408
< > **⚠ `A-37`·`A-39`·`A-43`·`A-47`.** 위 요약표의 네 「해소」 화살표가 전부 정본과
< > 어긋난다 — `OPEN-REG-05`(미해소) · `OPEN-QUAL-10`(부분) · `OPEN-QUAL-06`(이미 닫힌
< > 소유자를 재폐쇄) · `OPEN-QUAL-11`(무관한 소유자). 판정은 「귀속 정본표」가 이긴다.
<<후행공백>
< **해소되는 OPEN**: `OPEN-REG-05` · `OPEN-QUAL-10` · `OPEN-QUAL-06` · `OPEN-QUAL-11`
---
> **해소되는 OPEN**: ~~`OPEN-REG-05`~~ · ~~`OPEN-QUAL-10`~~ · ~~`OPEN-QUAL-06`~~ · ~~`OPEN-QUAL-11`~~ → **없다** ⚠ 아래 정정
328,329c411,435
< **유지되는 OPEN**: `OPEN-QUAL-05`(알려진 제한) · `OPEN-QUAL-09`(시공능력 미달 처리) ·
< `OPEN-ML-05`(정책 값 경계) · `OPEN-SET-04`(재관측 노출) · `OPEN-NUM-01`/`OPEN-REG-04`
---
> > **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 4 · 「귀속 정본표」 전수 반영)** — **이 묶음이
> > 해소한 활성 `OPEN`은 없다.** 셋을 하나씩 다시 잰 결과:
> >
> > - **`OPEN-REG-05`** — **활성.** 상태 정본 `regression-ledger.md` §9의 그 행이 *"두 금액의
> >   차이가 무엇으로 이루어지는가"*를 묻고 *"차이의 크기·과세 처리는 결정도 측정도 없다"*고
> >   적는다. U-1·U-1b는 **과세 처리 축**에만 답했다. 남는 축 둘 — **두 금액 차이의 잔차
> >   성분**과 **저장 값의 행별 실제 과세 처리**. `docs/adr/0002:185`도 그것을 활성으로 든다.
> > - **`OPEN-QUAL-10`** — **활성.** 게시 요건 축 전부에 더해 **보유액 축의 `unit`**도 두
> >   결정이 정한 것이 아니다(사전 §11).
> > - **`OPEN-QUAL-06`** — **U-5보다 먼저 닫혀 있었다.** 운영자 결정 2026-08-26이 분해 여부에
> >   답했고 `capability-map.md` §12.1이 해소로, §12.2가 상태 `확정`으로 등재한다. 같은
> >   §12.2 행이 *"scout 노트의 4분류는 후보이지 확정 목록이 아니다"*라 적어 **값 목록을 그
> >   답에서 제외**한다. U-5는 그 확정의 **하류 실행**이다.
> >
> > **결정 내용은 어느 것도 바뀌지 않는다.** 원 표기는 지우지 않고 취소선으로 남긴다 —
> > **이 파일은 결정 기록이다.**
><후행공백>
> **유지되는 OPEN**: `OPEN-QUAL-05`(알려진 제한 — **답을 받고도 활성**이며 종료 조건인
> `v2-지침서.md` §4.2 개정이 운영자 결정 Q2로 정해졌으나 **집행이 M0 종료 slice**다) ·
> `OPEN-QUAL-09`(시공능력 미달 처리) ·
> **`OPEN-REG-05`**(잔차 성분 · 저장 값의 행별 과세 — ⚠ 위 정정) ·
> **`OPEN-QUAL-10`의 게시 요건 축과 보유액의 `unit`**(⚠ U-2b 절의 정정 · 위 정정) ·
> **`OPEN-QUAL-11`**(허용업종 결합 규칙 — ⚠ U-8 절의 정정) ·
> `OPEN-ML-05`(정책 값 경계) · `OPEN-SET-04`(재관측 노출 — **U-3이 닫지 않는다**, ⚠ U-3 절의
> 정정) · `OPEN-SET-05`(재공고 대사 대상 선택 규칙) · `OPEN-NUM-01`/`OPEN-REG-04`
332,335c438,442
< > **⚠ 「해소되는 OPEN」 넷은 전부 판정이 바뀌었다** — `A-37`·`A-39`·`A-43`·`A-47`.
< > **「유지되는 OPEN」 목록은 정본과 일치한다**(`A-51`) — 그래서 이 파일은 `:497` 의
< > 「`OPEN-NUM-01` 닫음」과 여기의 「`OPEN-NUM-01` 유지」가 **서로 모순**인 상태였다.
< > 「귀속 정본표」가 앞쪽을 오귀속으로 판정해 모순을 해소한다.
---
> **그리고 이 묶음이 활성 `OPEN`을 남겼다** — `OPEN-DIC-NN`(사전 §9).
> **어느 것이 이 묶음의 결정이 남긴 미완에서 직접 나왔는지는 사전 §9의 「어느 결정의
> 잔여인가」 칸이 적는다** — 여기서 열거하지 않는다. **셈도 열거도 이 주석이 내지 않는다** —
> `commands.md` **C-6.2**·**C-6.3**이 낸다.
> **「전건 완료」는 「잔여 없음」이 아니다** — 아래 절 머리가 그것을 적는다.
exit=1
```
...
```
for c in $(git log --format='%H' 2b05684..e67d177 \
...
```
-- e67d177 docs(m0-0c): 앞 커밋이 §1.1 에 남긴 들쭉날쭉한 줄바꿈을 이 파일의 폭에 맞춘다
   docs/discovery/data-dictionary.md
-- 3829137 docs(m0-0c): 출력 블록 전부를 선언 SHA e9c321f 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- e9c321f fix(m0-0c): OPEN-DIC-09 의 근거 귀속을 축별로 가르고 OPEN-DIC-10 이 「금액 축 밖」의 경계를 갖게 한다 (검증 H-1·J-2)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- ec91497 docs(m0-0c): 출력 블록 전부를 선언 SHA ef999de 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- ef999de fix(m0-0c): OPEN-DIC-09 의 되짚기를 두 물음 모두로 넓힌다 (F-3 정밀도)
   docs/discovery/data-dictionary.md
-- 273b750 docs(m0-0c): 출력 블록 전부를 선언 SHA 35e9b0f 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 35e9b0f fix(m0-0c): OPEN-DIC-10 의 선택지를 배타·소진으로 다시 적는다 (검증 O-1)
   docs/discovery/data-dictionary.md
-- 2c44b95 fix(m0-0c): OPEN-DIC-09 의 「늦은 관측」 갈래를 걷는다 — 필드별 fold 가 이미 낸다 (검증 F-3)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- 4b8cb92 fix(m0-0c): OPEN-DIC-10 의 범위를 한 쪽으로 정한다 — 금액 축은 정의 ① 소유 (검증 F-2)
   docs/discovery/data-dictionary.md
-- 41f63d8 fix(m0-0c): §4.1 인용을 불릿 전문으로 되돌리고 자리수의 근거를 §1.1 두 행으로 옮긴다 (검증 F-1)
   docs/discovery/data-dictionary.md
-- d210d8a docs(m0-0c): 출력 블록 전부를 선언 SHA 7079215 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 7079215 fix(m0-0c): 새 OPEN 두 행의 라운드 표기를 문서 규약에 맞춘다 (C-4.1 수복)
   docs/discovery/data-dictionary.md
-- 0edf514 fix(m0-0c): RoundingPolicy 의 최소 계약을 정의하고 값을 OPEN-DIC-10 이 소유한다 (codex 5차 medium)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- 50a464d fix(m0-0c): fold 의 순서 술어 — 형태를 정의하고 키를 OPEN-DIC-09 로 등재한다 (codex 5차 high #2)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- 2d0ce2b fix(m0-0c): 파생값의 되짚기 축을 OPEN-DIC-08 로 등재한다 (codex 5차 high #1)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- 809552c fix(m0-0c): OPEN-DIC 셈을 산문에서 걷는다 — 블록이 낸다 (codex 5차 선행)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/codex-review-20260830T090353Z.json
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- cf1b6a3 docs(m0-0c): 출력 블록 전부를 선언 SHA 285341c 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 285341c fix(m0-0c): §1.4.2 ⚠ 정정의 전칭 열거 절을 걷는다 — 전수를 견디지 못했다 (검증 V-2)
   docs/discovery/data-dictionary.md
-- fe988bf docs(m0-0c): 출력 블록 전부를 선언 SHA db5d4b5 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- db5d4b5 fix(m0-0c): §1.4.2 ⚠ 정정의 근거 ② 를 걷는다 — 앞 문면에 대해 거짓이었다 (검증 V-1)
   docs/discovery/data-dictionary.md
-- d42b35c docs(m0-0c): 출력 블록 전부를 선언 SHA 63aed9a 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 63aed9a fix(m0-0c): 사전이 ADR 을 번호가 아니라 결정 이름으로 가리킨다 (C-4.1 FAIL 수복)
   docs/discovery/data-dictionary.md
-- 5ad30ad docs(m0-0c): Codex 4차 verdict 를 evidence 에 등재한다 (내용 무수정)
   reports/evidence/m0/0c/codex-review-20260830T053622Z.json
-- bdcb887 fix(m0-0c): 귀속 전수 정정 나머지와 OPEN-DIC 일곱의 잔여 귀속 (「귀속 정본표」 A-01~A-63)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 4be3800 fix(m0-0c): OPEN-REG-05 활성 복원 · OPEN-QUAL-05 자기모순 해소 · 귀속 넷 정정 (codex 4차 high #2 · medium)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 1624c19 fix(m0-0c): 율 타입의 carrier 를 축별로 판정 — 둘은 넓히고 하나는 요구를 좁힌다 (codex 4차 high #1)
   docs/discovery/data-dictionary.md
-- 43bbf62 docs(m0-0c): 출력 블록 전부를 선언 SHA 9c55ede 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 9c55ede fix(m0-0c): 보유액 축의 unit 을 정한 결정이 없다 — 근거를 대고 귀속을 쪼갠다 (F-4)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/scope.md
-- 0faf79b fix(m0-0c): 율 타입 넷의 층을 실물에 맞춰 재배정한다 — BidRate 는 legacy-behavior (F-3)
   docs/discovery/data-dictionary.md
-- 9811fb7 fix(m0-0c): 시공능력 요건 금액의 서명을 Money 에서 UnnormalizedFigure 로 바꾼다 (F-2)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
-- 2f7cb05 fix(m0-0c): 결정 사본 U-8 산문에 인라인 정정 표시를 단다 (F-5)
   reports/evidence/m0/0c/decisions-2026-08-28.md
-- 0c1b7ab fix(m0-0c): U-8 의 축을 소유한 OPEN 은 OPEN-QUAL-01 이고 이미 닫혀 있었다 (F-1)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 8c34179 docs(m0-0c): 출력 블록 전부를 선언 SHA ca8a13b 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- ca8a13b fix(m0-0c): C-6.3 각주가 세 OPEN 을 한 갈래로 묶은 것을 가른다 (전수 훑기 잔여)
   reports/evidence/m0/0c/commands.md
-- 0124dd4 fix(m0-0c): head_sha 뒤 후속 커밋 성격의 열거를 닫지 않는다 (codex 3차 low)
   reports/evidence/m0/0c/scope.md
-- 4df8de8 fix(m0-0c): 결정 사본의 U-9 행 자기모순을 정본 대조로 닫는다 (codex 3차 medium)
   reports/evidence/m0/0c/decisions-2026-08-28.md
-- e963913 fix(m0-0c): 율 타입 넷에 provenance·층·부재 표현을 붙이고 §12.2 에 등재한다 (codex 3차 high 3)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 824ad39 fix(m0-0c): OPEN-QUAL-10 을 게시 요건 축에 대해 활성으로 되돌린다 (codex 3차 high 2)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 51749db fix(m0-0c): OPEN-QUAL-11 을 활성으로 되돌린다 — U-8 은 다른 질문에 답한다 (codex 3차 high 1)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/checklist.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 8c00ced docs(m0-0c): Codex 3차 verdict JSON 을 내용 무수정으로 등재한다
   reports/evidence/m0/0c/codex-review-20260830T034122Z.json
-- f0bdfbb fix(m0-0c): :725 뒷절을 앞절과 같은 5860da8 앵커의 과거 기록으로 맞춘다 (게이트 high 1)
   reports/evidence/m0/0c/scope.md
-- ae99571 fix(m0-0c): 라운드 13 표에 footer 를 달고 :660 의 두 벌을 가리키기로 줄인다 (운영자 판정)
   reports/evidence/m0/0c/scope.md
-- d28dff8 fix(m0-0c): :725 의 라운드 13 판정을 현재형에서 그때의 기록으로 못 박는다 (운영자 판정)
   reports/evidence/m0/0c/scope.md
-- 4e49a17 fix(m0-0c): :603 의 「규약대로」 단정을 한 것으로 바꾼다 (운영자 판정)
   reports/evidence/m0/0c/scope.md
-- 01dcb02 fix(m0-0c): 다른 레인이 소유한 셈 세 줄을 걷고 남은 「규약대로」 단정 둘을 한 것으로 바꾼다 (검증 Q-1·Q-2)
   reports/evidence/m0/0c/scope.md
-- 89ac05b docs(m0-0c): scope.md 에 수정 라운드 13 을 등재한다
   reports/evidence/m0/0c/scope.md
-- 5860da8 fix(m0-0c): 근거절을 블록의 실제 출력에 맞추고 되풀이·뭉침·규약 단정을 걷는다 (검증 P-1~P-4)
   reports/evidence/m0/0c/scope.md
-- dbbd258 docs(m0-0c): scope.md 에 수정 라운드 12 를 등재한다
   reports/evidence/m0/0c/scope.md
-- 767cbfb docs(m0-0c): 출력 블록 전부를 선언 SHA 6724943 트리에서 다시 뜨고 N-1·N-2 를 같은 커밋에서 닫는다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 6724943 fix(m0-0c): §12.2 각주의 분자·분모 요구 귀속을 §7.1 로 좁힌다 (검증 N-3)
   docs/discovery/data-dictionary.md
-- 85b9ac9 docs(m0-0c): 이력 표에 f0fc837·cb2cbdb 두 행을 세운다
   reports/evidence/m0/0c/scope.md
-- cb2cbdb fix(m0-0c): 이력 표가 나른 비율 미저장의 옛 근거를 걷는다 (검증 L-3 전수)
   reports/evidence/m0/0c/scope.md
-- f0fc837 docs(m0-0c): scope.md 에 수정 라운드 11 을 등재한다
   reports/evidence/m0/0c/scope.md
-- 9dd5dab docs(m0-0c): 출력 블록 전부를 선언 SHA e88b59f 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- e88b59f fix(m0-0c): §7.3 머리의 귀속과 비율 미저장의 근거를 정확히 적는다 (검증 L-3)
   docs/discovery/data-dictionary.md
-- 8419388 fix(m0-0c): §5.2 경계 규칙의 귀속을 「그대로 적용」에서 「같은 형태」로 좁힌다 (검증 L-2)
   docs/discovery/data-dictionary.md
-- ac6d10b fix(m0-0c): §7.1 C1 술어를 열거로 닫는다 — 옆 문장에 기대지 않는다 (검증 L-1)
   docs/discovery/data-dictionary.md
-- 6ce5a80 fix(m0-0c): 결정 사본과 원본의 차이를 세는 산문 둘을 지운다 (검증 M-2)
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- 0f07fd8 fix(m0-0c): NoticePublished 배제 근거를 실제로 그 형태를 막는 자리로 옮긴다 (검증 M-1)
   docs/discovery/data-dictionary.md
   reports/evidence/m0/0c/scope.md
-- b244bff docs(m0-0c): scope.md 에 Codex 수정 라운드 2 를 등재한다
   reports/evidence/m0/0c/scope.md
-- 40e9535 docs(m0-0c): 출력 블록 전부를 선언 SHA 6d0fe39 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- 6d0fe39 fix(m0-0c): C-1 의 비교 경계를 행 번호에서 날짜 머리로 옮긴다 — 「파일 끝까지」도 거짓이었다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
-- 9520e41 docs(m0-0c): checklist 를 이 라운드의 산출물 변경에 맞춘다 (A2 · A7)
   reports/evidence/m0/0c/checklist.md
-- 81e4aa9 fix(m0-0c): 결정 사본의 「그대로 옮겼다」를 실제 형태로 고치고 세 자리를 함께 훑는다 (codex 2차 medium 5)
   reports/evidence/m0/0c/codex-review-20260829T231517Z.json
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/decisions-2026-08-28.md
   reports/evidence/m0/0c/scope.md
-- b305e41 fix(m0-0c): head_sha 뒤 0C 커밋의 개수 단정을 지우고 명령을 가리킨다 (codex 2차 medium 4)
   reports/evidence/m0/0c/scope.md
-- 4d8b84e fix(m0-0c): 멱등 단위와 재관측 단위를 두 키로 가르고 키의 구성은 OPEN 으로 올린다 (codex 2차 high 3)
   docs/discovery/data-dictionary.md
-- 0f05b82 fix(m0-0c): 오염 측정 타입이 §7.1·§7.2의 요구를 담게 하고 C-4.2의 한계를 적는다 (codex 2차 high 2)
   docs/discovery/data-dictionary.md
-- 578718b fix(m0-0c): provenance 축의 모순을 걷는다 — OperatorDeclared 신설, 중복 필드 제거, C2 주장 정정 (codex 2차 high 1)
   docs/discovery/data-dictionary.md
-- 197bea3 docs(m0-0c): 이력 표에 bf03494 · 78b097b 두 행을 세운다
   reports/evidence/m0/0c/scope.md
-- 78b097b fix(m0-0c): 라운드 10 행의 표본 수를 실측으로 고치고 검증자 라벨을 걷는다
   reports/evidence/m0/0c/scope.md
-- bf03494 docs(m0-0c): 이력 표에 앞뒤 이력 커밋 두 행을 세운다 (87e2451 · aea96c7)
   reports/evidence/m0/0c/scope.md
-- aea96c7 docs(m0-0c): scope.md 에 수정 라운드 10 을 등재한다
   reports/evidence/m0/0c/scope.md
-- f388a69 docs(m0-0c): 출력 블록 전부를 선언 SHA c941828 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- c941828 docs(m0-0c): 한계의 예시가 접미 붙은 키 계열을 포함하게 한다
   reports/evidence/m0/0c/commands.md
-- 6925f53 fix(m0-0c): 「덮는다」의 조건(영숫자 시작·길이 하한·따옴표 짝)을 적어 서술을 실물에 맞춘다 (W-2 low)
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
-- f4d1fe5 fix(m0-0c): 대조군 라벨을 실측에 맞춰 되돌린 편집과 결과를 하나씩 짝짓는다 (W-1 medium)
   reports/evidence/m0/0c/scope.md
-- 87e2451 docs(m0-0c): 라운드 9 이력에 V-1 잔여 수정과 둘째 재취득을 등재한다
   reports/evidence/m0/0c/scope.md
-- 57ddeb2 docs(m0-0c): 출력 블록 전부를 선언 SHA 1aa4826 트리에서 다시 뜨고 head_sha 를 함께 옮긴다
   reports/evidence/m0/0c/commands.md
   reports/evidence/m0/0c/scope.md
...
다시 돌리려면 위 두 자리의 `e67d177`를 HEAD로 바꾼다.
...
git diff --check 2b05684..e67d177 | sed 's/[[:space:]]\{1,\}$/<후행공백>/'
echo "base_sha 기준 지적: $(git diff --check 2b05684..e67d177 | wc -l | tr -d ' ')"
echo "review_base 기준 지적: $(git diff --check aff62ab..e67d177 | wc -l | tr -d ' ')"
```
...
`MASKS`가 한 줄씩 밝힌다. 선언 SHA `e67d177`.
...
문서: docs/discovery/data-dictionary.md
§12.2 표 칸이 덮는 이름: 75
① 타입 선언 인자 + ② 필드·성분 표에서 뽑은 이름: 70  미덮개: 0
③ 본문의 단독 백틱 이름: 77  덮개·①②·목록·해시 밖: 0
...
③이 낸 이름을 빼는 갈래 — 덮개 45 · ①② 추출 0 · NOT_A_FIELD 목록 29 · commit 해시 규칙 3 · 남은 것 0
...
목록의 크기와 이번 실행이 실제로 쓴 항목 — NOT_A_DECL 10/10 · NOT_A_FIELD 29/29 · LEGACY_DECLS 2/2  쓰이지 않은 항목: 0
    (없음)
③ 규칙 밖 — `([a-z][A-Za-z0-9_]*)` 는 맞고 `([a-z][A-Za-z0-9]*)` 는 아닌 백틱 이름: 27 (덮개·①② 안 1 · 이 검사 어디에도 없음 26)
    ...
    not_applicable · observed_at · opened_at · opening_checked_at · planned_price ·
    policy_version · sample_scope · sample_size · separate_regime · std_bid_rate · std_rate ·
    total_count · winning_rate
④ ③ 규칙 밖 규칙도 못 보는 한정 이름 — `([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)+)`: 13 (판정에 넣지 않고 이름째 낸다 — 각주)
    ...
    TenderResultEvent.observed_at · bid_target.py · checklist.md · commands.md · date.min ·
    decisions.md · field_contract_spec.py · money.provenance
...
선언 앵커가 읽은 자리 71 ⊆ 사각지대 앵커가 본 자리 81 — 그 밖으로 샌 자리: 0
...
이름 규칙 밖 인자(legacy 선언 밖): 0자리
    (없음)
이름 없이 타입만 적힌 인자: 17자리
    Actual(Instant) · AssessmentRate(Rate) · AwardRate(Rate) · BidRate(Rate) · Column(Float) ·
    FloorRate(Rate) · Known(Money) · OperatorAction(Review) · OperatorAction(Skip) ·
    OperatorAction(Submit) · Resolved(PolicyVersion) · Resolved(T) · Scheduled(Instant) ·
    Substituted(Instant) · Uncertain(RequirementDataAbsent) · Uncertain(RequirementDataAbsent) ·
    Uncertain(RequirementDataAbsent)
...
활성으로 복원", "임계 자체는 미결"이라 적는 행들이 그렇다. 선언 SHA `e67d177`.
...
```
  ...
  OPEN-DIC-06   이 문서가 신설
  OPEN-DIC-07   이 문서가 신설
  OPEN-DIC-08   이 문서가 신설
  OPEN-DIC-09   이 문서가 신설
  OPEN-DIC-10   이 문서가 신설
  ...
  OPEN-STR-01   결정 완료 — 사전이 등재한다
  OPEN-STR-03   결정 완료 — 사전이 등재한다
  OPEN-STR-07   결정 완료 — 사전이 등재한다

신설: 10 · 활성: 15 · 결정 완료: 19 · 미분류: 0
exit=0
```

> **이 분류의 읽는 법과 항목별 처리는 `checklist.md` **A5**가 적는다** — 이 파일은
> 그 판정을 싣지 않는다. **이 블록이 읽는 것은 상류 registry의 상태뿐이다.**
> **「활성」으로 찍힌 셋은 전부 실제로 활성이다** — `OPEN-REG-05`(과세 처리 축만 답했고
> 잔차 성분과 저장 값의 행별 과세가 남는다) · `OPEN-QUAL-10`(게시 요건 축과 보유액의
> `unit`이 남는다) · `OPEN-QUAL-11`(통째로 활성). 앞서 이 자리가 셋을 **「닫혔는데 상류
> 행만 안 고쳐 활성으로 찍히는 것」**으로 적었고 **셋 다에 대해 거짓이었다**
> (사전 §11의 ⚠ 정정 셋 · `checklist.md` **A5**). 상류 registry 갱신은 별도 slice의 몫이며
> **그 미갱신이 이 셋의 활성 사유가 아니다.**

---

## C-7 · legacy 인용 (A6)

`bid-vector` symlink가 있어야 재현된다. 선언 SHA `e67d177`.
...
- **낱말만이 아니라 그 옆의 값 자리까지 가려서 낸다.** 어휘 갈래가 매치하는 것은 **낱말**이라
  ...
  **구분자 갈래**(`낱말 [:=] 값` — 낱말이 따옴표로 닫혀 있어도 좋다, json 꼴)와
  **붙는 값 갈래**(`낱말 바로 뒤에 붙는 값`)까지 늘렸다. 구분자는 남기고 낱말 자리와
  값 자리를 **따로** 가리므로 꼴은 읽을 수 있다. **두 갈래에는 각각 조건이 붙고 그
  조건은 이 절의 각주가 적는다.** ① 그 조건 안에서 걸린 값은 **이 파일에 실리지 않고**
  ② **이 출력 자신이 다음 라운드의 매치가 되지 않는다** — 재귀가 여기서 끊긴다.
  **판정 어휘(`PATS`)는 그대로다 — 고친 것은 인쇄지 판정이 아니다.**
- **능력을 이 디렉터리의 현재 내용이 아니라 심어 둔 가짜 값 일곱으로 잰다.** 내용이 깨끗하면
  스캔은 잡을 것이 없어도 침묵하므로, 침묵이 능력을 뜻하지 않는다. 표본 일곱은 **어휘가
  값을 데리고 오는 다섯 꼴**과 **값 형태 둘**을 각각 하나씩 든다 — 따옴표 든 낱말(json 꼴)과
  쉼표 든 값이 그 다섯에 든다.
- **값이 실제로 가려졌는지도 블록이 잰다.** 표본에서 값 조각을 뽑아 **그것이 통째로든
  여덟 자 이상 이어서든 출력에 남으면 `FAIL`**이다. 남았다는 사실은 **표본 이름으로만**
  내고 조각 자체는 인쇄하지 않는다 — 그 인쇄가 곧 유출이다.
...
# 심어 둔 가짜 값(planted probe) — **진짜 비밀값이 아니다.** 일곱 다 이 자리에서 지어낸
# 형태이고 어떤 계정·채널·사업자에도 대응하지 않는다. 검사의 능력을 이 디렉터리의 현재
# 내용이 아니라 이 상수로 잰다 — 내용이 깨끗하면 스캔은 잡을 것이 없어도 침묵한다.
# 값끼리 여덟 자 이상 겹치지 않게 지었다 — 겹치면 한 표본이 샐 때 다른 표본까지 샌 것으로 센다.
PROBES = [("어휘 + 따옴표 값",  'api_key = "q7-fake-01-quoted"'),
          ("어휘 뒤 바로 값",   'Authorization: Bearer w8.fake.02.bare'),
          ("쉼표 든 값",       'db_password=v9-fake,03,comma'),
          ("어휘 + 콜론 값",    'slack_token: u0-fake-04-colon'),
          ("따옴표 든 낱말",    '"api_key": "s1-fake-05-json"'),
...
def val(p):
    ...
    m = VALUE.search(p)                 # 값 자리가 따로 없는 표본은 값 형태 매치 자체가 값이다
...
# 값이 통째로 남는 것만이 아니라 여덟 자 이상이 이어 남는 것도 유출로 센다.
# 남았는지는 표본 이름으로만 낸다 — 남은 조각 자체를 여기 인쇄하면 그 인쇄가 곧 유출이다.
def left(v): return any(v[i:i+8] in l for l in L for i in range(max(1, len(v)-7)))
leak = [k for k, p in PROBES if left(val(p))]
...
reports/evidence/m0/0c/codex-review-20260829T231517Z.json — 매치 1줄
reports/evidence/m0/0c/codex-review-20260830T034122Z.json — 매치 0줄
reports/evidence/m0/0c/codex-review-20260830T053622Z.json — 매치 0줄
reports/evidence/m0/0c/codex-review-20260830T090353Z.json — 매치 0줄
reports/evidence/m0/0c/commands.md — 매치 13줄
reports/evidence/m0/0c/decisions-2026-08-28.md — 매치 0줄
reports/evidence/m0/0c/scope.md — 매치 6줄
docs/discovery/data-dictionary.md — 매치 0줄
    reports/evidence/m0/0c/checklist.md:226: | <가림 6자> 스캔 | `commands.md` **C-8** — 매치와 그 판정이 그 절에 있다 |
    ...
    reports/evidence/m0/0c/codex-review-20260829T231517Z.json:66:     "C-8은 현재 패키지에서 PASS했지만 문서가 밝힌 대로 접미사가 붙은 `<가림 6자>_key`·`client_<가림 6자>_id`·`<가림 5자>s` 형태의 값 가림은 보장하지 않는다.
    reports/evidence/m0/0c/commands.md:1646: ## C-8 · <가림 6자> 스캔
    reports/evidence/m0/0c/commands.md:1680: PATS = [("비밀 어휘", re.compile(r"(?i)(api[_-]?key|<가림 6자>|<가림 5자>|<가림 8자>|<가림 7자>|begin (rsa|ec|openssh))")),
    reports/evidence/m0/0c/commands.md:1681:         ("식별자",   re.compile(r"([0-9]{3}-[0-9]{2}-[0-9]{5}|<가림 7자>|telegram[_-]?id|@[A-Za-z0-9_]{5,}bot)"))]
    reports/evidence/m0/0c/commands.md:1692: PROBES = [("어휘 + 따옴표 값",  '<가림 7자> = <가림 19자>'),
    reports/evidence/m0/0c/commands.md:1693:           ("어휘 뒤 바로 값",   'Authorization: <가림 7자><가림 15자>'),
    reports/evidence/m0/0c/commands.md:1694:           ("쉼표 든 값",       'db_<가림 8자>=<가림 16자>'),
    reports/evidence/m0/0c/commands.md:1695:           ("어휘 + 콜론 값",    'slack_<가림 5자>: <가림 16자>'),
    reports/evidence/m0/0c/commands.md:1696:           ("따옴표 든 낱말",    '"<가림 7자>": <가림 17자>'),
    reports/evidence/m0/0c/commands.md:1697:           ("값 형태 — 번호",    '<가림 12자>'),
    reports/evidence/m0/0c/commands.md:1698:           ("값 형태 — 개인키",  '-----<가림 9자> PRIVATE KEY-----')]
    reports/evidence/m0/0c/commands.md:1825: > **공백으로 끝나는 어휘**(`PATS`에서는 `<가림 7자>` 하나뿐이다)일 때만 재고, 값이 **영숫자로
    reports/evidence/m0/0c/commands.md:1827: > 한 조각이어서 낱말 뒤에 구분자가 바로 오지 않는 줄**(`<가림 6자>_key = …` ·
    reports/evidence/m0/0c/commands.md:1828: > `client_<가림 6자>_id=…` · `<가림 5자>s = […]`)이나 값이 낱말보다 앞에 오는 줄 — 까지 덮는다고
    reports/evidence/m0/0c/scope.md:152: | **A8** | 불변: 인용 형식 위반 0 · 중복 id 0 · <가림 6자> 스캔 통과 · `git diff --check` 0 | agent-workflow §6, evidence-pack
    reports/evidence/m0/0c/scope.md:660: 재취득 고정점, `C-10` 두 환경, 불변·경계·이력, 진짜 <가림 6자> 0. 막은 것은 **그 라운드가
    reports/evidence/m0/0c/scope.md:667: | `6925f53` | **`W-2`(low)** — **「덮는다」고 적은 꼴 안에서도 값이 남았다.** `NEXT`가 요구하는 **영숫자 시작·여덟 자 이상**도, `SEP`가 요구하는 **따옴
    reports/evidence/m0/0c/scope.md:668: | `c941828` | 한계의 **유일한 예시**(*"값이 낱말보다 앞에 오는 줄"*)가 **실무에서 가장 흔한 계열을 대표하지 못했다** — 어휘가 더 긴 식별자의 한 조각이라 낱말 뒤에 구분자
    reports/evidence/m0/0c/scope.md:749: `in_scope` 밖 0 · 진짜 <가림 6자> 0 — **전부 재현으로 섰다.** 막은 것은 **직전 라운드가 새로 쓴
    reports/evidence/m0/0c/scope.md:784: 번씩** · 「분모 둘이 섞였다」 진단 · 불변 · 경계 · `in_scope` 밖 0 · 진짜 <가림 6자> 0 · 이력 표
...
> **가림이 덮는 것은 두 갈래이고 각각 조건이 붙는다.** **구분자 갈래** — 낱말 **바로 뒤**에
> 닫는 따옴표가 한 자 와도 좋고, 그 뒤 공백을 사이에 두고 `:` 또는 `=` 가 오는 줄이다.
> 값이 따옴표로 열렸으면 **그 줄 안에서 짝이 맞아야** 하고, 따옴표 없는 값은 **공백이나
> 따옴표를 만나는 자리에서 끝난다** — 그 뒤는 값으로 보지 않는다. **붙는 값 갈래** — 낱말이
> **공백으로 끝나는 어휘**(`PATS`에서는 `bearer ` 하나뿐이다)일 때만 재고, 값이 **영숫자로
> 시작하고 여덟 자 이상**이어야 한다. **이 조건을 벗어난 자리** — **어휘가 더 긴 식별자의
> 한 조각이어서 낱말 뒤에 구분자가 바로 오지 않는 줄**(`secret_key = …` ·
> `client_secret_id=…` · `tokens = […]`)이나 값이 낱말보다 앞에 오는 줄 — 까지 덮는다고
> 주장하지 않는다. **조건 안의 꼴에 대해서는 산문이 아니라 블록이 잰다** —
> 표본 일곱의 값 조각이 통째로든 여덟 자 이상 이어서든 출력에 남으면 `FAIL`이다.
> **심어 둔 표본 일곱은 진짜 비밀값이 아니다** — 이 자리에서 지어낸 형태이고 어떤 계정·
> 채널·사업자에도 대응하지 않는다.
...
**대상 파일** — **이 파일 자신**(읽는 체크아웃의 내용). **실행 트리** — `e67d177`
(C-1 ~ C-9가 선언한 SHA).
...
**이 검사의 출력은 각 블록 명령의 첫 줄 58자를 그대로 싣는다**(아래 명령의 `head`).
그래서 **선언 SHA가 그 58자 안에 드는 블록이 하나라도 있으면, 선언 SHA가 옮겨 갈 때마다
이 절 두 블록의 기록된 출력도 반드시 함께 바뀐다.** 재취득이 바꾸지 않을 수 없는 자리라는
뜻이다 — **「재취득이 무엇을 바꿨나」를 세는 산문이 이 성질을 빠뜨리면 그 셈은 언제나
좁다.** 실제로 한 번 그렇게 났다(`scope.md`의 `9dd5dab` 행). **어느 블록의 머리가 선언
SHA를 나르는지는 산문이 세지 않는다 — 아래 두 출력이 그대로 낸다.**
...
# 대상 파일 = 이 파일 자신(읽는 체크아웃의 내용) · 실행 트리 = e67d177 (블록들이 선언한 SHA)
...
git worktree add --detach "$WT" e67d177 >/dev/null 2>&1
...
환경 전제 — bid-vector: 있음 · _workspace: 있음
  축어 일치            # 경계를 **행 번호가 아니라 날짜 머리로** 잡는다 — 어느 쪽 파일이 자라도 낡지 않는다.
  축어 일치            for c in $(git log --format='%H' 2b05684..e67d177 \
...
git worktree add --detach "$CT" e67d177 >/dev/null 2>&1
...
환경 전제 — bid-vector: 없음 · _workspace: 없음
  환경 부족(_workspace)  # 경계를 **행 번호가 아니라 날짜 머리로** 잡는다 — 어느 쪽 파일이 자라도 낡지 않는다.
  축어 일치            for c in $(git log --format='%H' 2b05684..e67d177 \
  축어 일치            # 지적 줄을 그대로 실으면 이 파일이 다시 후행 공백을 갖는다 — 표지로 바꿔 싣는다.
  축어 일치            python3 - <<'PY'
  축어 일치            python3 - docs/discovery/data-dictionary.md <<'PY'
...
## C-11 · Codex 6차 리뷰의 실행 메타와 preflight

`codex-review-gate`가 리뷰 레인에 지운 기록 의무다 — **reviewer 메타**(`cli_version` ·
`model`+effort)와 **memory 오염 preflight**. **6차 리뷰 레인은 이 파일이 자기 리뷰 대상이라
쓰지 못했다** — 리뷰 라운드에 리뷰 대상을 고치면 `base`/`head`가 흔들린다. 그래서 **0C 종료
커밋이 이 절을 신설해 대신 쓴다.** 0D가 `f207412`·`a2f31f0`에서 같은 의무를 같은 형태로 대신
쓴 선례를 따른다.

### 이 절에 명령/출력 쌍을 두지 않는다 — 판단과 근거

**0D의 같은 절(`C-12`)은 명령 블록을 두고, 그 패키지의 래칫(`C-11`)이 명령 문면에서
`$HOME`·`~/`·`_workspace/` 토큰을 찾아 `EXT`로 빼 준다.** **0C에는 그 분류가 없다.** 이
패키지의 래칫은 **`C-10`**이고 그것이 아는 제외는 둘뿐이다 — **`SELF-EXCLUDE-C10` 표지**와
**`bid-vector`·`_workspace` 경로 부재(`환경 부족`)**. 그래서 여기에 명령 블록을 두면 세 가지가
한꺼번에 어긋난다.

- **시점 값은 축어로 다시 뜨지 않는다.** `~/.codex`만 읽는 블록은 `C-10`의 경로 토큰에 아예
  걸리지 않고, `_workspace/`를 읽는 블록도 **그 경로가 있는 환경(`C-10.1`)에서는 실제로
  재실행된다.** 두 갈래 다 `불일치`로 떨어져 **`C-10`이 `FAIL`이 된다.**
- **쌍이 늘면 `C-10.1`·`C-10.2`의 기록된 출력이 그 자리에서 낡는다** — 재취득을 강제한다.
  `C-10`은 **이 파일 자신**을 대상으로 읽으므로 쌍의 수와 각 블록의 머리가 그 출력에 실린다.
- **`SELF-EXCLUDE-C10`을 붙여 피하는 길은 표지의 뜻을 왜곡한다.** 그 표지는 「자기 자신」을
  뜻하고 이 절은 `C-10`이 아니다.

**남은 길은 `C-10`에 `EXT` 축을 새로 다는 것뿐인데, 그것은 `approve`를 받은 뒤에 검사 자체를
고치는 일이다.** 이 slice는 종료 기록만 남기므로 **축을 열지 않는다.** 그래서 이 절은
**명령/출력 쌍 없이 시점 기록만 싣는다** — 쌍이 없으니 `C-10`이 셀 것이 늘지 않는다.
**0D의 `EXT`와 효과는 같고 기제는 다르다**: 0D는 분류기가 빼고, 여기서는 **잴 대상 자체가
없다.** 그 판단이 옳은지는 주장이 아니라 **`C-10` 두 블록을 다시 돌려** 확인했다 —
결과는 이 절을 실은 커밋의 메시지가 싣는다.

**아래 값들은 이 파일의 어느 블록도 내지 않는다.** 「명령이 내는 셈을 산문에 옮겨 적지
않는다」(`:5`)에 걸리지 않는 이유가 그것이다 — **옮겨 적은 것이 아니라 이 자리가 그 기록의
원본**이다. 값의 출처는 각 표가 함께 적는다.

### C-11.1 · reviewer 메타

| 무엇 | 값 | 출처 |
| --- | --- | --- |
| `cli_version` | `codex-cli 0.149.0` | 등재된 verdict JSON의 `reviewer` |
| `model` | `gpt-5.6-sol (reasoning effort: high)` | 같은 자리 |
| 리뷰 실행이 남긴 머리글 | `OpenAI Codex v0.149.0` · `model: gpt-5.6-sol` · `reasoning effort: high` | `_workspace/m0-0c/codex.raw-output-r6.txt`의 머리 |

**판정**: 등재된 JSON의 `reviewer`와 **리뷰 실행이 실제로 남긴 머리글이 같은 값을 말한다.**
`model_reasoning_effort`가 **`high`로 고정된 채 돌았다** — 하네스가 재현성 때문에 못박은
값이다(`CLAUDE.md` 변경 이력 2026-08-26).

**두 출처의 확인 가능 범위가 다르다.** 머리글 파일은 `_workspace/`(gitignore 대상)이라
**리뷰어 worktree에서도 이 저장소의 clean checkout에서도 열리지 않는다.** 등재된 JSON은
저장소 안이므로 **그 축만 어디서나 확인된다.** 그 JSON이 이 패키지에 등재됐다는 사실은
`C-2.1`이 내는 경로 목록에 든다 — **다만 그 블록의 range 끝은 선언 SHA이고 등재는 그 뒤의
커밋이므로 지금 기록된 출력에는 없다.** range 끝을 HEAD로 바꿔 다시 돌리면 나오며, 바꿀
자리는 `C-2.1`의 각주가 적는다.

### C-11.2 · memory 오염 preflight

| 무엇 | 값 |
| --- | --- |
| preflight ① — consolidate 이후 이 저장소 흔적 | `traces=15` |
| preflight ② — stage1에 올라온 리뷰 세션 | `stage1_review_rows=0` |
| 같은 핸들로 읽은 `stage1_outputs` 총 행 | `591` |

**출처**: 6차 리뷰 레인의 preflight 실행. **레인이 이 파일에 쓰지 못했고 그 명령 출력이 파일로
남지도 않았으므로 이 표가 그 기록 자리다** — 0D `C-12.2`의 마지막 행이 3차 리뷰의 같은 의무를
대신 진 것과 같은 형태다.

**판정**: `stage1_review_rows=0` — 리뷰 세션 transcript가 stage1에 올라와 있지 않다.
**총 행이 0이 아니므로 「테이블이 비어서 0」도 「못 열어서 0」도 아니다.** 두 수를 **같은
핸들로** 받았다는 것이 그 판정을 받치는 자리다 — 대체 핸들로 바꾼 뒤에도 `0`이 나왔다면
「열리지 않아서 0」과 구별되지 않는데, 같은 핸들이 `591`을 함께 냈으므로 그 갈래가 닫힌다.
`traces`는 **consolidate 이후 이 저장소 흔적의 양**이고 0이 아니다 — 그래서 호출이 memory를
끄는 플래그를 붙인다. 그 플래그의 문면은 `.claude/skills/codex-review-gate/SKILL.md`가 갖고
**이 값은 그 플래그의 효과를 재지 않는다**(흔적의 양일 뿐이다).

**`?mode=ro`가 실패해 `?immutable=1`로 대체했다.** 스킬 문면은 `?mode=ro`를 적고 레인은 그것을
그대로 돌렸으며, 실패한 뒤 같은 파일을 `?immutable=1`로 열어 preflight를 마쳤다. 그 대가는
**파일이 동시에 쓰이면 찢긴 페이지를 읽을 수 있다**는 것이다. **이 갈라짐을 스킬에 반영할지는
하네스 소유자의 판정이며 이 slice는 `.claude/`를 고치지 않는다.**

**이 명제의 정본은 0D `commands.md` `C-12.2`의 관측 표다.** **그 slice는 닫혔고 이 커밋은 그
파일을 고치지 않는다.** 그래서 이번 관측은 여기에 남기고, **두 자리를 함께 읽어야 하는
기록이라는 사실 자체를 적어 둔다.** 이 관측이 그 표에 더하는 것은 **실패 한 건**이다.

**리뷰어가 기제를 진술했다 — 후보는 후보로 남는다.** 6차 리뷰 레인은 실패의 기제를 이렇게
적었다: *`mode=ro`도 WAL `-shm` sidecar를 만들어야 하는데 샌드박스가 그것을 막는다.
`immutable=1`은 잠금을 아예 건너뛴다.* **이것은 진술이지 실험이 아니다.** 0D가
`_workspace/m0-0d/20_verifier_report_h1fix.md` `O-4`에서 좁힌 후보(**WAL sidecar의 유무**)와
**같은 방향**이고, 여기에 **막는 주체가 샌드박스**라는 갈래가 하나 더 붙어 후보가 좁혀졌다.
**그래도 승격시키지 않는다** — sidecar를 만들고 없애며 `mode_ro`를 가르는 실험은 어느 레인도
돌리지 않았고, **진술은 관측이 아니며 기제를 말하는 진술은 그 기제의 증거가 아니다.**
0D의 판정(*"원인을 확정하지 못했다"*)은 **그대로 선다.** 이 절이 더하는 것은 **관측 하나와
좁혀진 후보**뿐이다.
