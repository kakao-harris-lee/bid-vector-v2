...
head_sha: 9d8d206   # 이 파일을 쓴 가장 최근 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | **`F-9` 본체 — C-10을 두 환경에서 실행으로 잰다.** 「어느 트리에서 돌려도 같은 답이 나온다」는 거짓이었다. `bid-vector`·`_workspace`가 **git 미추적이라 SHA에서 복원되지 않고 실행 CWD에서 온다.** 그 사실을 주장으로 적지 않고 **C-10.1(두 경로 있음) · C-10.2(없는 clean worktree)** 두 블록으로 **재서** 싣는다. 이 이력 절과 `head_sha`도 이 커밋이다. **이 커밋에 대해서는 아무것도 주장하지 않는다** |
