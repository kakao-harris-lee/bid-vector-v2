...
head_sha: 19c7e57   # 이 파일을 쓴 가장 최근 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | **출력 블록을 `19c7e57` 트리에서 다시 뜨고 `C-10`을 자기 참조 없이 다시 짰다** — 대상 파일을 **이 파일 자신**으로 두고 **자기 절은 표지로 걸러 뺀다.** 두 환경 측정은 그대로다. 이 이력 절과 `head_sha`도 이 커밋이다. **이 커밋에 대해서는 아무것도 주장하지 않는다** |
