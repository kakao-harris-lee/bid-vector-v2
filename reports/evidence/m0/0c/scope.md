...
| `c81c8a1` | **`F-1`·`F-3` 본체(1차)** — 열여섯 블록 **전부**를 `1e9e670` 트리(worktree + `bid-vector`·`_workspace` symlink)에서 `bash -c`로 다시 돌려 **stdout만** 받아 실었다 |
| `6eba815` | **C-10 신설** — 축어 재현을 재는 검사 |
| `5e2c67e` | **A8이 다시 잡았다** — C-5의 자른 끝이 공백이면 기록된 출력에 후행 공백이 남는다. `C-6.2` 때와 같이 **명령을 고쳐** 옳은 문자열을 내게 했다 |
| `989e2c1` | **C-2.2도 같은 형태였다** — `git diff --check`의 지적 줄을 그대로 실으면 이 파일이 다시 후행 공백을 갖는다. **표지로 바꿔 싣도록 명령을 고치고** 열여섯 블록을 `5e2c67e` 트리에서 **다시 떴다** |
| 이 커밋 | C-10을 새 두 SHA로 다시 재고, 이 이력 절과 `head_sha` 갱신. **이 커밋에 대해서는 아무것도 주장하지 않는다** |

**같은 형태가 두 번 더 나왔다** — 기록된 출력이 후행 공백을 나르면 그 파일이 다시
`git diff --check`에 걸린다. **세 번 다 산문으로 예외를 두지 않고 명령을 고쳤다**
(`C-6.2` · `C-5` · `C-2.2`).
