...
head_sha: 989e2c1   # 이 파일을 쓴 가장 최근 커밋의 직전 커밋. 「head_sha와 range」 절 참조
in_scope:
...
| 이 커밋 | C-10을 새 두 SHA로 다시 재고, 이 이력 절과 `head_sha` 갱신. **이 커밋에 대해서는 아무것도 주장하지 않는다** |

**같은 형태가 되풀이됐다** — 기록된 출력이 후행 공백을 나르면 그 파일이 다시
`git diff --check`에 걸린다. 걸린 자리(`C-6.2` · `C-5` · `C-2.2`)마다 **산문으로 예외를
두지 않고 명령을 고쳤다.**

**어디까지 쟀는지만 적는다** — C-10은 **대상 파일 `989e2c1` · 실행 트리 `5e2c67e`** 두
