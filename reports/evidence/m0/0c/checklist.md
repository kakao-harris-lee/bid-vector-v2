...
| §12.2가 **필드**를 전부 덮는다 | `commands.md` **C-4.2** — 타입 선언과 필드 표에서 이름을 뽑아 덮개를 대조한다. **리터럴만 보던 C-4의 사각지대**가 Codex finding **A**로 드러나 신설했다 |
...
| 검사의 한계를 밝힌다 | **C-4.1** 아래 인용 블록(마스크가 무엇을 빼는지 `MASKS` 각 줄이 적는다) · **C-4.2** 아래 인용 블록(`snake_case`와 여러 이름을 한 칸에 적은 행을 못 본다 — §12.2가 그 자리를 손으로 등재한다) |
