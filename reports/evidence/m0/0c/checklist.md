...
| 값이 미정인 것은 `OPEN`으로 귀속된다 | **§12.1·§12.2 두 표의 `소유 OPEN` 칸**이 그 귀속을 적는다 — **여기 옮겨 적지 않는다.** 앞서 이 자리가 열거를 들었고 **`OPEN`이 늘 때 낡았다** |
...
| U-2 | 시공능력평가금액 부가세 포함 (**과세 처리만** — 단위를 정하지 않는다) | §1.2 · §1.2.4 |
| U-2b | 직전 연도 공시값 (**기준 시점만**) | §1.2.4 |
...
**분류를 읽는 법**: 「활성」으로 찍힌 것은 **전부 실제로 활성이다.**
`OPEN-REG-05`·`OPEN-QUAL-10`·`OPEN-QUAL-11` 셋 다 그렇다.

> **⚠ 정정 (2026-08-30, Codex 리뷰 라운드 4 high #2)** — 앞서 이 자리가 `OPEN-REG-05`를
> **「닫혔는데 상류 registry 행만 안 고쳐 활성으로 찍히는 것」**으로 해석했고
> **그 해석이 틀렸다. 상류가 옳았다.** 상태 정본 `regression-ledger.md` §9의 그 행은
> **활성**이고, 같은 행이 *"차이의 크기·과세 처리는 결정도 측정도 없다"*고 적는다 —
> U-1·U-1b는 **과세 처리 축**에만 답했고 **잔차 성분**과 **저장 값의 행별 과세**가
> 남는다(사전 §1.2.2 · §11). `docs/adr/0002:185`가 그 `OPEN`을 **활성**으로 드는 것과
> 이제 한목소리다. **`OPEN-QUAL-10`·`OPEN-QUAL-11`도 앞 라운드에 같은 자리에서 같은
> 오분류를 겪었다** — 세 번째다. **evidence가 상류와 어긋나면 상류가 이긴다.**

`OPEN-QUAL-06`은 「활성」으로 찍히지 않는다 — **분해 여부를 운영자 결정 2026-08-26이
답했고** `capability-map.md` §12.1이 해소로, §12.2가 상태 `확정`으로 등재한다.
**U-5와 사전 §3.2.3은 그 확정의 하류 실행이지 그 `OPEN`을 새로 닫는 것이 아니다** —
같은 §12.2 행이 *"scout 노트의 4분류는 후보이지 확정 목록이 아니다"*라 적어 값 목록을
그 `OPEN`의 답에서 제외한다(사전 §11의 ⚠ 정정).
**이 파일에서 수식어 없는 §12.2는 사전의 것이다.**

**id 둘이 이 라운드에 목록으로 들어왔다** — `C-6.3`이 둘 다 **「결정 완료」**로 찍고
**활성 셈(15)은 그대로다.**

- **`OPEN-QUAL-01`** — 사전이 앞서 그 id를 **한 번도 적지 않았고**, 그 자리에 *"U-8의
  결합 축을 소유한 `OPEN`이 따로 없었다"*는 **부재 주장**을 두었다. **그 주장이
  거짓이었다** — 그 축의 소유자가 `OPEN-QUAL-01`이고 **운영자 결정 2026-08-26으로 이미
  해소**됐다(사전 §3.2.2 · §11의 재정정). **U-8이 닫는 활성 `OPEN`이 없다는 결론은 그대로
  서고 사유가 바뀐다.**
- **`OPEN-STR-01`** — 사전 §1.4.2의 ⚠ 정정이 **곱셈 base 축의 소유자가 아님을 밝히려고**
  든다(그 결정의 답은 **전략 예산 필터 축**이다). **사전이 그 결정을 등재하지는
  않는다** — `C-6.3`의 라벨은 그 블록 각주가 적듯 **상류 registry의 상태**만 읽는다.
...
| `OPEN-REG-05` | §1.2.2 · §11 | **U-1·U-1b가 답한 과세 처리 축만 등재**하고 새로 결정하지 않았다. **남는 축 둘**(두 금액 차이의 **잔차 성분** · 저장 값의 **행별 실제 과세 처리**)에 정의를 붙이지 않았다. `R-BASIS-01`의 fixture 유보는 **조건이 그대로 서고 사유가 하나 늘었다**(사전 §11). 상류 registry 행은 고치지 않았다 |
| `OPEN-QUAL-10` | §1.2 · §1.2.4 · §5.5 · §9 · §11 · §12.2 · §13.4 | **각 결정이 실제로 답한 축만 등재했고 그 `OPEN`은 활성이다** — 남는 것은 **공고 게시 요건 축 전부**와 보유액 축 안의 **`unit`**이다. **그 축 안에서도 각 결정이 실제로 정한 것만 매단다** — U-2는 과세 처리, U-2b는 기준 시점이고 **단위를 정한 결정은 없다.** 보유액의 단위는 **V2가 직접 받는 입력의 계약**이 정하며(`Money`, `v2-지침서.md` §4.1) 그 근거를 사전 §11이 적는다. **공고 게시 요건 축의 단위·과세는 정하지 않았다** — 조달청 문서에 근거가 없어 관측으로만 알 수 있고, §12.2가 `requiredAmount`의 단위에 이 `OPEN`을 소유자로 달았다. **서명도 그 미결을 나른다** — 앞서 `requiredAmount: Money`로 적어 산문과 어긋났고, `UnnormalizedFigure`(원문 수치 + §5.3 필드 계약)로 바꿨다. 단위가 정해지기 전에는 `Money`로 정규화할 수 없고 **보유액과의 비교가 타입으로 막힌다**(사전 §5.5) |
...
**아래 표에 등록한 것은 전부 활성이다.** **결정 사본의 「0C 사용자 결정 — 전건 완료」는
「운영자에게 물을 것을 다 물었다」는 뜻이지 「잔여 미결이 없다」는 뜻이 아니다.** 0C가
사용자 입력을 더 기다리지 않고 착수한 판단은 옳았고, **같은 묶음이 활성 `OPEN`을 남긴
것도 함께 참이다.** 어느 결정의 잔여인지는 **사전 §9의 「어느 결정의 잔여인가」 칸**이
정본이고, **셈은 이 산문이 내지 않는다** — `commands.md` **C-6.2**·**C-6.3**이 낸다.
...
| `OPEN-DIC-09` | fold의 총순서 키와 tie-break | Codex 5차 high #2가 **「마지막」이 순서 술어를 전제하는데 그 술어가 없다**는 것을 적출했다. 순서 술어가 관측이 나르는 것이라는 **형태**는 `v2-지침서.md` §4.5에서 서지만 **키의 구성은 정책 결정**이고 legacy에 답이 없다 |
| `OPEN-DIC-10` | `RoundingPolicy`의 `mode` 값·금액 축 밖의 `scaleDigits`·첫 `effectiveFrom`·그 「금액 축 밖」의 경계 | Codex 5차 medium이 **이 미결을 소유하는 `OPEN`이 없다**는 것을 적출했다. **형태**(자리수·방식·version)와 **정해진 둘**(금액 축의 자리수 · 하한 이상 보장)은 §1.1이 정의했고, **값은 승인 명세도 legacy도 정하지 않는다.** **경계는 0C 자체 발견이다** — legacy에 「집계」이면서 「금액」인 자리가 실재해(사전 §9의 그 행이 좌표를 든다) 「금액 축 밖」이 무엇을 가리키는지가 정해지지 않았고, **그 경계가 이 미결의 범위를 이루므로 이 `OPEN`이 함께 갖는다** |
