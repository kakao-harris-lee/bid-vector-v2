...
| `git diff --check` 0 | **초록 출력은 `commands.md` C-2.3**이다. **C-2.2**는 그와 별개로 **지적이 있던 시점**을 남긴다 — 그 블록이 선언한 SHA에서는 지적이 있고, 지적된 자리는 **그 시점의 C-5 출력**이다. 두 블록 다 range의 끝 SHA를 HEAD로 바꿔 리뷰 시점에 다시 잴 수 있다. **C-10은 이 축을 재지 않는다** — 그것이 재는 것은 stdout 문자열 동등뿐이다 |
