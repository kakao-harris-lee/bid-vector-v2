...
| `OPEN-QUAL-10` | §1.2 · §1.2.4 · §5.5 · §9 · §11 · §12.2 · §13.4 | **운영자 보유액 축만** 결정 등재로 닫았다. **공고 게시 요건 축의 단위·과세는 정하지 않았다** — 조달청 문서에 근거가 없어 관측으로만 알 수 있고, §12.2가 `requiredAmount`의 단위에 이 `OPEN`을 소유자로 달았다. **서명도 그 미결을 나른다** — 앞서 `requiredAmount: Money`로 적어 산문과 어긋났고, `UnnormalizedFigure`(원문 수치 + §5.3 필드 계약)로 바꿨다. 단위가 정해지기 전에는 `Money`로 정규화할 수 없고 **보유액과의 비교가 타입으로 막힌다**(사전 §5.5) |
