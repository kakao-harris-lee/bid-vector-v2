...
**`OPEN-QUAL-11`은 다르다 — 그것은 실제로 활성이다.** 앞서 이 자리가 그것을 「닫혔는데
상류 행만 안 고쳐 활성으로 찍히는 것」 쪽에 세웠고 **그 분류가 틀렸다**(사전 §11의
⚠ 정정 — U-8은 그 `OPEN`의 질문에 답하지 않는다). `OPEN-QUAL-06`은
...
| `OPEN-QUAL-10` · `OPEN-REG-05` | §11 | **결정으로 닫힌 것을 등재**했을 뿐 새로 결정하지 않았다. 상류 registry 행은 고치지 않았다 |
