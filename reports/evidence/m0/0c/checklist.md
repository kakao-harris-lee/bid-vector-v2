...
| `OPEN-DIC-09` | fold의 총순서 키·tie-break·늦은 관측의 적용 | Codex 5차 high #2가 **「마지막」이 순서 술어를 전제하는데 그 술어가 없다**는 것을 적출했다. 순서 술어가 관측이 나르는 것이라는 **형태**는 `v2-지침서.md` §4.5에서 서지만 **키의 구성은 정책 결정**이고 legacy에 답이 없다 |
| `OPEN-DIC-10` | `RoundingPolicy`의 `mode` 값·금액 축 밖의 `scaleDigits`·첫 `effectiveFrom` | Codex 5차 medium이 **이 미결을 소유하는 `OPEN`이 없다**는 것을 적출했다. **형태**(자리수·방식·version)와 **정해진 둘**(금액 축의 자리수 · 하한 이상 보장)은 §1.1이 정의했고, **값은 승인 명세도 legacy도 정하지 않는다** |
