...
**일곱을 등록했고 일곱 다 활성이다.** **그중 셋은 이 slice가 등재한 운영자 결정의 미완이
직접 만든 것**이다 — `OPEN-DIC-01`(U-6) · `OPEN-DIC-04`(U-1·U-1b) · `OPEN-DIC-07`(U-3).
**결정 사본의 「0C 사용자 결정 — 전건 완료」는 「운영자에게 물을 것을 다 물었다」는 뜻이지
「잔여 미결이 없다」는 뜻이 아니다.** 0C가 사용자 입력을 더 기다리지 않고 착수한 판단은
옳았고, **같은 묶음이 활성 `OPEN`을 일곱 개 만든 것도 함께 참이다.**
어느 결정의 잔여인지는 **사전 §9의 「어느 결정의 잔여인가」 칸**이 정본이다.
