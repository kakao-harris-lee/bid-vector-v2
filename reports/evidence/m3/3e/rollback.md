...
## 대상 경로 (기계 산출, verifier r2 정리 라운드 반영)
...
config/quality/gate-tests.properties, koneps는 무변경이라 목록에 없음). **V5·V6가 V4로
흡수되며(운영자 결정) 18→16경로로 줄었다** — 목록을 다시 기계 산출했다(add-then-delete가
같은 range 안에서 순 0으로 접힌다).
...
A=6(삭제 대상), M=10(base로 restore 대상), 총 16경로. **라운드마다 파일이 늘거나 줄면 이
명령을 다시 돌려 목록을 갱신한다** — 손으로 쓰지 않는다(이번이 그 축소 실례다).
...
경로(`docs/discovery/capability-map.md`·`milestone-3.md` 등)**는 되돌리지 않는다 — 이
slice의 in_scope가 아니거나(전자) 문서 레인 소유(후자, verifier r1 L-5 뒤 명시)라 되돌림
대상에 없다.

## 임시 clone 실측(2026-09-09, verifier r2 정리 라운드 재실측)

- `git clone --quiet . <scratchpad>/3e-r2-rollback-<random>` → 위 `git restore` 실행 →
  **exit 0**.
...
## `V4` 삭제가 되돌림이 아닌 경우 — 적용 이력이 있는 DB

위 되돌림은 **코드·마이그레이션 파일** 롤백이다. **이미 `V4`(흡수본, provenance·유니코드
공백 CHECK 포함)가 migrate된 PostgreSQL 인스턴스(예: 로컬 개발 DB, `flyway_schema_history`에
그 행이 있는 상태)에서는 파일 삭제만으로 스키마가 되돌아가지 않는다** — Flyway는 적용
이력을 보고 검증하므로, 파일을 지운 채로 `flyway migrate`를 돌리면 `flyway_schema_history`와
파일 집합이 어긋나 `validate()`가 실패한다(`CleanMigrationTest`의 「flyway validate가
통과한다」 test가 그 어긋남을 실제로 잡는다).
...
운영 DB(M6 6C 소관)에 이미 V4가 적용된 뒤의 롤백 절차(down-migration 등)는 이 slice의
범위 밖이다. **`V4`는 이제 흡수본이라 향후 슬라이스가 다시 파일을 더 나눌 경우 이 절이
다시 갱신된다.**
