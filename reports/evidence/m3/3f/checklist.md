...
## 사용자 승인 — 2026-09-09, slice 3F 종결

verifier r1 `not-ready`(**F-1** 컬럼별 `COALESCE` 가 짝 컬럼에 옛 값을 남겨 `Determined`↔`RankMissing`·
`RankDuplicated` 전이가 **정상 저장에서 `PSQLException`** · **F-2** 3E H-2 재발 — 축별 관측 시각이 없어
낡은 값이 최신으로 읽힘) → 수정 라운드 1 → r2 `not-ready`(**N-1** F-3 수정이 같은 계열의 새 결함 —
`INSERT … ON CONFLICT` 의 CHECK 가 **병합 후 행이 아니라 제안 tuple** 에 걸려, 부모가 총예가건수를
가졌고 관측이 옳게 생략한 **유효한 쓰기**를 거부) → 수정 라운드 2 → r3 `ready-for-review` + M-1·low
일괄 `3901acd` 위에서 **사용자 승인 2026-09-09**. **재작업 2/5.**

**N-1 을 제약이 아니라 타입으로 닫았다.** 범위를 벗어났다고 **판정하려면 그 범위를 알아야 하므로**
「범위 초과 + 범위 모름」은 논리적으로 불가능한 상태인데, 타입이 그것을 허용하고 DB 제약으로 막으려 한
것이 뿌리였다. `OutOfRange` 가 **자기가 대조한 상한을 값으로 나르게** 하니 불가능한 상태가 표현 불가가
되고, 저장 tuple 이 자기 완결이 되어 페어 CHECK 가 자연스러워지고, 읽기가 부모 축을 참조하지 않게 되어
오검출 원인 자체가 사라졌다. verifier 가 타입 우회 경로를 찾지 못했고 음성 대조를 **두 방향**(상한 없는
`OUT_OF_RANGE` · `VERIFIED` 에 붙은 떠도는 상한) 다 쳐서 둘 다 물었다. M-1 은 그 타입의 **하한**까지
`require` 로 강제해 닫았다(하한 1 은 추첨번호가 1-기반 인덱스라는 도메인 진실이다).

**운영자 결정 여덟**: D-3F-1~6 · **D-3F-3 은 도메인 결정으로 해소**(실현 사정률에 추첨 귀속이 필요 없고
번호 집합으로 충분 — 그 답이 투찰자별 canonical 표와 **정체성 문제를 함께 지웠다**. masking 이 자연 키를
지우고 남은 후보인 `opengRank` 는 실측에서 전 행 채워지고 유일한 건이 **4/15** 였다) · **D-3F-7 = P-13**
(투찰 축 필드 계약 열 키 채택, **평가점수 넷 제외** — 문서가 범위·소수 자리를 적지 않아 scale 미확정).

**알려진 제한 등재 유지**: 평가점수 넷 미보존(`OPEN-3F-EVALUATION-SCORE-SCALE`) · 추첨 귀속 미보존
(운영자 결정, raw 감사에 남아 소급 승격 가능) · 순위 결측·중복 시 개찰 1위 축 부재
(`OPEN-3F-OPENG-RANK-SEMANTICS`) · 총예가건수 없으면 범위 검사 불가 · **`bidNtceOrd` 항상 전송은 실측
없는 안전 우선 선택**(운영 배선 전 읽기 전용 호출 한 번으로 닫는다) · 운영 배선은 M4 4B ·
치환의 실제 방어는 컴파일이 아니라 런타임 masking 이다(r1 F-4 로 과대 주장을 철회했다).

**`COL-03` 이 닫혔다** — 3E 의 복수예비가격 15행과 3F 의 추첨번호가 함께 서서 「복수예비가격 15개와
추첨번호를 확보한다」의 acceptance 가 성립한다. 같은 승인에서 `origin/main` push.
