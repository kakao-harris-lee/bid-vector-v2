...
| `bidNtceNm` | 입찰공고명 | text(표시·감사용·감시 키워드 매칭 입력, 키 아님) — *"공사명 또는 사업명이라고도 하며 입찰공고 내용을 요약한 이름"* | 항목크기 1000 | 문서 **필수**(`1`) — **계약 행은 옵션**으로 둔다(수집이 공고명 부재로 항목을 탈락시키지 않는다, 부재는 `null` — D-6F4-8) | `authoritative` · **P-15**(M6/6F-8, 2026-09-24) |
...
>
> **P-15(2026-09-24) 도 이 셈을 재대조하지 않았다** — §1.3 에 더한 공고명 키 하나(`bidNtceNm`)는 입찰공고정보서비스
> 참고자료의 응답 항목이고(같은 표의 다른 오퍼레이션 응답에 항목크기 1000·구분 `1` 로 선언), 위 60 은 legacy
> `KNOWN_FIELDS` 기준이라 그 포함 여부를 이 갱신에서 세지 않았다 — 값의 정본은 §1.3 행이다.
...
### 1.11 개찰완료 축(투찰 행) 필드 계약 — **P-13 승인 2026-09-09 (평가점수 넷 제외)**

§1.7 은 **legacy 가 소비하는** 개찰 축 17 키를 덮는다. 개찰완료 오퍼레이션
(`getOpengResultListInfoOpengCompt`)의 투찰자별 필드는 **legacy 가 그 오퍼레이션을 부르지 않아**
§1.7 에 없고, 그래서 M3/3F 가 수집하려는 순간 **등재된 계약이 없다**는 것이 드러났다(구현 레인 조사 —
`KonepsFieldContract` 생성자가 procurement `internal` 이라 어댑터가 계약 없이 값을 꺼낼 경로가
**구조적으로 없다**). 출처는 낙찰정보서비스 참고자료(`koneps-scsbid-reference`)이고 §1.9.4 가 같은
오퍼레이션을 이미 인용한다.

| rawName | concept(문서 국문) | scale | 항목크기 | nullability(문서 항목구분) | 층 |
| --- | --- | --- | --- | --- | --- |
| `opengRsltDivNm` | 개찰결과구분명 | `OPAQUE_TEXT` | 30 | **필수** | `authoritative` |
| `bidClsfcNo` | 입찰분류번호(동일 공고번호의 집행일련번호) | `IDENTIFIER` | 5 | **필수** | `authoritative` |
| `rbidNo` | 재입찰번호 | `IDENTIFIER` | 3 | **필수** | `authoritative` |
| `opengRank` | 개찰순위 | 셈(순위) | 7 | 옵션 | `authoritative` · **실측 보강 아래** |
| `prcbdrNm` | 투찰업체명 | `OPAQUE_TEXT` | 200 | 옵션 — *"개찰결과구분명이 「개찰완료」일 경우 **필수**"* | `authoritative` |
| `bidprcAmt` | 투찰금액 | `WON_INTEGER` | 21 | 옵션 | 단위 `authoritative`(*"(원화,원)"*) · 과세 **미선언** |
| `bidprcrt` | 투찰률 | **percent**(*"투찰금액/예정가격 *100 임(%)"*) | 18 | 옵션 | scale·산식 `authoritative` · 밴드 미확정 |
| `drwtNo1` · `drwtNo2` | 추첨번호 1·2 | 셈(1-기반 인덱스) | 3 | 옵션 | 형태 `authoritative` · **범위 근거 아래** |
| `bidprcDt` | 투찰일시 | `DATETIME_NO_ZONE`(*"YYYY-MM-DD HH:MM:SS"*) | 19 | 옵션 | 형식 `authoritative` · zone **미확정**(§1.7.4 와 같은 결) |
| `bidPrceEvlVal` · `techEvlVal` · `techEvlNaturVal` · `totalEvlAmtVal` | 입찰가격점수 · 기술평가점수 · 기술평가원점수 · 종합평가점수 | **미확정**(점수 축이 3A 어휘에 없다) | 25 | 옵션 — *"낙찰방법이 협상에 의한 계약"* 일 때 | 형태 `authoritative` · scale·범위 **미확정** |
| `prcbdrBizno` | 투찰업체사업자등록번호 | — | 10 | **필수** | **계약을 등재하지 않는다** — P-10 (a) 로 어댑터 경계에서 치환한다. 등재하면 값이 도메인에 들어올 통로가 생긴다 |
| `prcbdrCeoNm` | 투찰업체대표자명 | — | 35 | 옵션(개찰완료 시 필수) | 같음 — **개인 성명**이다 |
| `rmrk` · `cnsttyAccotBidAmtUrl` | 비고 · 공종별입찰금액URL | — | 200 · 150 | 옵션 | **등재하지 않는다** — 소비자가 없고, URL 은 문서가 *"차세대 나라장터 개편 이후 **제공 불가**"* 라 적는다 |

**읽어야 할 것 다섯.**

1. **`prcbdrBizno` 가 「필수」인데 계약을 등재하지 않는다.** §1.7.3 이 `bidwinnrBizno` 에서 한 판단과
   같다 — 응답에 오는 것은 막을 수 없고 **경계 안으로 들이지 않는 것**이 P-10 (a) 다. 계약 미등재가 그
   차단의 실물이고, 미등재 키는 §5.3 규율 1 대로 회계에 실린다.
2. **`opengRank` 는 문서가 옵션이라 적고 실측이 그것을 확증했다** — 표본 15건(참가업체수 상위 편향)에서
   **전 행 채워지고 유일한 건 4/15**, 결측·중복이 흔하다(§1.9.7 과 같은 탐침). **행 정체성으로 쓸 수
   없다** — 3F 는 이 값을 **관측으로만** 싣고 순위를 재계산하지 않는다(동값이 실재한다).
3. **`drwtNo1`·`drwtNo2` 의 범위 근거는 이 오퍼레이션 밖에 있다.** 총예가건수(`totRsrvtnPrceNum`)가
   **이 응답에 없다**(실측). 범위 검사는 예비가격 상세 축의 값을 읽어야 하고, 없으면 **검사 불가**를
   결과로 낸다(통과 아님).
4. **`bidprcrt` 는 percent 이고 분자가 투찰금액**이다 — §1.7.2 의 `sucsfbidRate`(분자가 최종낙찰금액)와
   **다른 축**이다. 두 율을 한 자리에 접지 않는다(P-11 이 같은 이유로 갈랐다).
5. **평가점수 넷은 scale 이 미확정이다** — 점수 축이 3A `FieldScale` 어휘에 없고 문서가 범위·소수 자리를
   적지 않는다. **어휘를 지어내지 않는다**(2026-09-01 규칙) — 필요한 확장은 P-13 채택의 비용으로 아래에 싣는다.

#### P-13 채택이 요구하는 3A 확장

| # | 없는 자리 | 어느 행이 요구하는가 |
| --- | --- | --- |
| ① | `FieldConcept` 에 투찰 축 개념 토큰(개찰결과구분·입찰분류번호·재입찰번호·개찰순위·투찰업체명·투찰금액·투찰률·추첨번호·투찰일시·평가점수 넷) | 표 전 행 |
| ② | `FieldScale` 에 **점수** 축 — `WON_INTEGER`·`PERCENT`·`COUNT`(3E 신설) 어느 것도 아니다 | 평가점수 넷 |
| ③ | `SourceEndpoint` 에 개찰완료 군 토큰 — 3E 가 세 군을 갈랐고 이 오퍼레이션은 넷째 군이다 | 표 전 행의 `presentIn` |

**②는 값이 미확정이라 열지 않는 선택도 있다** — 평가점수 넷을 이번에 등재하지 않고 미등재 키로 두면
①③ 만 필요하다. **P-13 이 그 범위를 함께 묻는다.**

> **P-13 승인 결과(2026-09-09, 결정 정본은 §6b)**: 표를 채택하고 **평가점수 넷은 제외**한다 — 문서가
> 범위·소수 자리를 적지 않아 scale 이 미확정이고 어휘를 지어내지 않는다. 그 넷은 **미등재 키로 회계**된다.
> 따라서 3A 확장은 **①③ 둘**이고 **②(점수 scale)는 열지 않는다.**
...
## 6b. 승인 결과 — 2026-09-08~09 (개찰 축 + P-7 + P-13)

**P-7 · P-9 · P-10 · P-12 · P-13 이 승인됐다** — 요청 표는 `_workspace/m3-3b2/01_curator_approval_request.md`,
...
| **P-13** | **§1.11 의 개찰완료 축(투찰 행) 필드 계약** 채택 + 평가점수 넷 포함 여부 | `authoritative`(문서 출처) | **(a) 표 채택 · 평가점수 넷 제외.** 등재: `opengRsltDivNm`·`bidClsfcNo`·`rbidNo`·`opengRank`·`prcbdrNm`·`bidprcAmt`·`bidprcrt`·`drwtNo1`·`drwtNo2`·`bidprcDt`. **등재하지 않음**: `prcbdrBizno`·`prcbdrCeoNm`(P-10 (a) — 계약 미등재가 차단의 실물이다) · `rmrk`·`cnsttyAccotBidAmtUrl`(소비자 없음, URL 은 문서가 「제공 불가」) · **평가점수 넷**(문서가 범위·소수 자리를 적지 않아 scale 미확정 — 어휘를 지어내지 않는다. 미등재 키로 회계된다). **함께 승인된 3A 확장은 ①③ 둘**(`FieldConcept` 투찰 축 토큰 · `SourceEndpoint` 개찰완료 군) — **②(점수 scale)는 열지 않는다.** 추가만이며 3A corpus 27/27 과 기존 계약 행은 불변 | §1.11 |
