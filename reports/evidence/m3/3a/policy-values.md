# 3A 정책 데이터 값 표 — **운영자 승인 2026-09-07** · **§1.7·§1.9 추가 2026-09-08(승인 대기)**
...
>
> **2026-09-08 추가 — 아직 승인되지 않았다.** 운영자가 **낙찰정보서비스 1.1 참고자료**와
> **공공데이터개방표준서비스 1.2 참고자료**를 확보해(P-6 의 선행 조건) M3/3B-2 curator 레인이
> **§1.7 을 「문서 미확보」에서 필드 계약 표로 채우고 §1.9 「낙찰정보서비스 오퍼레이션 계약」을
> 신설했다.** 두 절의 `authoritative` 칸은 §1.1~§1.6 과 같은 잣대(항목설명 문면에서 직접 읽은 것)로
> 썼으나 **초기값 채택은 승인 대상**이며 요청 표는 `_workspace/m3-3b2/01_curator_approval_request.md`
> 의 **P-9~P-12** 다. §6 의 「승인 대기」 절이 그 상태를 싣는다. **P-1~P-6 의 승인 범위는 바뀌지
> 않았다** — 이 두 절의 어느 값도 아직 인스턴스화 대상이 아니다.
...
### 1.7 개찰·예비가격 키 — **문서 확보 2026-09-08 · 승인 대기(P-9)**
...
### 1.9 낙찰정보서비스 오퍼레이션 계약 — **문서 확보 2026-09-08 · 승인 대기(P-12)**
...
커밋도 그 레인이다. 그 레인이 사용량 한도로 중단된 뒤 **§1.9 와 §6 「승인 대기」 절은 세션 모델이 직접**
...
**아래 여섯은 아직 결정되지 않았다.** 요청 표(선택지·추천안·근거)는
`_workspace/m3-3b2/01_curator_approval_request.md` 이고 이 절은 **상태만** 싣는다. 승인이 오면 §6 의
표 아래에 결정 원문을 축어로 붙이고 이 절의 해당 행을 지운다(§6 이 결정의 정본 자리다).

| # | 대상 | 근거 절 | 상태 |
| --- | --- | --- | --- |
| **P-7** | **업무구분 문서 열거값**(물품·용역·공사·외자)을 P-1~P-6 과 같은 격의 승인 항목으로 등재 | §1.5 · §5 | **대기** — 3A 구현 레인 인계 `_workspace/m3-3a/05_handoff_to_curator_P7.md`. 3A 코드(`CollectionPolicyTest`)가 이미 그 값을 대조하므로 **값을 바꾸는 결정이면 3A 후속 커밋이 따른다** |
| **P-8** | **첨부 URL 필드 계약**(`ntceSpecDocUrl1` 계열) 표 추가 | 3C `commands.md` 알려진 제한 · `OPEN-3C-ATTACHMENT-FIELD-CONTRACT` | **대기** — 3B-2 또는 M4 4B 착수 전 |
| **P-9** | §1.7 의 `authoritative` 칸을 **3B-2 필드 계약 정책 데이터의 초기값**으로(P-1 의 개찰 축 판) | §1.7.1~§1.7.5 | **대기** — 채택하면 「P-9 채택이 요구하는 3A 확장」 ①②④ 가 3A 좁은 확장 한 커밋으로 따라온다 |
| **P-10** | `bidwinnrNm`·`bidwinnrBizno`·`opengCorpInfo`(및 §1.9.4 의 `prcbdrBizno`·`prcbdrCeoNm`)의 **수집·저장·원문 보존·로그 masking 정책** | §1.7.3·§1.7.5·§1.9.4 · `data-extract.md` §7 | **대기 — 3B-2 의 차단 조건.** 승인 전에는 계약 미등재 = 미지 필드 회계이며 어댑터가 값을 도메인에 들이지 않는다 |
| **P-11** | `sucsfbidRate` 의 **percent → fraction 변환**을 계약 `scale` 로 두고 밴드는 `data-dictionary.md` §1.4.3 참조로(legacy `0.5~1.0` 재선언 불채택 확정) | §1.7.2 | **대기** |
| **P-12** | §1.9 의 오퍼레이션 계약 — **`inqryDiv` 를 오퍼레이션 군별 정책 데이터로** 두고 base URI·tps·항목크기 문면을 초기값으로 | §1.9.1~§1.9.5 | **대기** |

**이 절이 있는 동안 §1.7·§1.9 의 어느 값도 인스턴스화 대상이 아니다** — 「미확정 칸은
인스턴스화하지 않는다」와 같은 규율이 **미승인 절 전체**에 적용된다.
