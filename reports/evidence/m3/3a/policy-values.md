...
### 1.11 개찰완료 축(투찰 행) 필드 계약 — **P-13, 승인 대기 (2026-09-09 작성)**
...
> **P-13 이 묻는 것**: 위 표(사업자등록번호·대표자명·비고·URL **제외**)를 3F 필드 계약 정책 데이터의
> 초기값으로 채택하는가. 그리고 **평가점수 넷을 이번에 포함하는가**(포함하면 ② 어휘 확장이 따라온다).
> 값은 전부 문서 문면 인용이므로 **등재 결정이지 값 결정이 아니다.**
...
## 승인 대기 — P-8 · P-13 (+ P-11 은 귀결)

| # | 대상 | 근거 절 | 상태 |
| --- | --- | --- | --- |
| **P-13** | **§1.11 의 개찰완료 축(투찰 행) 필드 계약** + **평가점수 넷 포함 여부** | §1.11 | **표 작성됨 2026-09-09(세션 모델) · 채택 대기 — M3/3F 의 차단 조건**. 계약 없이는 어댑터가 값을 꺼낼 경로가 구조적으로 없다(`KonepsFieldContract` 생성자가 procurement `internal`) |
