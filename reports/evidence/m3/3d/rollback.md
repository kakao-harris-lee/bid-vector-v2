...
## 실측(임시 clone, F-7 3B 몫 이어붙임 뒤 재실측 — 세 번째)

1. 임시 clone에 이 head(`782c4ae`)를 clone, `main` 체크아웃.
2. 위 `git restore` + `git clean` 두 명령 실행(`KonepsJson.kt`·`KonepsRawItemMapper.kt`·
   `KonepsOpenApiNoticeSourceTest.kt` 새로 추가된 세 경로 포함) — `exit=0`.
3. `git status --short` — `adapters/build.gradle.kts`·`gradle/libs.versions.toml`·
   `config/quality/gate-tests.properties`·`procurement/.../RawObservation.kt`·위 koneps 세
   파일까지 총 7개 `M`(base에도 있던 파일이라 삭제가 아니라 수정 복원 — sourceText 필드·
   mapper 배선·새 test가 모두 사라진 base 상태), 나머지(3D 신설 persistence 파일 전부)는
   완전 제거(untracked 잔존 없음).
4. 되돌린 트리에서 `./gradlew --no-build-cache clean check` — **SUCCESS**(347 tasks, 3B
   착수 이전 상태로 돌아간 저장소가 여전히 초록임을 확인).
5. 임시 clone 삭제.
