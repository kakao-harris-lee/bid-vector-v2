...
**verifier r3 F-18·F-19·F-20 수정 뒤 추가 확인**: F-14 수정이 세 번째 술어 수정이 되자
"구멍을 하나씩 막는" 접근을 접고 `breaking-mutations.sh` 전체를 양성 단언 구조로
재작성했다. (i) F-18 — `VAR=$(policy_value ..)`의 `exit 2`가 명령 치환 서브셸만 끝내
부모가 빈 값으로 계속 갔다(정책 키 삭제 → exit 0, `min` 삭제 + 목록 축소 → 최소치
단언 소멸, `min` 삭제 + 빈 목록 → 우회 후보 (1) 이 통과). `policy_value`가 `return 1`을
내고 호출부가 `|| exit 2`로 받는 관례로 바꿔 정책 키 삭제·`min` 삭제 둘 다 exit 2 로
확인했다. (ii) F-19 — COMPILE 가드가 「있으면 멈춤」이라는 음성 단언뿐이라 `"type"`을
하나도 못 뽑으면(`--error-format=text`로 모의) 전건이 거짓 `[잡힘]`으로 exit 0 이었다 —
`caught`을 "컴파일 오류 없음 AND 진짜 규칙 이름 확보"라는 양성 조건으로 바꿔 같은
모의가 exit 2 를 내는 것을 확인했다. S-3 스크립트 자신에 `buf --version` 대 정책
`tool.buf.version` 대조도 신설했다(불일치 exit 2 확인). (iii) F-20 — `expected.tsv`가
결과 기록일 뿐 대조하는 게이트가 없어 규칙이 조용히 표류해도 안 잡혔다 — 커밋된 표를
기대값으로 읽어 `rule_type` 불일치를 exit 1 로 내도록 바꾸고(재현: `field-delete` 행을
임의 값으로 바꾸자 `[규칙 표류]` + exit 1), 갱신은 `--update` 플래그로만 하게 했다
(플래그 없이 원복 재실행 시 파일 무접촉 확인).
