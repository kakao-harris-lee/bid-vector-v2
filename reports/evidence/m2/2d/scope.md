...
in_scope:
  ...
  - build-logic/**                                    # `contractGate` task: buf lint·breaking·재생성 diff 0 · 생성물 수동 편집 금지
  - config/quality/gate-tests.properties              # `gate.tests.contract` 키(2D 게이트 test 등재 — 게이트 정의 편집, 사유를 evidence 에)
  - config/quality/contract-policy.properties         # 정책 데이터: max_message_bytes · 승인 태그 이름 규칙 · mutation 집합 목록(매직넘버 금지)
  - ml-contract/src/test/**                            # Kotlin 쪽: unknown field·unknown enum·max payload·deadline·cancellation·round-trip (in-process grpc-testing)
  - adapters/src/test/kotlin/**                        # consumer test 통합(2B·2C 의 fake servicer test 를 한 suite 로)
  ...
  - .github/** 또는 CI 정의                           # 조건부 — 저장소가 CI 정의를 갖는 경우에만 `contractGate` 등재
...
acceptance_commands:
  ...
  - "./gradlew contractGate"                                                                          # S-2 — lint·breaking(against 승인 태그)·재생성 diff 0
...
| ③ | **재생성 diff 0** — `contractGate` 가 `generateProto` 를 돌리고 `git diff --exit-code -- ml-contract/src/generated ml-engine/src/ml_engine/contracts` 가 0 이어야 한다. 생성물 수동 편집이 게이트에서 드러난다 | 「설계 규칙」 생성된 코드는 수동 편집하지 않는다 |
...
| ⑨ | **게이트 등재** — `gate.tests.contract` 키로 1A 의 `gateExecutionGate` 가 2D test 의 실행을 강제(제외·필터 우회 차단 — 1A 관례). `contractGate` 는 `check` 의 의존 | `milestone-1.md` 「완료 조건 — 게이트 회피 경계」 · 1A `gate-tests.properties` |
...
| **D-2D-2** | **생성물을 커밋하는가.** D-M2-1 (a) 가 「생성물만의 모듈」을 두었다 | (a) **커밋한다 + 재생성 diff 0 게이트(③)** (b) 빌드 산출물로만(커밋 안 함) | **(a)** — (b) 는 리뷰 diff 에 생성물이 없어 「수동 편집 금지」를 증명할 대상이 없고, codex/verifier 의 오프라인 worktree(§4b 관례)에서 protoc 플러그인 배포본이 없으면 빌드 자체가 안 선다. 크기는 래칫 제외 모듈이라 문제없음 | 착수 전 |
...
| **D-2D-4** | mutation 집합은 `.proto` 를 **저장소에 두지 않고** 스크립트가 임시로 만든다 — 저장소에 breaking 인 `.proto` 가 있으면 buf 가 그것을 읽는다. 스크립트 종료 코드는 브리프 2 스윕 규약(0/1/2) | — | 계약 고정 |
| **D-2D-5** | `max_message_bytes` 는 정책 데이터이고 **양쪽이 같은 파일을 읽지 않는다**(Python 은 Gradle 정책을 못 읽음) — 대신 **양쪽 test 가 같은 testdata 의 초과 표본으로 거부를 확인**한다(값 동일성의 증명은 test 로) | — | 계약 고정 |
| **D-2D-6** | 취소 검증은 플래그가 아니라 **자원 해제**(fake servicer 의 「계산 시작/중단」 카운터, Kotlin 의 채널 반환) — 조사 02 의 grpc/grpc#36193 | — | 계약 고정 |
...
**우회 후보(≥5)**: (1) mutation 스크립트가 빈 집합을 돌고 exit 0 → 양성 대조(②) + 집합 크기 ≥ N 을 정책 데이터로 단언 (2) `against` 를 HEAD 로 바꿈 → `contract-policy` 의 태그 규칙과 대조하는 test(게이트 정의 편집 — 경계 밖이나 검출은 함) (3) 생성물을 고치고 `generateProto` 를 `onlyIf { false }` → 재생성 diff 게이트는 실행 자체가 조건 — `gateExecutionGate` 가 `onlyIf` 를 잡는지 착수 시 실측(1A 실측 범위 확인) (4) unknown enum 거부를 Kotlin 만 하고 Python 은 안 함 → 양쪽 test 가 같은 testdata (5) `max_message_bytes` 를 한쪽만 올림 → D-2D-5 (6) 취소 test 를 `cancelled()` 폴링으로 → D-2D-6 리뷰 항목(test 가 거짓 초록일 수 있는 자리 — 알려진 제한) (7) 승인 태그를 breaking 뒤로 옮김 → 운영자 행위, 경계 밖·등재.
...
- `grpcio-tools` 번들 protoc 버전 미확인 → 재생성 diff 0(③) 의 전제: **Kotlin 플러그인의 protoc 과 Python 의 protoc 이 같은 `.proto` 에서 같은 descriptor 를 내야** 한다 — 버전이 달라도 proto3 wire 는 같지만 생성 소스는 다르므로 **양쪽 생성물은 각자의 도구 버전에 고정**(정책 데이터에 둘 다 기록).
