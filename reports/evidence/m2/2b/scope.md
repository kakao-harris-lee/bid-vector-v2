...
| # | 일 | 승인 문면 |
| --- | --- | --- |
| ① | **`CalculateOptimalBid` 입력** — `RequestEnvelope`(2A ④) + `FeatureInputs`(D-2B-1: 원 fact — `base_amount: Money[basis=BASE_AMOUNT 필수]`·`category_code`·`agency_id`(불투명 식별자, DB id 아님)·`base_amount_provenance: ProvenanceKind`·`opened_on`) + `repeated CompetitionSample`(D-2B-3) + `OptimizationObjective`(D-2B-4). floor rate 는 **입력에 없다** — legacy 에서도 모델 입력이 아니라 guardrail 입력이었고(조사 (a-4)), V2 는 Kotlin `decision` 소유 | 2B 「입력: canonical 금액, floor rate, versioned feature vector, 정제된 경쟁 표본, optimization objective」 — floor rate 는 D-M2-4 로 제외, 「versioned feature vector」는 `feature_schema_version` 아래의 이름 붙은 fact 로 읽는다(D-2B-1) |
...
| ④ | **후보는 율뿐** — `Candidate { CandidateLabel label, Rate bid_rate[source=BID_RATE], Weight weight(decimal) }`. 원 단위 가격 없음(D-2B-2). 후보 셋의 가중치는 응답이 싣되 **정책 version 을 동반**(legacy 는 경로마다 0.24/0.52/0.24 와 0.22/0.56/0.22 가 갈렸다 — 조사 (b-1)) | ML-01 「각 후보의 rate가 fraction 단위」 · §4.1 |
...
| ⑥ | **모델 식별은 구조화** — `ModelRelease { release_id, artifact_checksum, feature_schema_version, code_version, dataset_id }` 다섯(H-9). `Success.release` 는 요청의 `exact_release` 와 같아야 한다(2A ⑥ 제3 변환 금지) | `v2-지침서.md` §5 「schema version, code version, dataset id, checksum, metric」 · §9 재현성 |
...
| ⑧ | **`GetModelMetadata`** — 입력 없음(봉투만) → `{ ModelRelease promoted, repeated string supported_feature_schema_versions, Readiness readiness ∈ { READY, LOADING, NOT_READY } }`. `latest_promoted` 선택자의 해석 근거를 client 가 얻는 유일한 자리 | 2B 「release, feature schema, readiness 정보」 |
...
| **D-2B-3** | **경쟁 표본의 형태.** legacy 는 duck-typed dict 15+키에 DB id 셋(`historical_data_id`·`project_id`·`tender_result_id`)이 실렸다 | (a) **`CompetitionSample { Rate bid_rate[BID_RATE], Money base_amount[BASE_AMOUNT], ProvenanceKind base_amount_provenance, opened_on, optional Rate award_rate[AWARD_RATE] }`** — 식별자 없음, 정제는 송신 전 Kotlin 어댑터(M4) (b) 표본 대신 「표본 조회 키」를 보내고 ml-engine 이 읽음 | **(a)** — (b) 는 serving 이 DB 를 갖게 된다(§3.2 금지). GBM 은 표본을 쓰지 않고 distribution predictor 만 쓰므로 `repeated` 가 비어 있어도 유효(비어 있음 ≠ 결측 — 「표본 없음」은 `Unmeasurable(INSUFFICIENT_SAMPLES)` 의 원인이지 요청 오류가 아니다) | 착수 전 |
...
- GBM 피처 5개 중 3개가 아티팩트 의존 파생값 → D-2B-1 (a). `denominator_source` 는 서빙에서 `clean-base` 상수 고정 — V2 는 요청의 `ProvenanceKind` 가 실값을 나른다(D-M2-10).
- GBM 은 경쟁 표본을 읽지 않고 distribution predictor 만 읽는다(basis `clean` 행만) → D-2B-3 의 「비어 있어도 유효」.
