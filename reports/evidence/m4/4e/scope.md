...
`git log --oneline f600909fcde1c08bb83abaf749cfa681539831a0..HEAD -- CLAUDE.md .claude/` — 착수 시 **없음**. 이 브랜치는 격리돼 있어 하네스
레인 커밋이 섞일 자리가 없다. 리뷰 요청 시점마다 재실행해 갱신한다. `m4/2026-09-08` 쪽 하네스 커밋은 병합 때 들어오며 이 slice 산출물이 아니다.
...
| **`OPEN-4E-CORPUS`**(신설) | ② 전수 표의 corpus 승격 — `m4` 병합 뒤 curator, 운영자가 원할 때(D-4E-6) |
| **`OPEN-4E-POLICY-VALUES`**(신설) | ⑦ 의 값(환경 → 모드 매핑, suffix 길이) — 종결 승인 시 확정(`policy-values.md`) |
