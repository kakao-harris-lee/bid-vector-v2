...
**우회 후보 — 값 위조 축**: (1) 차원이 다른 `values` → ③ 이 `Unavailable` 로 접는다 (2) `normalization=UNSPECIFIED` 로 정규화 안 된 벡터 주입 → ③ (3) release 공백 → ③ (4) `latest_promoted` 요청에 다른 release 응답 → ④ (5) 예외를 던져 상위가 `try` 로 삼키게 → **port 계약이 결과 갈래**라 `try` 쓸 자리가 없다(4B-2 관례) (6) **벡터 값 타입을 밖에서 지어 점수 경로에 주입** → **열려 있다(정정 2026-09-11).** 착수 계약은 「생성 경로를 `internal` 로 닫는다」고 적었으나 `internal` 은 **모듈 범위**라 `workflow` 의 값 타입을 `adapters` 어댑터가 생성할 수 없다(4D-1 `BidRateCandidates` 와 같은 자리). 4C-1 `ClaimedOutboxRow` 배치는 **가능하지만 위조를 닫지 않고 옮긴다** — 복원자를 감싸는 배선이 public 이어야 해서 「가짜 port」 경로가 남는다(verifier r1 판정). `EmbeddingVector` 는 권한을 나르는 값이 아니라 **숫자**라 `EventEnvelope` 와 같은 등급이 아니다. **오늘은 소비자가 없어 도달 불가이나 「구조상 불가」가 아니다** — `OPEN-4D2-VECTOR-FORGERY-AT-WIRING` 으로 **4B-6(소비자 배선 slice)** 에 넘긴다. `init` 가 막는 것은 빈 리스트·norm 1±`COARSE_NORM_EPSILON` 밖·비유한 값뿐이며 **형태 하한이지 위조 방어가 아니다**.
