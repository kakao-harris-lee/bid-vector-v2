...
**`OPEN-LEAK-BASELINE-COORD` 신설 (2026-09-12, PR #7)**: 그 baseline 의 **키가 `파일:줄`이라
위쪽 편집에 밀린다.** 이 파일의 패턴 어휘 인용 한 줄이 그 위 4줄 삽입으로 `:64 → :68` 로 옮기자,
게이트는 `:68` 을 「새 매치」로 막고 `:64` 는 아무것도 안 가리키는 죽은 항목이 됐다 — CI 가
붉어진 실제 사유다(PR #7 1차). 게이트는 죽은 항목을 `stale_baseline` 으로 **보고는 하지만 막지
않는다**. 하네스가 네 번 겪고 성문화한 **「낡는 좌표」 클래스**(선언 SHA → 정책 version → RED
수치 → 줄 번호)가 이 게이트의 키 설계로 **상설화**된 것이고, 그 규율을 evidence 문서에는
적용하면서 게이트 자신에는 적용하지 않았다. 처분(내용 기반 키로 교체)은 **게이트 술어 변경**이라
표적 재검증이 필요하고 이 PR 의 범위(주석 정정)가 아니다 — 후속 slice 로 인계한다.
