...
### 상충 7건의 성격 — 「판정이 갈린다」가 아니라 대개 **정본 표가 낡았다**

팀장이 둘을 직접 재확인했고 성격이 같았다.
...
나머지 다섯(`OPEN-4C1-TX-CONTRACT-UNVERIFIED`·`OPEN-2E-TEXT-SYNTHESIS`·`OPEN-4B6B-POLICY-VALUES`·
`OPEN-NOTI-05`·`OPEN-NOTI-06`)은 재고 §6.2 에 양쪽 문면이 병기돼 있다. 그중 **`OPEN-NOTI-06` 은
결이 다르다** — 담당이 「M4 이후 운영 관측」과 「4B 소유로 이관」으로 갈리는데 **받은 4B slice 가
없다**. 이것은 갱신 누락이 아니라 **소유자 공백**이다.
