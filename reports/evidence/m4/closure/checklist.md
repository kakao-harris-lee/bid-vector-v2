...
**확인 불가 → 해소(판정 뒤, 2026-09-18).** 「반환형으로만 스치는 좌표를 텍스트 술어가 못 본다」는
r2 시점에 미병합 레인의 실측이라 확인 불가로 내렸다. 그 레인이 **PR #34 로 병합돼 근거가 저장소에
들어왔다** — 그 slice 계약의 결정 `D-6F1-8` 이 「`StrategyRow.toRow()`(→`budgetColumnsOf`)가
`ProvenanceCodec.kindOf(it).name` 을 써 **반환형으로만** `bidvector.procurement.ProvenanceKind`
를 스쳤다 — import 는 없어 소스 텍스트 게이트가 보지 못했다」로 적고, 처방으로 문자열 반환
진입점(non-inline)을 신설했다. **그 slice 의 base 도 이 판정과 같은 `8652893`** 이다.

그래서 이 문장은 이제 **사실로 쓸 수 있다** — 단 **출처는 M4 가 아니라 그 slice 의 계약**이고,
「아무도 심지 않은 위반」이라는 성격도 그 문서가 「실재 의존 위반」으로 적어 뒷받침한다. 수치
(좌표 감소·게이트 통과 수)는 그 문서에 그 형태로 없으므로 **인용하지 않는다.**

이것이 제한 ⑥ 을 왜 열린 것으로 두는지의 가장 강한 근거다 — M4 가 선언한 사각이 **다른 계층에서
실제 위반으로 실현됐다.**
...
### 상충의 성격 — 스윕 시점 **7 = 갱신 누락 4 · 그 밖 3**, 결정 반영 후 **6 = 갱신 누락 4 · 그 밖 2**

초판은 「대개 갱신 누락이고 판정이 갈리는 것이 아니다」로 단정했다. **3/7 에서 틀렸다**
(verifier r1 MEDIUM-5). 갱신 누락은 `OPEN-DIC-03`·`OPEN-4C1-TX-CONTRACT-UNVERIFIED`·
`OPEN-4D3-SAMPLE-SUPPLY`·`OPEN-NOTI-05` **넷**이고, 나머지 셋은 다른 성격이었다.
**결정 2 로 `OPEN-NOTI-06` 이 이월로 나가 그 밖이 둘이 된다**(verifier r2 MEDIUM-2).
...
## 운영자 결정 (2026-09-18) — 넷 다 추천안

| # | 결정 | 귀결 |
| --- | --- | --- |
| 1 | **최소 종결** — 판정만 확정하고 상충 문면 정정·미결 이월처 지정·이월 빈 둘의 재분류는 **각 계열 소유 slice** 로 넘긴다 | M5 가 이월 14건을 남기고 종결한 전례와 같다. `in_scope` 를 넓히지 않는다 — **`docs/discovery/capability-map.md` 와 사전은 이 slice 가 편집하지 않는다**. 낡은 표 넷은 당장 그대로 남고, 그 사실을 여기 선언으로 남긴다 |
| 2 | **`OPEN-NOTI-06` → 6F-7 이월** | 알림 요청을 outbox 로 잇는 slice 가 그 축의 유일한 실제 소비자다. 소유자 공백이 닫히므로 **배타 분류가 상충 → 이월로 한 칸 이동한다**(아래 수치). **단 아래 「받는 자리의 지위」를 함께 읽어야 한다** |
| 3 | **게이트 공백은 후속 slice 등재만** | 완료 조건 ⑦ 을 미충족으로 내리지 않는다. M4 는 port 경계까지가 범위였고 이 공백은 새로 생긴 것이 아니다. 식별자를 신설해 추적한다 — **`OPEN-M4-APP-LAYER-GROUP-GATE`** |
| 4 | **재검증 통과 뒤 PR** | 정정 커밋을 검증 레인이 다시 본 뒤 push·PR 하고 판정 코멘트를 남긴다(전역 규약 §2) |

### 판정 뒤 `main` 이 움직였다 (2026-09-18, 선언)

이 판정의 base 는 `8652893` 이고 그 뒤 `main` 이 **43 커밋** 움직였다(병합 하나 — **PR #34**,
배선 slice 6F-1). 판정을 다시 돌리지 않고 **무엇이 달라지는지만 선언한다.**

| 이 문서의 진술 | 병합이 바꾼 것 |
| --- | --- |
| 「6F-7 은 이 base 의 승인 문서에 없다」 | **조건이 충족됐다** — `main` 의 `milestone-6.md` 가 이제 6F-1~6F-7 을 정의한다. 아래 절의 「조건부」는 **해소**이고, 문면은 base 기준 사실로 남긴다 |
| 조건 ⑥ 의 좁혀짐 사유(「배선 부재」) | **줄었지만 닫히지 않았다** — 비어 있던 port 구현 하나(전략 영속)가 실 구현으로 채워졌다. 사슬을 재려면 남은 어댑터들과 app 조립(6A)이 여전히 필요하다 |
| 제한 ⑥ 의 「확인 불가」 | **해소됐다**(아래 축 3) |
| 완료 조건 전건 측정 | **재측정하지 않았다.** 코드 변경이 0 인 문서 slice 이고 base 를 문면에 못 박았다. 조건 판정이 바뀔 자리는 위 둘뿐이며 **둘 다 좁혀짐을 더 좁히는 방향**이다(판정을 뒤집지 않는다) |

### 받는 자리의 지위 — **6F-7 은 이 base 의 승인 문서에 없다**(판정 뒤 해소)

**정직하게 적어 둔다**(verifier r2 MEDIUM-1). 판정 base 의 `milestone-6.md` 는 slice **6A~6E**
를 정의하고 **6F 군은 없다.** 6F-1~6F-7 은 **아직 병합되지 않은 다른 레인**의 문서에서 정의되며,
이 base 의 저장소에서 `6F-7` 이 나오는 자리는 **이 종결 문서들뿐**이다. 같은 전제를 세 곳이
공유한다 — 결정 2 의 이월처 · 완료 조건 ⑤ 의 받는 자리 · `OPEN-STR-12` 의 전망.

**그래서 이 셋은 조건부다** — 그 레인이 병합되면 성립하고, 병합되지 않거나 slice 번호가 바뀌면
받는 자리를 다시 정해야 한다. **결정 자체는 유효하다**(운영자가 그 축의 실제 소비자를 지목했다);
문서가 밝히지 않은 것이 결함이었다. `OPEN-NOTI-06` 은 그때까지 **「이월(조건부)」** 로 읽는다.

이것은 **r1 HIGH-2 와 같은 계열의 실수**다 — 병합되지 않은 레인의 사실을 base 문서에 근거 없이
실었다. HIGH-2 는 「실측」을, 이 자리는 「slice 번호」를 그렇게 썼다. r1·r2 양쪽 검증이 조건 ⑤
행에서 이것을 한 번씩 놓쳤다는 사실도 함께 남긴다.

### 결정 2 의 귀결 — 배타 분류 갱신

**아래 「스윕 시점」 열은 2026-09-18 스윕 직후의 값이고, 「결정 반영」 열이 현행이다.**

| 갈래 | 스윕 시점(결정 전) | **결정 반영(현행)** |
| --- | --- | --- |
| 종결 | 36 | 36 |
| 이월 | 59 | **60** (`OPEN-NOTI-06` 수령: 6F-7, 조건부) |
| 미결 | 12 | 12 |
| 상충 | 7 | **6** |
| 참조 상태 | 1 | 1 |
| 합 | 115 | **115** |

`open-inventory.md` 는 **스윕 기록이라 고치지 않는다** — 그 파일은 「어느 문서가 무엇을 적는지」의
사본이고, 결정에 따른 재분류는 이 문서가 정본이다.

### 결정 1 이 남기는 것 (선언)

최소 종결을 고르면 아래가 **의도적으로 남는다.** 숨기지 않고 적는다.

- **갱신 누락 4건의 낡은 표**(`OPEN-DIC-03`·`OPEN-4C1-TX-CONTRACT-UNVERIFIED`·
  `OPEN-4D3-SAMPLE-SUPPLY`·`OPEN-NOTI-05`) — 정본 표가 종결된 항목을 열린 것처럼 적는 상태가
  계속된다. 각 계열 소유 slice 가 자기 문면을 고칠 때 함께 닫는다.
- **정본이 두 값을 말하는 하나**(`OPEN-4B6B-POLICY-VALUES`)와 **닫는 주체가 갈리는 하나**
  (`OPEN-2E-TEXT-SYNTHESIS`) — 남의 표 갱신이 아니라 그 문서 자신의 판정이 필요하다.
- **이월 문면이 빈 둘**(`OPEN-4D2-VECTOR-FORGERY-AT-WIRING`·
  `OPEN-3E-OPENING-AMOUNT-AUTHORITY-GUARD`) — 문면상 이월이지만 받는 쪽 evidence 에 식별자가
  0건이거나 전제가 이미 깨졌다. **실질은 미결에 가깝다는 판정을 여기 남기고 재분류는 하지 않는다.**
- **미결 12건의 이월처 미지정** — `OPEN-STR-12` 는 완료 조건 ⑤ 와 같은 뿌리라 6F-7 뒤에 자연히
  닫히고, 나머지 열하나는 소유 slice 가 정해질 때 붙는다.

### 신설 — `OPEN-M4-APP-LAYER-GROUP-GATE`

**무엇**: `group.forbidden`(드라이버·HTTP 클라이언트·프레임워크)과 domain 외부 allow-list 가
domain 층에만 적용돼 **application 층에는 금지 group 게이트가 없다.** JDK DB·network API 는
텍스트 allow-list 가 설계상 허용한다. 완료 조건 ⑦ 이 문면으로 금지한 **인라인 변종**이 게이트
전건을 통과한다(verifier r1 HIGH-1 변이 실측).
**소유**: 미정 — 후속 slice. **근거**: 이 문서 축 1 의 ②·⑦ 행과 verifier r1 보고서.
**capability-map 등재는 이 slice 밖**이다(결정 1) — 등재 빚을 여기 선언한다.
