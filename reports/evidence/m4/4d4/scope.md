...
head_sha: 342327d9f9fe730ae631611cc3a0e495cbbab012
in_scope:
...
| (7) | `String.format`·`%.2f`·`Locale` 로 숫자를 찍는다 | **갱신 2** — `ArchitectureGateTest` 의 Locale 축은 `layer.domain` 모듈에만 걸리고 `workflow` 는 `layer.application` 이라 대상이 아니다(verifier r1 V-1 실측: `Locale.getDefault()`+`String.format` 을 심어도 전건 exit 0). 잠금은 S-5 관례의 **소스 경계 test** `EvidenceLinesBoundaryTest`(`EvidenceLines.kt` 소스에 `java.util.Locale`·`java.text.`·`String.format`·`.format(` 참조 0 + 양성 대조) · 골든 test 의 `toPlainString()` 고정 |
...
| 2 | 2026-09-16 | 우회 (7) 의 닫힘 기제를 `ArchitectureGateTest` 에서 **소스 경계 test `EvidenceLinesBoundaryTest`**(신설, in_scope 추가, `gate.tests.workflow` 등재)로 교체. D-4D4-4 의 「`ArchitectureGateTest` 축」 문구 정정. 골든·시그니처·배치 불변 | verifier r1 **V-1(high)** — `ArchitectureGateTest` 의 허용목록·멤버 규칙은 `policy.domainModules`(`layer.domain`)에만 걸리고 `workflow` 는 `layer.application` 이라 대상이 아니다. 실측: `EvidenceLines.kt` 에 `java.util.Locale` import + `String.format(Locale.getDefault(), "%d", …)` 을 심어도 전건 `clean check` exit 0. 계약이 인용한 「4B-8 실측」은 `procurement`(domain) 의 것이었고 application 층까지 선다는 근거가 없었다 — 오늘의 출력은 옳지만 회귀 잠금이 없었다. 대안(`domainModules` 에 `workflow` 추가)은 app 게이트 정책 변경이라 이 slice 범위 밖 |
