...
게이트가 그 내용을 요구하지 않는다는 사실을 실측으로 뒷받침한다 — 승인 문서가
4D-4 착수를 선언한 채 남는 것은 게이트 위반이 아니라 팀장의 종결 절차 몫이다.
...
A  workflow/src/test/kotlin/bidvector/workflow/evaluation/EvidenceLinesBoundaryTest.kt
...
커밋(`913a3e3`·`af6b669`·`97fe044`) 셋만 만졌다(`git log --oneline 44721cf..HEAD --
...
git restore --source=44721cf --staged --worktree -- \
  ...
  workflow/src/test/kotlin/bidvector/workflow/evaluation/EvidenceLinesBoundaryTest.kt \
  config/quality/gate-tests.properties
```

`git checkout <base> --` 는 쓰지 않는다(신규 경로마다 pathspec 오류로 exit 1 —
1A 16차 high 재발 방지). `EvidenceLines.kt`·`EvidenceLinesTest.kt`·
`EvidenceLinesBoundaryTest.kt` 는 `--source` 에 해당 경로가 없으므로 restore 가
작업 트리에서 삭제한다.

## 임시 clone 실측(`git clone --no-hardlinks`)
...
architecture 게이트까지 걸치므로 전건을 돈다) → **exit 0**. 되돌린 트리가 게이트를
붉히지 않는다 — evidence 디렉터리(`reports/evidence/m4/4d4/**`)·`milestone-4.md`
는 되돌리지 않았고 그것이 게이트에 걸리지 않음도 이 실행으로 확인됐다
(M4/leak-baseline-coord 사고의 재발 없음). 정확한 task 수·소요 시간은
`commands.md` 참고(값을 이 문서에 중복 기입하지 않는다 — 낡는 좌표 회피).
