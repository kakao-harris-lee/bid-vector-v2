...
그 둘을 건드렸다(`git log --oneline 9eddf7525b74f89ce279acbb3adf4948f05c25f1..HEAD --
config/quality/gate-tests.properties milestone-4.md` — commands.md 「하네스 레인 변경
확인」과 같은 방식으로 실측, 이 절은 코드 편집 자체가 아니라 그 두 파일의 커밋 이력을
잰다). 따라서 hunk 단위 격리 없이 **전체 restore로 충분하다**.
...
(evidence 자신 — `checklist.md`·`commands.md`·`policy-values.md`·`rollback.md` — 은 이
커밋 이후 낼 목록에 추가된다.)
...
파일 전부가 삭제 대상에 포함된다. `reports/evidence/m4/4b5/scope.md`는 이 slice의
계약 문서 자체이므로, 계약은 유지한 채 코드만 되돌리려면 `reports/evidence/m4/4b5`를
이 경로 목록에서 빼고 `scope.md`만 남긴다.
...
- 위 restore 명령 실행 → `git status --porcelain` D 17(main 7·test 9·evidence 1
  `scope.md`)/M 2(`gate-tests.properties`·`milestone-4.md`) — 위 「대상 파일 목록」과
  일치.
