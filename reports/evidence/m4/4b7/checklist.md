...
| D-4B7-2 — 자격 규칙표 ②~⑦ | `judgeEligibility`+`Check<T>` guard 체인(`SampleEligibility.kt`) | `SampleEligibilityTest`의 사유별 test(아래 대응표) |
...
| (10)(13) `maxSamples` 초과 시 오래된 표본 잘림 | `ORDER BY actual_opening_at DESC NULLS LAST LIMIT` | `JdbcCompetitionSampleSourceTest`「상한을 넘는 후보는 최신 순으로 잘린다」 |
...
| 표면 | 실측 생성부 |
| --- | --- |
| `CompetitionSample(` | `BidPredictionRequest.kt:86`(타입 선언) · `SampleEligibility.kt`의 `sampleOf`(유일한 production 생성부) · test 셋(`OpportunityAnalysisTest`·`RequestMappingTest`·`MlTestFixtures`) |
...
| `reservePriceAmounts`·`drawNumberSet`·`provenanceLabelFor` | `internal`(모듈 스코프) — `SampleEligibility.kt`의 `sampleOf`/`Check` 함수들만 호출. 「경계로 처리」 행 없음 |
