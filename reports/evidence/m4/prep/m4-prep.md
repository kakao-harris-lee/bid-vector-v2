...
| **4B application use case** | notice 수집 완료 → qualification(1C) → low-cost strategy filter(1E `WatchRules`) → 필요 시 ML 요청(4D) → decision 후보 조립(1D) → state 저장 + event 기록 · 트랜잭션 경계·실패 상태 명시 · catch-all 금지 · lease port(ADR 0005 D-10) · **앱 알림함 기록의 생성**(NOTI-01 「임계 미달이면 기록 1건, 채널 알림 0건」·ADR 0005 D-4 「채널 억제는 기록 억제가 아니다」 — `NotificationRequested` 가 기록 write 와 같은 트랜잭션에서 생기고, 채널 배달 요청은 게이트 통과 시에만 outbox 로) | M3 3A·3D · 4A·4C | **부분**(ML 요청 단은 4D 뒤 — 그 전엔 `MlUnavailable` 경로만) | 착수 시 |
...
| **D-M4-4** | **이벤트 봉투 형태**(4C, `OPEN-DIC-07`·`OPEN-DIC-09`) | (a) **`EventEnvelope<P>(eventId: UUIDv7, aggregateId, aggregateVersion, occurredAt, correlationId, causationId?, idempotencyKey, actor: Actor?, payload: P)`** — payload 는 도메인 sealed(1E `StrategyUpdated` 등). **`actor` 는 `StrategyUpdated` 봉투에 한해 필수**(4A ③ 의 command actor 를 그대로 실음 — 타입 수준으로는 `StrategyUpdated` 전용 봉투 생성자가 `Actor` 를 요구), 다른 payload 는 선택 (b) 평면 필드 | **(a)** — 봉투와 payload 의 소유가 다르다(봉투 = workflow, payload = 도메인). `idempotencyKey` 는 부작용 선언(ADR 0005 D-3)의 키. command 의 actor 는 4A, 봉투의 actor 는 4C — 두 자리의 값은 같고 소유가 다르다 |
...
- M4 코드 착수 금지(M1~M3 승인 선행). 이 문서·4A·4E 초안·조사 노트까지. `workflow/`·`adapters/` 는 앵커 상태 유지. 착수 시 `base_sha` 40자.
- **Phase 2.5**: 4A(전이표·불변식)·4C(outbox 원자성)·4E(masking·idempotency 타입 불변식)는 설계 검토 대상 — scope 의 위협 모델 절은 검토 입력.
