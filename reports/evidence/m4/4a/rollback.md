...
`git diff --name-status <base>..HEAD` 에서 낸다(2026-09-08, HEAD `1db6282`).
**A(신규, 27건)**·**M(수정, 6건 — `CorpusExecutors.kt`·`SharedKernelCorpusConformanceTest.kt`·
`gate-tests.properties`·`fixtures/manifest.yaml`·`milestone-4.md`·`workflow/build.gradle.kts`)**.
전체 33건 중 `reports/evidence/m4/4a/scope.md`(M, 하네스 레인) 제외 32건이 이 slice 대상.
**라운드마다 파일이 늘면 이 목록을 다시 낸다** — 이 slice 는 5개 커밋으로 한 번에 종결됐다.
...
git restore --source=9948c6e4056bbf71fa6683aa67d30c2a49fc6eae --staged --worktree -- \
  ...
  fixtures/expected/strategy-edit-001.json fixtures/expected/strategy-edit-002.json \
  fixtures/expected/strategy-edit-003.json fixtures/expected/strategy-edit-004.json \
  fixtures/expected/strategy-edit-005.json \
  fixtures/input/strategy-edit-001.json fixtures/input/strategy-edit-002.json \
  fixtures/input/strategy-edit-003.json fixtures/input/strategy-edit-004.json \
...
`--source` 에 없는 경로(신규 파일 26건)는 이 명령이 삭제까지 처리한다 — 별도 `git rm` 불필요.
...
1. 복원 명령 exit: **0**
2. `git status --porcelain` — **D 26건 · M 6건**, 목록의 A/M 건수와 정확히 일치(신규 26 + 수정 6).
...
4. `./gradlew --no-daemon :workflow:compileKotlin :app:compileTestKotlin` — **`BUILD SUCCESSFUL`**
   (29 actionable tasks, 15 executed).
5. `./gradlew --no-daemon :workflow:test :app:test --tests '*Conformance*'` — **`BUILD SUCCESSFUL`**
   (`:workflow:test NO-SOURCE`, `:app:test` 74→69 tests 로 정상 축소 — 새 5건이 사라졌으므로 기대한 결과).

되돌린 트리의 `workflow` 모듈은 `ModuleBoundaryAnchor.kt` 하나만 남는다(1A 관례 — 경계 규칙이
빈 집합 위에서 공허 통과하지 않게 하는 anchor). `app` 의 conformance corpus 는 4A 이전 66건
(`strategy-watch` 8·`strategy-validation` 3 등 기존 축)으로 되돌아간다.
