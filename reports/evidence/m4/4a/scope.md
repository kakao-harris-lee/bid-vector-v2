...
acceptance_commands:
  ...
  - "./gradlew :workflow:moduleDependencyGate :workflow:sizeGate :workflow:cpdCheck"                   # S-3 — application 층 게이트(도메인 게이트 셋은 domain 층 전용 — 착수 시 workflow 에 무엇이 걸리는지 실측)
...
| ③ | **actor / operator scope** — command 는 `actor: Actor` 를 나르고(`Operator(id)` · `System(reason)` — D-M4-2 타입만), 세션은 시작 actor 와 다른 operator 의 command 를 거부. 단일 회사(`OPEN-STR-07` 해소)라 operator 식별자는 세션 소유권 검사에만 | 4A 「actor/operator scope」 · 1E D-17 |
...
| ⑦ | **채널 독립** — use case 입력은 도메인 값(field id·원시 값 문자열)이고 Telegram/웹 DTO·alias 라우팅은 어댑터. `workflow` 가 Telegram library 를 import 하지 않음은 `moduleDependencyGate`(외부 의존은 domain 층만 검사 — application 층은 **`group.forbidden` 확장 필요 여부 착수 시 실측**) | 4A 「Telegram DTO 는 adapter 에만 존재하고 domain state 가 Telegram library 를 import 하지 않는다」 · STR-11 「채널 독립 유스케이스 하나」 |
...
**방어한다**: (a) 검증 없이 적용(② + 1E `internal constructor`) (b) 전이표 밖 전이의 조용한 무시(① 거부 관측) (c) 우회 write 경로(⑥ — `StrategyRepository.save` 는 use case 만 호출: 리뷰 항목 + ArchUnit 가능 여부 실측) (d) 중복 command 의 이중 적용(④) (e) stale confirm(⑤ revision 대조) (f) System 의 무승인 갱신(D-4A-5) (g) Telegram 타입의 application 유입(⑦).
**방어하지 않는다**: 채널 어댑터의 alias 매핑 옳음 · 저장 구현의 원자성(4C/3D) · 스냅샷 재계산의 실행(4B/후속) · timeout 값의 옳음.

**우회 후보(≥5)**: (1) `Applied` 로 직접 `copy` → 전이는 `apply(command)` 하나(1E `internal constructor` 관례) (2) `Confirmed` 에 revision 없이 → 필수 필드 (3) `StrategyRepository.save` 를 어댑터가 직접 호출 → 리뷰·ArchUnit(착수 시) (4) timeout 을 정책에서 무한 → 정책 상한 불변식 (5) 같은 `commandId` 로 다른 command → `IdempotencyConflict` 거부 (6) `System` actor 로 `Confirmed` → D-4A-5 거부 test.
