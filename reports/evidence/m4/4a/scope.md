...
- **종결 승인 이후(2026-09-09) 같은 브랜치에 하네스 커밋 1건이 붙었다** — `harness(v2-slice-pipeline): Phase 2.5 에 (2b) 값 획득 축 신설`
  (`CLAUDE.md`·`.claude/skills/v2-slice-pipeline/SKILL.md`). **slice 산출물이 아니며 in_scope 밖**이고, 4A 검증·승인이 끝난 뒤에 붙었으므로
  판정 대상이 아니다. 근거는 이 slice 의 r2·r3 실측이고 운영자 채택 2026-09-09. **rollback 은 in_scope 경로 한정이라 이 커밋을 되돌리지 않는다.**
  이 브랜치는 `main` 과 아직 병합되지 않았으므로 이 하네스 개선은 `main` 의 다른 레인에 아직 보이지 않는다 — 병합 시점에 전파된다.
