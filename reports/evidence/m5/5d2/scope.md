...
| `OPEN-5D2-BID-RATE-UPPER` | D-2B-8(`fraction > 1` 위반, 축 한정 없음) vs 표본 밴드 상한 1.5 — 2F 가 D-2B-8 문면을 축 한정으로 개정하거나 5C 코퍼스 실측으로 밴드를 1.0 으로. 5D-2 는 밴드만 |
...
| 2026-09-15 (verifier r1 뒤) | **① 관문 보강** — `observed_bid_rate.fraction` 은 wire `Rate` 파싱 관문(빈 문자열·비수치·비유한 → `BidRateUnparseable`; 파싱된 값이 `policy.bid_ratio.plausible` 밴드(0.5~1.5, 5D 승인) 밖 → `BidRateOutOfBand`)을 **거부 사유**로. **충돌 등재(verifier r2 N-1 — `OPEN-5D2-BID-RATE-UPPER`)**: D-2B-8 원문은 「`Rate.fraction` 이 1 초과면 계약 위반(`INVALID_REQUEST`)」로 **축 한정이 없다**. 반면 legacy 표본 투찰비 밴드는 0.5~**1.5**(5D 승인 `bid_ratio.plausible_max`, data-dictionary B5). 둘이 어긋난다: wire 규칙대로면 표본 `observed_bid_rate > 1` 은 서버 요청 검증(5E)에서 `INVALID_REQUEST` 로 요청 전체가 거부되고 밴드 상한 1.5 는 도달 불가; 밴드대로면 D-2B-8 이 표본 축에서 무력화. **5D-2 는 밴드 하나만 본다**(표본 관문에 `> 1` 을 두 번째 상한으로 두지 않음 — 어느 쪽이 정본이든 교집합만 통과하므로 fail-closed). 정본 결정은 2F(계약)·5E(요청 검증) 에서 — 후보 (a) D-2B-8 을 「대상 공고·응답 후보율 축」으로 한정 개정 + 표본 축은 밴드 (b) 밴드 상한을 1.0 으로 낮춤(legacy 표본 중 100% 초과 투찰 실존 여부 5C 코퍼스에서 확인) — `Decimal()` 예외가 `serve_bid_rates` 밖으로 새는 경로 0. `reserve_prices` 의 각 `Money` 는 `amount_won > 0` 만이 아니라 **5B `Money` 성분 규칙 넷**(amount > 0·currency KRW·basis BASE_AMOUNT·provenance ≠ UNSPECIFIED; vat 는 5B 도 미판정)으로 → 위반 `SampleRejected(ReservePriceInvalid)`. `award_rate`(optional) 도 있으면 같은 파싱 관문. **③ 문면 정정** — `ratio_sample_count == observation_count` 가 파이프라인상 항상 성립해 `TOO_FEW_RATIO_SAMPLES` 는 `distribution_availability` 단위 술어로만 도달(알려진 제한 2 — 정합). **⑥ 문면 정정** — golden 011 `verified_paths` 에 `posteriorStd` 없음(fixture 정본 우선). golden 011 test docstring 에 「서빙 경로가 아니라 커널 직접 호출」 명시(계약 갱신 623baa1 ③) | verifier r1 F-1(high: `InvalidOperation` 탈출 — 저장소에서 wire `Rate.fraction` 을 파싱하는 유일 자리, test 0)·F-2(medium: `reserve_prices` basis/currency 미검증)·F-3~F-8(low) |
| 2026-09-15 (verifier r2 뒤) | **① 문면 정정** — `fb4e001` 의 「`> 1` 거부」 삭제 + **`OPEN-5D2-BID-RATE-UPPER` 신설**(D-2B-8 wire 규칙 vs 표본 밴드 1.5 충돌 — 정본 결정은 2F/5E; 5D-2 는 밴드만, 교집합 통과라 fail-closed). 「5성분」→「성분 넷」. **N-2 등재** — `award_rate` 파싱 실패가 `BID_RATE_UNPARSEABLE` 를 재사용(어느 필드인지 미구별) → 알려진 제한(후속: `AWARD_RATE_UNPARSEABLE` 분리는 2F 소비 라운드에서). **N-3 등재** — `Decimal` 관용(공백·개행·`_`)은 전부 fail-closed 밴드 거부로 귀결, `parse_rate` docstring 에 관용 범위 명시 | verifier r2 N-1(medium — 승인 문서 둘 충돌: 계약 `> 1` vs checklist 「이중 정의 금지」; 코드는 checklist 쪽이 맞음)·N-2~N-4 |
