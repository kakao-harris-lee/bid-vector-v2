...
| (11) | `base_amount` basis 상이 | `_validate_base_amount`가 매 표본 basis·currency·provenance·amount_won 넷을 검증(5B 규칙 재현) |
...
1. **`OPEN-5D2-SAMPLE-SEGMENT`(계약 갱신 `623baa1`, 2026-09-15)** — M2 wire `CompetitionSample`
   ...
   `category_code` 추가)가 오면 조립기 시그니처 변경 없이 소비를 시작할 수 있다.
...
7. **origin(`BidRateOrigin`) 필드 미검증** — scope.md ①의 명시 관문 목록(7 사유)에
   `origin == BID_RATE_ORIGIN_OBSERVED` 검증이 없다. proto 주석은 "항상 OBSERVED"라고
   적지만 이 slice 는 그 불변식을 강제하지 않는다(Kotlin 송신측 책임으로 남김).

## 재검증 명령

`reports/evidence/m5/5d2/commands.md` S-1~S-9 전건. 결과: 전부 exit 0, 390 passed
