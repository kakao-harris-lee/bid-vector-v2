...
4. **`reports/evidence/m5/5e2/scope.md`는 착수 커밋 기준 단일 역적용** — 이 파일은
   `5939b33`에서 신규 생성됐고 이후 수정 이력이 없다(rollback.md 등 나머지 evidence
   파일은 위 1 에서 디렉터리째 삭제로 이미 처리). 신규 파일이라 「단일 역적용」은 곧
   `rm reports/evidence/m5/5e2/scope.md` 와 동치(1 에 포함) — 별도 명령 불필요.
