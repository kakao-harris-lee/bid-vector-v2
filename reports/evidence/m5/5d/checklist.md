...
- `OPEN-5D-GOLDEN`: 변경 없음(curator 병행 레인, `ml-kernel-*` 도착 대기 — M-3 은 그 통합 라운드에서 처리, 알려진 제한 11).
- `OPEN-5D-REAL-BOOSTER`: 변경 없음(5C artifact 도착 후).
- `OPEN-5D-DISTRIBUTION-ENGINE`(신설, 팀장 범위 판정 2026-09-12): K5·K6·K7 → 후보 3 을 잇는 분포 엔진 조립(legacy `distribution.py`)은 5D in_scope 에 없다 — ML-04 둘째 acceptance(수축 가중치가 응답 근거에)·golden 011·`segment_support` 폴백은 그 조립 slice 가 만족시킨다(알려진 제한 8·9·6). 처분은 운영자 결정 대기.
