...
`4ad2de7f44c5758df73e1a72244c17e7c2ea3966`(verifier r2 F-1 반영 — **파일 집합이 이전
라운드와 달라졌다**: `ml-engine/src/ml_engine/inference/rounding.py` 신규 — `scenario.py`
의 quantize 로직을 `policy.py`와 공유하기 위해 분리한 모듈. 아래 목록·되돌리기 명령·
임시 clone 재실측을 모두 이 파일 추가를 반영해 다시 산출했다).

```
$ git diff --name-status f5020aa982e6c5ade01f1660c0568dcd75c2fa7e..4ad2de7f44c5758df73e1a72244c17e7c2ea3966 \
...
diff 안에 포함되므로 같은 명령 한 번으로 걷힌다. `ml-engine/src/ml_engine/inference/
__init__.py`는 5D 착수 전 원본이 docstring 뿐이었으므로(base 존재) `git restore`로 전체
복원해도 안전하다.
...
## 임시 clone 실측(2026-09-12, verifier r2 F-1 수정 뒤 재실측 — `git clone --no-hardlinks`)

1. `/tmp/5d-rollback-check3`에 `git clone --no-hardlinks`(현재 브랜치 HEAD, 코드 마지막
   커밋 `4ad2de7` 포함).
2. 위 되돌리기 명령 실행 → `git status --short`가 신설 11개 삭제(정책 YAML 1 + inference
   9(`rounding.py` 포함) + registry/artifact.py 1) + `tests/inference`·`tests/registry`
   디렉터리 삭제(11 파일) + `reuse.md` 삭제 + `__init__.py`·`pyproject.toml` 수정만
   보임(세 라운드 편집이 한 번에 걷힘).
3. `git diff f5020aa982e6c5ade01f1660c0568dcd75c2fa7e -- ml-engine/src/ml_engine/inference
   ...
   | wc -l`(작업트리 diff, `--cached` 없음) → **0**(in_scope 전체 원복 확인, `rounding.py`
   포함 3 라운드 누적 편집 포함).
...
5. `uv run mypy --strict src/ml_engine` → `Success: no issues found in 18 source files`
   (5D 이전 5A+5B 파일 수와 일치 — `rounding.py`도 함께 걷혔으므로 27 이 아니라 18로
   돌아온다).
...
(M4 실측과 같은 갈래, 여기서는 발생하지 않았다: 임시 clone 재현이 exit 0 확인). 파일
집합이 라운드마다 달라질 수 있다는 것 자체가 이 문서를 매 라운드 「SHA 만 갱신」으로
끝낼 수 없는 이유다 — `rounding.py` 신설이 그 실례다(이번 라운드가 diff·rm 목록·
임시 clone 재실측을 전부 다시 산출한 근거).
