...
acceptance_commands:
  ...
  - "(cd ml-engine && uv run python -m pytest tests -q -m 'not legacy_parity')"                       # S-5 — 5A 게이트 + 5B + 5D 판정(legacy_parity 제외)
  - "(cd ml-engine && uv run python -m pytest tests -q -m legacy_parity)"                             # S-8 — legacy 출력 대조 **관측 전용**(exit 기록만, 판정 근거 아님)
...
| 2026-09-12 (verifier r2 뒤) | **⑧ 불변식 추가** — `quantize(clamp_min, bid_rate_digits) > 0`(quantize 전 값이 아니라 **quantize 뒤** 값으로 판정 — `clamp_min` 과 자릿수의 관계) → 위반 `PolicyRejected`. **(2b) 갱신** — `LoadedArtifact` 는 `_VerifiedBytes` 없이 생성 불가(타입)이나 모듈 private 이름을 import 해 넘기는 2줄 우회는 Python 한계(알려진 제한에 `InferencePolicy` 잔여와 같은 형식으로 등재). acceptance S-5/S-8 문면을 위와 같이 정정 | verifier r2 F-1(medium)·F-2~F-4(장부) |
