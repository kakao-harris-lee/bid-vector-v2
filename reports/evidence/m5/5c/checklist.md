...
| `write_artifact(trained, release_inputs) -> ArtifactBytes \| NameMismatch \| CanonicalizationRejected` | 연다 — 유일 조립 지점. `TrainedArtifact(...)` 직접 생성(`booster` 필드에 임의 `BoosterLike` 주입)으로 `feature_name()` 검증만 통과하면 임의 부스터 텍스트를 artifact 에 실을 수 있다(알려진 제한 ①·②, 5B 「booster 텍스트는 자기서술」과 같은 갈래) |
| `ReleaseInputs`·`derive_release_id` | 연다 — `ReleaseInputs.__post_init__`이 5필드 전부 비어있음/공백을 거부(방어선 있음), 값 자체의 진위(예: 가짜 `dataset_id`)는 검증하지 않는다(호출자 책임, D-5C-9 계약 그대로) |
