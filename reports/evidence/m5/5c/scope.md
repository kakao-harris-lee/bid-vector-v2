...
acceptance_commands:
  # 정본 = CI `ml-engine` job 전건(하네스 2026-09-12 규율 — 부분 게이트 금지). Kotlin job 은 소스 무접촉이라 착수 시 돌리지 않았으나,
  # **`leakPatternGate` 의 scanRoot 가 `reports/evidence` 라 evidence 를 고치는 라운드는 Kotlin `check` job 도 닿는다**(PR #13 CI 첫 실행 실측 —
  # 비밀값 스캔 절의 어휘 축어 인용 3건으로 붉음, verifier r5 L-9). evidence 편집 라운드는 `./gradlew --no-daemon check` 도 전건 돌린다(S-10).
  - "./gradlew --no-daemon check"                                                                      # S-10 — evidence 편집 라운드 한정(leakPatternGate 포함 Kotlin job 전건)
...
| 2026-09-15 (PR #13 code-reviewer 뒤) | **③ 문면 재확인** — 최소 학습 표본 게이트는 `admit_corpus` 직후가 아니라 **`build_row` 를 통과해 행렬에 실린 행 수**에 건다(승인 행 = 행렬 행; 두 자리가 같은 `_required_rows` 술어 공유, 경계 포함형). code-reviewer H-1(494 Missing/6 Present 에서 6행 성공 artifact) 시정. **⑪ 보강** — `load_training_policy` 는 `yaml.YAMLError` 도 `PolicyRejected(MALFORMED)` 로(H-2). **① 보강** — manifest 필드 타입 검사(bool 정확·int 는 bool 제외·문자열 비어 있지 않음, M-1) · `file://host/path` 거부, `file:///abs` 만(M-2). **알려진 제한 신설**(verifier r5 L-8): 재게이트 거부 `TrainingRejected(INSUFFICIENT_TRAINING_ROWS)` 는 계수만 나르고 사유별 분해(`BASE_AMOUNT` 결측 n 등)는 `RejectedRowAccounting` 이 성공 경로에만 실린다 — 5E `detail_code` 설계·5C-2 계약에서 `TrainingRejected` 에 회계 필드를 더할지 결정(`OPEN-5C-REJECT-ACCOUNTING`). acceptance 에 S-10(Kotlin `check`, evidence 편집 라운드 한정) 추가(L-9). `OPEN-5C-YAML-ERROR-5D`(5D `inference/policy.py` 동일 구멍 — 5D 레인) | PR #13 리뷰 · verifier r5 |
| 2026-09-13 (사용자 승인 뒤, 5D 병합 반영) | **D-5C-0·1·2·4·7·9 확정**(사용자 「승인 push, pr 진행」). **5D 가 로컬 `main`(`cefb19c`)에 먼저 병합됨** → 5C-1 은 그 위로 rebase(「병합 순서」 그대로). **D-5C-9b**: 5D read model(`registry/artifact.py` `_parse_release`)이 `release.artifact_checksum` 을 **비어 있지 않은 문자열로 요구하되 `expected` 와 등가성은 요구하지 않는다**(자기서술 필드, 5D 문면). 그래서 5C-1 writer 는 그 자리를 비우지 않고 **「그 필드를 빈 문자열로 둔 canonical bytes 의 sha256」**(두 단계 직렬화 — legacy manifest `payload_sha256` 과 같은 형태)을 쓴다. wire `ArtifactReference.release.artifact_checksum`(= sha256(최종 bytes), `ArtifactBytes.sha256`)과 **값이 다르다** — 같은 이름·다른 정의라 `OPEN-5C-ARTIFACT-CHECKSUM-PLACEMENT` 는 열린 채로(정합은 2F/5E 에서 이름을 가르거나 5D 가 필드를 선택으로), checklist 가 두 정의를 표로 남긴다. **`OPEN-5C-ARTIFACT-ROUNDTRIP` 은 이 rebase 에서 5C-1 이 닫는다**(나중 병합 쪽 규칙) — `write_artifact` → 5D `load_artifact(bytes, ModelReleaseRef)` 왕복 test 를 `tests/training/` 에. 5D 가 더한 ruff `BLE`·mypy `warn_unreachable`·`legacy_parity` 마커는 그대로 승계(5C-1 의 마커 중복 한 줄 제거). in_scope 추가: `ml-engine/tests/training/test_artifact_roundtrip.py` | 사용자 승인 · 5D 종결 2026-09-13 · 「레인 격리」 절 |
| 2026-09-12 (verifier r1 뒤) | in_scope 에 `ml-engine/uv.lock` 추가(계약 (b) 의 필연적 결과, 착수판 누락). **⑧·⑨ 문면 재확인** — `write_artifact` 는 `TrainedArtifact` 하나만 받고 `release` 넷은 `TrainedArtifact`(dataset_id·spec version·checksum·seed·code_version)에서만 파생한다 — 호출자 인자 경로 없음. **④ 문면 재확인** — OOF 누수 test 는 「모든 행」 단언(legacy 두 단언 그대로). **⑫ 문면 보강** — 양성 대조는 실제 `pyproject.toml` 의 계약 블록 존재를 test 가 단언하고(5A `test_forbidden_contract_forbids_bidvector_…` 패턴) 미니 프로젝트 fixture 는 그 계약을 **실제 pyproject 에서 읽어** 적용한다 | verifier r1 H-1·H-2·H-3·M-3 |
