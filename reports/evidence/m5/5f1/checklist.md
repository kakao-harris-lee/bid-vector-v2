...
4. **`ml-engine/tests/inference/test_scenario.py`(scope 밖) 편집 필요** — `center=1.0` fixture
   상수가 새 `clamp_max=1.0` 경계와 우연히 겹쳐 `test_candidates_have_fixed_order_and_labels`·
   `test_matches_legacy_scenario_bid_rates_formula` 둘이 RED 다. `src/**`나 산식 자체는 무관 —
   순수 fixture 값 충돌이다. **팀장에게 scope 확장 요청을 보내고 확인 대기 중**(2026-09-16,
   메시지 「5F-1 scope gap: test_scenario.py needs edit」) — 확인 전까지 이 파일은 무편집.
...
2. **`test_scenario.py` 두 test 가 미해결**(RED, scope 확장 대기) — 위 판단 4 참고. 이 slice 의
   in_scope 파일들은 전부 초록이지만, 전체 `pytest tests -q`(acceptance S-2)는 이 두 test 때문에
   아직 exit 1 이다(commands.md).
