...
rollback: in_scope 경로 한정. distribution.py·observations.py·facts.py 는 5D-2/5B 소유 파일 → 이 slice 자기 이력만 착수 경계 기준 단일 역적용(`git diff <base> -- <파일> | git apply -R`, 2026-09-16 규칙), tests 신설 삭제·기존 test 파일은 같은 역적용, evidence 는 남김. 임시 clone(`git clone --no-hardlinks`) 에서 실측 ①~⑥(⑥ = 되돌린 트리에서 ml-engine job 전건 + 루트 Kotlin `check` **전건** — 이 slice 는 Kotlin 소스를 만지지 않으므로 「닿은 게이트」가 없어 전건이 요구치, verifier r2 N-1 정정). 수정 라운드로 in_scope 파일이 늘면 실측 절(커밋 목록·시점·④⑤⑥ 수치)을 다시 낸다. 되돌리면 5D-2 종결 상태(global-only) 로 복귀 — 5D-2 알려진 제한 1·8 이 다시 유효.
```
...
| **주입점** `resolve_text_fact(is_allowed_missing_reason: Callable[[int], bool])` — F-10 되돌림이 허용 집합 데이터를 술어 인자로 바꿈(verifier r2 N-2, 2026-09-10 「무엇을 주입하는가」 고정 항목) | 주입되는 것: 결측 사유 허용 술어(`int → bool`). 허락하는 것: 수용 범위 확대 — 결과는 `Missing(raw)` 라 값을 나르지 않음. 허락하지 않는 것: `Present` 생성·정규화 우회(반환 경로 셋 고정). 표본 축 호출부(`distribution._resolve_segment`)는 private 이고 술어를 하드코딩 — 새 권한 없음 |
| `FactRejectionReason.MISSING_REASON_NOT_ALLOWED`(표본 축 판독 전용 사유) | 열거 확장, 계수 축만(verifier r1 F-5 등재). **요청 축 허용 집합을 닫는 상수(`ALL_MISSING_REASONS`)는 두지 않는다** — F-10 철회 행 참조, 요청 축은 열린 집합(`UNSPECIFIED` 만 거부) |
...
| 2026-09-16 (verifier r1 F-10, HIGH — **직전 행 철회**) | 표적 11 의 D-5D3-6 보정을 **철회**한다. D-5D3-6 원문 「요청 축 기존 집합 유지」가 그대로 계약이다 — 요청 축은 base 와 같이 `UNSPECIFIED` 만 거부하고 그 밖의 정수(enum 밖 포함)는 `Missing(raw)` 로 수용, 표본 축만 `{NOT_COLLECTED_YET}` 으로 좁힌다(D-5D3-2). 구현(`ALL_MISSING_REASONS` 로 요청 축을 좁힌 것)은 **되돌린다**. 되돌린 뒤 요청 축 `missing = 99` → `Missing(99)`(base 동일), 표본 축 `missing = 99` → 그 표본만 `SEGMENT_REASON_NOT_ALLOWED` 를 **단언하는 test** 를 둔다(현재 이름만 「any non-unspecified 허용」을 주장하는 test 를 실제 단언으로 고침). (2b) 표의 `ALL_MISSING_REASONS` 행은 구현이 그 상수를 남기지 않으면 함께 삭제 | verifier 실측 넷: ① 요청 축 `Missing(raw)` 의 raw 는 소비처가 없어 수용해도 얻는 권한 0 — fail-closed 논거가 서지 않는다 ② 거부는 표본 하나가 아니라 **요청 전체**를 `Unmeasurable(FEATURE_ABSENT)` 로 죽인다(네 fact 축 공유 헬퍼라 `base_amount` 까지 파급) ③ 저장소 관례(`observations.py` 미인식 라벨은 비-CLEAN 강등, 요청 생존)와 어긋남 ④ enum 값 추가는 `contract-policy` 상 breaking 이 아니라 다음 additive 라운드에서 구버전 엔진이 전 요청을 거부 — 팀장의 보정이 전방 호환을 놓쳤다 |
| 2026-09-16 (verifier r2 `ready-for-review`, N-1·N-2 장부층) | `rollback:` 줄의 ⑥ 을 `leakPatternGate` 단독에서 루트 `check` **전건**으로(evidence-pack 2026-09-12 — 닿은 게이트가 없으면 전건) · (2b) 표에 F-10 되돌림이 만든 주입점(허용 술어 인자) 행 추가. 코드 무변경 | F-3 과 같은 클래스의 세 번째 자리(팀장 계약 문면) · 2026-09-10 (2b) 라운드별 갱신 규칙 |
| 2026-09-17 (3H-2 D-3H2-3, `ml-implementer` 레인) | **D-5D3-2 확장**: 표본 축(`agency_id`/`category_code`) 허용 결측 사유 집합을 `{NOT_COLLECTED_YET}` 에서 `{NOT_COLLECTED_YET, UNKNOWN}` 으로. `NOT_APPLICABLE`·`UNSPECIFIED`·미지 정수는 그대로 `SampleRejected(SEGMENT_REASON_NOT_ALLOWED)`. `distribution.py::_is_segment_missing_reason_allowed`(`_ALLOWED_SEGMENT_MISSING_REASONS`)와 그 문면·`observations.py` 문면을 정정. 두 번째 판독기를 두지 않는다(D-5D3-6 불변) — `resolve_text_fact` 술어 인자만 넓힌다. D-5D3-5 회귀(표본 축 전부 결측인 요청은 5D-2 결과와 비트 동일)를 `NOT_COLLECTED_YET`·`UNKNOWN` 둘로 파라미터화해 유지 확인 | 3H-1 뒤 수요기관 코드가 있어도 Kotlin 표본 축이 `UNKNOWN`(수집했으나 원천에 없음)을 실을 수 있는데, 닫힌 집합이 `NOT_COLLECTED_YET`만 허용해 그 표본이 거부되면 공종·전역 계층 관측에서도 탈락한다(`OPEN-3H-SAMPLE-MISSING-REASON`) — 거부가 죽이는 것은 표본 하나가 아니라 그 표본의 계층 기여 전부 |
