...
| 일자 | 갱신 | 사유 |
| --- | --- | --- |
| 2026-09-11 (r1 뒤, 2) | S-7 문면 보강 — `reuse_provenance_check.py` 는 **양방향**: (ㄱ) 이식 모듈 docstring 의 `Reuse:` 포인터 ↔ `reuse.md` 행 일치 (ㄴ) **`reuse.md` 의 모든 행에 대해 그 모듈에 포인터가 있어야 함**(없으면 exit 1). 양성 표본 둘 — 값 어긋남(`reuse-mismatch.md`)은 S-7 명령·CI step 이 직접 부르고, 포인터 없음 표본은 S-5 게이트 test 안에서 실행된다(verifier r2 N-2 정정) | verifier r1 F-3 — docstring 포인터를 지우면 `reuse.md` 행이 남아 있어도 S-7 이 초록(역방향 대조 부재) |
