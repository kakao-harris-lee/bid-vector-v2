...
| `gate_outcome(scores, policy) -> GateOutcome` | 연다 — 유일 **결과 타입** 판정 진입점, 임계 인자 없음. `Promotable` 은 이 결과의 `Passed` 에서만 |
| `passes_gate(scores, policy) -> bool`(r1 뒤 신설) | 연다 — 판정 술어 단일 출처(`gate_outcome` 과 안정성 sweep 이 **같은 함수 객체**를 호출). bool 만 내므로 `Passed`/`Promotable` 을 만들 수 없다 — 없던 권한 아님 |
| `DroppedRowCount`·`WindowResult.dropped_rows`(r1 뒤 신설) | 연다(읽기) — buildability 필터가 버린 행의 사유별 계수. **제외된 창에도** 실린다(r2 M-1r) |
| `_trial_outcome`(비공개, r1 H-3) | 안정성 sweep 내부 전용 — `__all__` 부재, import 불가가 test |
