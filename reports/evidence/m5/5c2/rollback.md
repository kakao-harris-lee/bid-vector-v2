...
**2026-09-16 정정(verifier r2 L-3r)** — `9853ae9`(「계약 갱신 — holdout 실행기
파일 분리 in_scope 등재」)를 구현 레인 커밋으로 분류한 근거는 **git 커밋 author
가 아니다**(이 저장소는 사람 계정 하나(`harris.lee`)로 모든 레인이 커밋해 author
필드로 레인을 못 가른다, verifier 지적과 동일 관측). 근거는 **내용**이다 —
`30b42d6`(팀장 착수, 「사용자 「5C-2 착수」」 근거로 §1 표·in/out scope 원안을
고정)과 달리 `9853ae9`는 구현 진행 중 발견한 사실(500줄 래칫 초과로 파일을
나눴다는 것)을 계약에 등재하는 커밋이고, 그 등재 자체가 5C-2 구현 작업의
일부다(설계 검토 문서가 계획한 단일 파일 계획과 실제 구현 결과가 갈린 자리를
그 자리에서 즉시 문서화한 것 — 「제안 결과와 그 사유를 즉시 기록한다」는 구현
레인 규율에 따른 자기 보고). `975d4d8`·`b6e0049`도 같은 성격(verifier r1·r2
리뷰 finding 을 반영한 계약 갱신)이다. 팀장 확인이 필요한 것은 「이 셋이
구현 레인 소유인가」가 아니라(내용으로 이미 판별됨) 3파일 분리 자체의 최종
승인 여부이고, 그것은 checklist.md 「계약과 어긋나 판단이 필요했던 자리」에서
별도로 팀장 검토 대상으로 남아 있다.
...
**하네스 경로는 되돌리지 않는다**(evidence-pack 규격) — `CLAUDE.md`·`.claude/**` 는 위 rm·restore
목록에 없고(전수 대조, verifier r3 §11 (b) 실측: rollback 전후 blob 해시 동일), 승인 문서의 하네스 레인
편집도 되돌림 대상이 아니다. 그런데 위 단일 역적용은 scope.md 를 착수판으로 돌리므로 **「하네스
레인 변경」 절의 등재까지 지운다**(되돌린 트리의 절이 「착수 시 없음」인데 range 에는 하네스 커밋이
남는다 — verifier r3 §11 LOW ①). 그래서 역적용 **뒤** 한 단계를 더 둔다:

```
# scope.md 역적용 뒤 — 하네스 레인 절을 현재 값으로 재등재한다(등재 대상은 이 명령의 출력)
git log --oneline c669a71..HEAD -- CLAUDE.md .claude/
```

출력의 각 커밋(2026-09-16 현재 `7f11038`·`eb327da`·`3df08f3` 셋)을 scope.md 「하네스 레인 변경 (상시 절)」에
해시·제목·경로·목적 한 줄씩 다시 적는다(형식은 현재 scope.md 의 그 절). 확인: 재등재 뒤 그 절의 건수 =
위 명령의 행 수. 이 단계가 커밋별 hunk 격리 대비 단일 역적용이 잃는 유일한 것이다(evidence-pack 규격 2026-09-16).
