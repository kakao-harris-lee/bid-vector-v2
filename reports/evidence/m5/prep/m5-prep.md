...
| **5D inference kernels** | model predict adapter(LightGBM, 분해 후 이식) · 순수 커널(추첨 분포·수축·성숙도 계산) · KDE density/optimization 이식은 **D-M5-9 결정 뒤**(win-proxy 체인 전용 — milestone 문면 항목 개정 동반) · 최소 표본·singular·NaN/Infinity · objective 별 후보·diagnostics · 업무 하한/자격/최종 결정 없음 | 커널은 독립. 출력 어휘(후보 3·성분 셋·`Unmeasurable` 두 사유 구별)는 2B 와 **같은 형태**로 두되 wire 매핑은 5E | `5d/scope.md`(초안 있음) |
...
| **D-M5-5** | **커널 이식 순서**(5D) | (a) **의존 없는 것부터**: `reserve_draw_distribution` → `assessment_shrinkage` → `award_rate_features`(5B — **2026-09-12 개정**: `reliable_base` enum 이식 없음, 분모 어휘는 wire `BaseAmountProvenanceLabel` 5값, D-5B-2) → `settlement_maturity`(계산만) → LightGBM predict adapter(분해 후) → (D-M5-9 결정 뒤) `award_margin_distribution`(KDE)·`award_landing_curve_builders` → (D-M5-8 결정 뒤) `award_landing_distribution`·`award_landing_curve` (b) 응답 경로 순 | **(a)** — 각 커널이 자기 property test 와 함께 들어오고 상위가 하위를 참조. **조사 (a) 실측**: KDE 와 곡선 빌더는 win-proxy 체인 안에서만 소비되므로 D-M5-9(8개 목록 개정) 뒤로, landing 둘은 D-M5-8 뒤로 밀린다 |
...
| `OPEN-ADR-10`(닫힘) | 재활용 출처 기록 = (d) 조합 — 5A 가 모듈 docstring 포인터 + `reports/evidence/m5/<slice>/reuse.md`, 그리고 **ADR 0009 D-6 의 두 자리 대조 검사**(`reuse_provenance_check.py`, 5A ⑦ — 첫 이식 slice 가 형태를 정한다, CI 안) |
...
- 금지 경로: M2 in_scope 전부 + `capability-map.md`·`data-dictionary.md`. 착수 시 `base_sha` 는 40자로(evidence-pack 규격 — 초안의 7자 앵커는 임시).
- **Phase 2.5**: 5A(import 경계 게이트·크기/복잡도 래칫)·5B(feature schema 계약)·5D(fail-closed 가드·결과 타입 불변식)는 설계 검토 대상 — scope 의 위협 모델 절은 검토 입력.
