...
in_scope:
  ...
  - ml-engine/pyproject.toml                                 # (a) import-linter forbidden 계약 한 블록 — `yaml` 은 `ml_engine.registry.policy` + 자기 `except yaml.YAMLError` 절을 가진 기존 로더 셋(training·evaluation·serving policy)만 import(ignore_imports 넷 — 계약 갱신 (4) 로 문면 정정), 나머지 ml_engine 전부 금지. D-5E3-6, 계약 갱신 (3). 다른 블록·의존성 무편집
...
**방어하지 않는다**: 창 하나 안의 LightGBM 학습 중단(라이브러리 경계 — 창 단위가 최소 단위) · 정책 값 결정 · 로더 밖의 다른 `yaml.safe_load` 호출(grep 으로 0 임을 실측해 등재) · **동적 import**(`importlib.import_module("yaml")` 류 — import 문이 아니라 import-linter 계약 사정 밖. 정책 로더로는 평범한 스타일이 아니고, 이 저장소의 게이트 전부가 정적 분석이므로 같은 경계. 계약 갱신 (4), verifier r2 LOW ②).
**우회 후보**: (1) 새 로더가 `load_policy` 를 안 쓰고 `yaml.safe_load` 직접 호출 → 게이트 test 의 기계 수집 대상이 아니면 못 잡는다 → ~~grep 게이트로 고정~~ **D-5E3-6 ①④ 로 대체(계약 갱신 (5))**: import-linter forbidden `yaml` 계약(S-4) + 예외 모듈의 yaml 참조를 AST 로 `YAMLError` 하나로 고정 — grep 술어는 r1 HIGH-1 을 만든 자리라 되살리지 않는다 (2) `should_stop` 을 안 넘기는 호출자 → 기본 거짓이라 기존 동작(취소 불가) — 조립 근이 넘기는지 test (3) 창 루프 확인을 학습 **뒤**로 옮기는 변이 → trainer 호출 수 test 가 붉어져야 함 (4) `HoldoutCancelled` 를 pipeline 이 `PipelineFailed` 로 매핑 → 상태 FAILED 위장 → test 가 CANCELLED 고정 (5) 게이트 test 의 로더 열거가 손 목록 → 새 로더 누락 → 기계 수집(`pkgutil`·`load_*` 이름 규약) + ~~수집 수 ≥ 4 단언~~ **D-5E3-6 ② 로 대체(계약 갱신 (5))**: 정책 모듈마다 로더 ≥1 · 수집 집합 == 열거 집합(하한 단언은 로더가 늘면 안 보이는 술어라 폐기). **(1)∩(5) 잔여(verifier r3 MEDIUM-2, 미차단)**: 예외 모듈 안에서 이미 import 된 `yaml` 을 **한 번 더 별칭으로** import 해 비-`load_` 이름으로 부르면 AST test 가 이름 `yaml` 에 걸려 놓친다 — 평범한 저자는 그 파일의 `yaml.safe_load` 를 쓰므로(그러면 잡힘) 알려진 제한으로 등재하고, 구조적 해소는 **세 로더의 죽은 `except yaml.YAMLError` 절 제거**(그러면 ignore 가 뿌리 한 줄로 수렴하고 AST 검사 대상·MEDIUM-2 도달 경로가 함께 사라진다 — out_of_scope 라 다음에 그 파일을 만지는 slice 의 후속 소폭). 대증 처방(AST test 가 `ast.Import` 바인딩 이름 집합으로 재기)은 운영자가 이번 PR 에서 원하면 한 줄.
...
| 2026-09-16 승인 전 일괄(4) | D-5E3-6 ① 문면 정정(`ignore_imports` 한 줄 → **넷**, 구현이 out_of_scope 에 강제된 것을 계약이 뒤따름 — verifier r2 LOW ①) · **D-5E3-6 ④ 신설**(예외 세 모듈의 yaml 참조는 `YAMLError` 뿐임을 AST 로 단언하는 test 하나 — r2 MEDIUM-1 교집합 폐쇄) · 위협 모델 「방어하지 않는다」에 동적 import 경계 등재(r2 LOW ②) · in_scope 주석 정정 | verifier r2 ready-for-review(MEDIUM 1·LOW 2) — 게이트 술어(test 추가)라 표적 재검증 1회 더 |
| 2026-09-16 종결 전 문면(5) | 우회 후보 (1)·(5) 의 낡은 처방(grep 게이트·수집 수 ≥ 4)을 D-5E3-6 현행으로 정정(verifier r3 LOW ②) · **(1)∩(5) 잔여 = r3 MEDIUM-2 등재**(예외 모듈 안 yaml 이중 별칭 import — 미차단, 알려진 제한 + 구조적 해소는 죽은 except 절 제거 후속 소폭) · rollback ⑥ 갈음 논거는 「트리 동일성」으로 정정(r3 LOW ①, rollback.md 는 구현 레인 문서 커밋) | verifier r3 ready-for-review(MEDIUM 1·LOW 2, MEDIUM-1 닫힘 실측) — 코드 무변경, 문서만 |
