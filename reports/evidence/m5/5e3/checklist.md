...
| D-5E3-6(계약 갱신 (3), 수정 라운드 1) | `ml-engine/pyproject.toml` import-linter forbidden 계약 — `ml_engine` 전체에서 `yaml` import 금지, `ignore_imports`는 `registry.policy`(뿌리, 실제 파싱)와 기존 세 소비 로더(`training`·`evaluation`·`serving`의 `policy.py`, out_of_scope 유지되는 죽은 except 절의 무해 참조) 넷. 게이트 test 수집을 이름 규약(모듈에 정의된 `load_` 접두, 뿌리만 이름 제외)으로 재작성 — 반환 타입 주석을 보지 않는다. 텍스트 스캔 test는 삭제(import-linter가 대체) |
