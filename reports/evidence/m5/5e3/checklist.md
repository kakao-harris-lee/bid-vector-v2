...
| D-5E3-5 | `holdout.py` 편집은 추가만(새 키워드 인자·새 결과 타입·루프 앞 확인 한 줄) — 산식·창 계획·보고서 조립 함수(`_assemble_report`·`_unaccounted_row_count`·`_unaccounted_or_reject`) 무변경, 5C-2 기존 test 22 case 전부 무편집 통과(`git diff` 로 확인 — 새 test 3 개만 추가, 기존 함수 시그니처 diff 없음). design ratchet 재통과를 위해 `_record_window_outcome` 헬퍼로 창 결과 분류 블록을 옮겼으나 그 블록 자체의 로직은 변경 없음(reuse.md 참고) |

## 우회 (1)~(5) ↔ test 대응표

| # | 우회 | 대응 test |
| --- | --- | --- |
| (1) | 새 로더가 `load_policy`를 안 쓰고 `yaml.safe_load`/`yaml.load`를 직접 호출 | `tests/gates/test_policy_loaders_fail_closed.py::test_only_registry_policy_calls_yaml_safe_load_directly`(소스 스캔, `registry/policy.py` 밖에서 직접 호출 0을 매 실행마다 재확인) |
...
| (5) | 게이트 test 의 로더 열거가 손 목록이라 새 로더 누락 | `tests/gates/test_policy_loaders_fail_closed.py::_iter_fail_closed_loaders`가 `pkgutil.walk_packages`로 `ml_engine` 아래 `policy` 서브모듈을 전수 순회하고, `load_*` 이름 규약 + 반환 타입에 `Rejected` 포함이라는 두 조건으로 대상을 고른다(손 목록 없음). `test_at_least_four_fail_closed_policy_loaders_are_collected`가 수집 수 ≥4 를 고정하고, `test_root_loader_itself_is_not_collected_as_fail_closed`가 뿌리(`registry.policy.load_policy`, 예외를 던지는 계약)가 이 집합에 섞이지 않음을 확인한다 |
...
3. **`_evaluate_windows`가 `_record_window_outcome` 헬퍼로 분리됐다**(design ratchet
   50줄 재통과 목적) — 순수 이동이라 동작 변화는 없지만, `holdout.py`가 D-5E3-5
   문면(「추가만」)과 문자 그대로 일치하지는 않는다. 5C-2 golden·mutation test 무편집
   통과(commands.md S-5·checklist D-5E3-5)가 동작 불변의 증거다.
