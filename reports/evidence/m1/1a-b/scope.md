...
| **D-6** (Phase 3 중 신설, 2026-09-05 — 팀장 판단, 운영자 사후 확인 요청) | 타입 멤버 축의 대상 source set | **D-5 가 실제로 트리거됐다**: Phase 2 가 도메인만 실측한 사이 `build-logic/src/test` 의 `SourceReferencesTest`(`@Test` 35, 471줄)가 멤버 30 을 넘겼다 — mixin 팽창이 아니라 test case 가 많은 정상 스위트다. 선택지 (a) 임계 상향 (b) test source set 면제 (c) 클래스 분할 (d) 예외 등재 중 **(b)** — 이 축의 근거(ADR 0007 §1.3)는 **프로덕션 타입의 합성 팽창**이고 테스트 클래스는 case 수만큼 자연히 커진다. 정책 키 `limit.type.members.source-sets=main`(기본값 없음 — 키 부재는 실패), 리포트는 제외 source set 의 수치도 관찰로 남긴다. **파일 500·함수 50 축은 test 에 그대로 적용**(1A 관례 불변, 면제는 멤버 축 하나). 임계 30 은 불변(D-5). (c) 는 우회 취급 위험, (d) 는 큰 테스트 클래스마다 반복돼 구조가 아니다 |
