...
| #4 | `DerivationRecord.inputs`가 `AmountRecord` 값 스냅샷만 담아, 값·`provenance`가 같은 서로 다른 입력 fact 를 구분 못 한다 — `OPEN-DIC-08`(B11) 이 요구하는 "입력 fact 의 안정적 참조" 요건 미충족 | **미착수 — team-lead 지시대로 정지하고 결정 요청(`scope.md` `OPEN-1B-STABLE-FACT-REF` 신설 등재).** B11 해소문이 "참조·version 을 나르는 정확한 필드 형태는 M1 1B 구현이 정한다"며 **형태**는 위임했으나, **그 참조가 가리키는 identity 자체**(어디서 얻는 안정적 참조인가)는 위임 밖이다. `data-dictionary.md` §12.2 를 읽었다 — 「식별자」갈래에 열거된 아홉(`limitGroupNo`·`licenseRegionCode`·`noticeRevision`·`deliveryKey`·`observationKey`·`key`·`sourceKey`·`modelArtifactId`·`inputSnapshotHash`·`policyVersion`) 전부 **다른 capability 소유**의 기존 식별자이지 "`AmountRecord`를 가리키는 안정적 참조"로 정의된 필드가 없다. `inputSnapshotHash`는 `DecisionProvenance`(B11 이 불채택한 선택지 ①의 carrier) 소유라 재사용하면 불채택 설계를 끌어오는 셈이 된다. `Money`에 여섯째 성분으로 참조를 얹는 것도 `ADR 0002` D-1("다섯 중 어느 하나라도 없는 금액은 `Money`가 아니다")이 막는다. content-addressed 해시 등 새 identity 체계를 짓는 것은 여러 capability 에 걸치는 cross-cutting 결정이라 이 slice 권한 밖이라고 판단했다. 코드·문서(등재 제외) 변경 없음 |
