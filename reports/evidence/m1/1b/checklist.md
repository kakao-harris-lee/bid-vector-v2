...
| L-11 | `Money` 상위 타입에 이항 연산이 들어가면 basis 차단이 무너짐 | **지켰다 — 단, 중간에 한 번 회귀했다가 verifier r4 H-1 로 잡혀 재수정됐다.** `Money` 는 여전히 `Comparable`·이항 연산을 구현하지 않는다. 각 금액 타입도 자기 축의 공개 `Comparable<Self>`를 구현하지 않는다(`won`만 비교해 `VAT`를 안 보던 결함(Codex #2)을 고치며 없앴다). **정정(verifier r4 H-1)**: 이 행이 이전에 "basis 교차 비교 차단은 여전히 지켜진다 — 제네릭 `T : Money`가 같은 타입만 받는다"고 적었던 것은 **거짓이었다** — Kotlin 은 `T`를 두 인자의 최소 상위 타입(LUB)으로 추론해 `compareKnownVat(base, estimated)`가 `T = Money`로 실제 컴파일됐다(회귀, `MoneyTest`의 옛 test가 같은 타입 쌍만 불러 놓쳤다). **재수정**: 제네릭 하나를 타입별 오버로드 여섯(`compareKnownVat(BaseAmount, BaseAmount)` … `AwardAmount`)으로 되돌리고 공통 로직은 `private fun compareSameType`에 위임했다 — LUB 추론이 일어날 자리 자체가 없다. `CompileFailureHarnessTest`의 fixture 11(basis 교차 쌍 음성 + 같은 타입 양성 + 오타 변이)이 모듈 밖에서 이 경계를 실측한다 |
...
### Codex 1차 리뷰 수정 라운드 — 커밋 넷(finding 별)

| 커밋 | finding |
| --- | --- |
| `0ffd32c` | #1 — 산술·파생 성공 경계(`roundedWith`·`divideForRate`·`sumOfBaseAmounts`)에 `hasDeclaredProvenance` 전건을 건다. 새 `ReasonCode.UNDECLARED_PROVENANCE` |
| `480eb57` | #2 — 여섯 `Money` 타입의 공개 `Comparable<Self>` 를 제거하고 `sameKnownVat` 전건을 건 `compareKnownVat(...)` 로 대체(반환 타입은 `Fact<Int>` — 달리한 결정) |
| `aa64f02` | #3 — `roundedWith` 에 `floor: BigDecimal? = null` 을 추가하고 미달 시 새 `ReasonCode.ROUNDED_BELOW_FLOOR` 로 `Unmeasurable`(clamp 대신 사유 있는 실패 — evidence 근거 기록). `ArithmeticTest.kt` 500줄 초과로 `UndeclaredProvenanceTest.kt` 신설(주제 분리) |
| `f6363b1` | #4 — `DerivationRecord` 안정적 참조 형태가 문면에 없어 구현을 멈추고 `scope.md` `OPEN-1B-STABLE-FACT-REF` 로 등재만 함(team-lead 지시대로 정지) **(decision 17 이전 문면 — "안정적 참조 형태"는 B11 원안(선택지 ②)의 요구다. decision 17 이 그 요구 자체를 접었다 — 아래 `50e77d9` 행·`scope.md` decision 17 참고)** |
| `50e77d9` | #4 후속 — 운영자 결정 2026-09-04(decision 17, B11 을 ①의 변형으로 조정) 구현. `DerivationRecord`에서 `inputs: List<AmountRecord>` 제거, `policyVersion`만 남김. 되짚기는 `DecisionProvenance`(§4.1) 소유로 정본 이관(`data-dictionary.md` §9·§11.1, 세션 모델 커밋 `e972930`) |

### Codex 1차 발견 — #1·#2·#3·#4 처리 결과
...
| #4 | `DerivationRecord.inputs`가 `AmountRecord` 값 스냅샷만 담아, 값·`provenance`가 같은 서로 다른 입력 fact 를 구분 못 한다 — `OPEN-DIC-08`(B11) 이 요구하는 "입력 fact 의 안정적 참조" 요건 미충족 | **닫혔다 — 운영자 결정 2026-09-04(decision 17) 구현.** B11(선택지 ②)이 요구한 「입력 fact 의 안정적 참조」는 가리킬 identity 가 이 문서 안에 없어(`data-dictionary.md` §12.2 식별자 아홉 전부 다른 capability 소유, `inputSnapshotHash`는 불채택 선택지 ①의 carrier), 새 identity 체계를 짓는 대신 **①의 변형으로 조정**됐다 — 파생값은 계산 정책 version만 싣고, 되짚기는 판정의 `DecisionProvenance`(§4.1)가 소유한다(`data-dictionary.md` §9·§11.1, 세션 모델 커밋 `e972930`). 구현(`shared-kernel/**`, 커밋 `50e77d9`): `DerivationRecord`에서 `inputs` 제거(`policyVersion`만 유지) · `UnroundedBidAmount`의 죽은 `baseInput: AmountRecord` 파라미터·`times` 연산자의 `export()` 호출 제거 · `asRate`의 `inputs` 매개변수 및 `assessmentRateAgainst`·`awardRateAgainst`·`bidRateAgainst` 호출부의 `listOf(export(), base.export())` 인자 제거 · `Derived`·`Measurement.Measured`의 `internal constructor`+`@ConsistentCopyVisibility`(H-3 처방)는 유지(값·정책 version 결속 보증은 이 결정과 별개로 필요). **RED**: `inputs` 필드만 먼저 지운 상태로 `:shared-kernel:compileKotlin`을 돌려 `MoneyArithmetic.kt`에서 컴파일 실패 실측 뒤 호출부를 고쳐 GREEN(`scope.md` 계약 갱신 절 참고). `ArithmeticTest.kt`의 B11 test 를 `derivedFrom.policyVersion` 대조로 바꿨다(파생 넷 중 성공 경로를 낼 수 있는 `roundedWith`·`bidRateAgainst` 둘을 덮는다). `AmountRecord` 자체는 `Money.export()`·`MoneyTest`의 소비처가 남아 있어 제거하지 않았다(grep 실측). `Carrier.kt`는 실측 결과 B11 되짚기 서술이 없어(H-3 KDoc은 조립·위조 방지 서술뿐) 편집하지 않았다 |

### verifier r4 수정 라운드 — 커밋 하나(H-1·M-1)

| 커밋 | finding |
| --- | --- |
| `f750c60` | H-1 — `compareKnownVat` 제네릭 하나가 LUB 추론으로 basis 교차 비교를 허용하던 회귀를 타입별 오버로드 여섯 + `private fun compareSameType`으로 되돌린다. M-1 — `compareSameType`에 `hasDeclaredProvenance` 전건을 걸어 비교 경로도 산술·파생과 같은 provenance 전건을 받는다(검사 순서: provenance 먼저, VAT 다음) |

### verifier r4 발견 — H-1·M-1·장부층 low 4 처리 결과

| # | 요지 | 처리 |
| --- | --- | --- |
| H-1 | Codex 1차 #2 가 만든 회귀 — 제네릭 `fun <T : Money> compareKnownVat(left: T, right: T)` 는 Kotlin 의 LUB 추론으로 `T = Money` 가 되어 `compareKnownVat(baseAmount, estimatedAmount)` 가 모듈 안팎에서 컴파일된다. `MoneyTest` 의 옛 test 는 같은 타입 쌍만 불러 공허하게 통과했다 | **닫혔다 — 팀장 결정대로 타입별 오버로드 여섯으로 되돌린다.** `compareKnownVat(BaseAmount, BaseAmount)`~`compareKnownVat(AwardAmount, AwardAmount)` 여섯을 선언하고 공통 로직은 `private fun compareSameType(left: Money, right: Money): Fact<Int>` 하나에 위임했다 — 오버로드 각각이 정확히 그 타입 하나만 받으므로 LUB 추론이 일어날 자리 자체가 없다. **함께 처리**: `CompileFailureHarnessTest` 에 fixture 11(negative — basis 교차 쌍 호출, "type mismatch" / positive — 같은 basis 쌍 / mutant — 오타 함수명, "type mismatch" 미포함 확인) 신설 — **모듈 밖(별도 컴파일 단위)** 실측. `MoneyTest` 의 공허했던 "서로 다른 basis 금액은 같은 타입이 아니라 컴파일이 안 된다" test 를 fixture 11 을 가리키는 서술로 정정. `Money.kt` 의 `compareKnownVat` KDoc·`checklist.md` L-11 행(위)의 "제네릭 `T : Money`가 같은 타입만 받는다"는 거짓 문장을 정정. **회귀 재현 실측**: 되돌리기 전 코드(제네릭 버전)로 fixture 11 을 돌려 `expected:<COMPILATION_ERROR> but was:<OK>` 로 실패함을 확인한 뒤(회귀를 실제로 잡는 fixture임을 증명), 수정 코드로 되돌려 GREEN 확인 |
| M-1 | `hasDeclaredProvenance` 의 KDoc 이 "산술·파생 성공 경계 전건의 유일한 자리"라 선언하는데 Codex 1차 #2 가 신설한 `compareKnownVat` 는 그 전건을 부르지 않아, 양쪽 다(또는 한쪽만) `Provenance.Undeclared` 여도 vat 가 같은 known 이면 성공(`Fact.Known`)이 났다 | **닫혔다 — 팀장 결정대로 ① 전건 추가.** `compareSameType` 에 `hasDeclaredProvenance` 전건을 걸어 어느 한쪽이라도 `Undeclared` 면 `Fact.Absent(UNDECLARED_PROVENANCE)` 를 먼저 낸다 — 검사 순서는 `sumOfBaseAmounts` 의 `accumulate` 와 같게 **provenance 먼저, VAT 다음**(둘 다 실패해도 `UNDECLARED_PROVENANCE`). `hasDeclaredProvenance` 를 `MoneyArithmetic.kt` 에서 `private`→`internal` 로 열어 `Money.kt` 가 재사용한다(같은 모듈 다른 파일, 중복 선언 없음). `hasDeclaredProvenance` KDoc 을 "산술·파생·**비교** 성공 경계 전건의 유일한 자리"로 정정해 그 계약이 실제로 참이 되게 했다. **test 셋**: 양쪽 Undeclared(같은 known vat) · 한쪽만 Undeclared(양방향) · Undeclared+vat 불일치 동시 성립 시 순서 확인(UNDECLARED_PROVENANCE 가 남) — RED 확인(전건 없이 실행해 세 test 전부 실패, `Known`/`VAT_TREATMENT_MISMATCH` 를 냄) 뒤 구현, GREEN 확인. **mutant 실측**: `compareSameType` 의 provenance 분기를 `false ->`로 죽인 로컬 mutant 로 `:shared-kernel:test`(MoneyTest) 를 돌려 위 세 test 전부 실패함을 확인한 뒤 원복(`git diff` 빈 출력 확인) |
| L-4(장부) | `scope.md` 「하네스 레인 변경」 절이 「없음」인데 실제로는 `551b603`·`f95feac` 둘이 range 에 있고, `commands.md` 는 그 절에 "이미 등재됨"이라는 거짓 참조를 갖고 있었다 | **닫혔다.** `scope.md` 「하네스 레인 변경」 절을 SHA·경로·목적 표로 갱신(`f95feac`·`551b603` 둘, 「slice 산출물이 아니며 in_scope 밖, 운영자 승인 하에 같은 range」 명시) — `551b603` 이 grep 패턴 밖의 in_scope 파일(`agent-workflow.md`·`milestone-0.md`는 완전 out, `milestone-1.md`는 1B 승격 절 밖 「선행 조건」·「Codex 독립 리뷰」 절)도 건드렸다는 사실을 함께 적었다. `commands.md` 의 거짓 참조를 「이전 문면이 낡아 있었다」는 사실로 정정 |
| L-2(장부) | `roundedWith` 의 `floor: BigDecimal? = null` 기본값 — 하한을 아는데 안 넘기는 호출부가 생기면 검사가 조용히 빠진다는 사실이 「알려진 제한」 표에 없었다(KDoc·checklist #3 행에는 있었다) | **닫혔다 — 아래 「verifier r4 low(L-2)」 신설 행으로 등재.** `roundedWith` 호출부 전수(`grep -rn "\.roundedWith("`)는 test 5파일뿐이고 production 호출부는 0개다(1B 는 도메인 값 타입만 낸다) — 지금은 실해가 없으나 하한을 아는 production 호출부가 생기는 순간 `floor` 를 빠뜨리면 검사가 조용히 빠진다는 것이 이 기본값의 opt-in 성질이다 |
| L-3(장부) | `checklist.md` 에 decision 17 이전 문면(파생값이 입력 fact 를 되짚는다)으로 읽히는 옛 서술 둘 — 커밋 제목 인용 1 · verifier r2 H-3 발견 요지 1 | **닫혔다.** 「Codex 1차 리뷰 수정 라운드 — 커밋 넷」표의 `f6363b1` 행과 「verifier r2 발견 — H-3」표의 H-3 행 옆에 각각 「(decision 17 이전 문면 — 이후 조정은 위 #4 행·`scope.md` decision 17 참고)」한 마디를 붙였다. 이력 자체는 지우지 않는다 |
| L-5(장부) | `scope.md`·`commands.md` 가 RED 실측을 `MoneyArithmetic.kt:114`·`:175` 로 적었는데 그 뒤 커밋(`aa64f02`·`50e77d9`)이 같은 파일을 편집해 좌표가 낡았다 | **닫혔다.** 두 문서의 좌표를 「`measured`(`DerivationRecord(inputs = …)` 호출 지점)」·「`asRate`(`DerivationRecord(inputs, …)` 호출 지점)」인용문 형태로 바꿨다(`evidence-pack` 「낡는 좌표」 규격) |

### verifier r4 low(L-2) — 「알려진 제한」 신설 행

| # | 요지 | 처리 |
| --- | --- | --- |
| r4-L-2 | `roundedWith(policy, floor: BigDecimal? = null)` — `floor` 를 아는 호출부가 인자를 빠뜨리면 하한 검사가 조용히 꺼진다 | **등재만 한다 — 알려진 제한(닫을 필요 없음, verifier r4 판정).** 현재 production 호출부는 0개(`grep -rn "\.roundedWith(" shared-kernel/src/main` — 매치 없음, test 5파일만 호출)라 지금 이 결함이 실제로 발현할 자리가 없다. 하한을 아는 production 호출부가 생기면 그 호출부가 `floor` 인자를 반드시 넘기는지 코드 리뷰로 확인해야 한다 — 이 slice 는 opt-in 기본값의 필요성(하한을 모르는 기존 호출부와의 호환, Codex 1차 #3)과 이 위험을 함께 KDoc·여기 양쪽에 남긴다 |
...
**완료 조건 — 2026-09-04 운영자 지시.** 코드 slice 는 Codex 2차 리뷰를 타지 않는다.
`CLAUDE.md`(운영자 지시 2026-09-04)·`milestone-1.md`(개정 커밋 `551b603`)가 코드 slice의
종결 조건을 「Codex approve + 사용자 승인」에서 **「verifier ready-for-review + 사용자
승인」**으로 정정했다 — 이 라운드(Codex 1차 finding #4 수정)도 그 조건을 따른다. 아래 표는
그 조건이 요구하는 산출물 충족 여부다.
...
| verifier `ready-for-review` | **충족.** verifier r5(2026-09-04, HEAD `a6d849f`, 코드 `f750c60`) — 산출물 blocker/high 0, r4 H-1·M-1 실행 실측으로 닫힘, acceptance B-0~B-7 exit 0(shared-kernel 77 tests). 재작업 누계 4회/상한 5(verifier not-ready 3 + Codex request_changes 1). **사용자 승인 2026-09-04 — 1B 종결.** 승인은 「코드가 계약을 지킨다」에 대한 것이며 1B 축 authoritative corpus 0건(`1B-c` 이관)은 승인 시 명시 제시됐다 |
