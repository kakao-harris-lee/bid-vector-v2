...
| ID | 판단 | 근거 |
| --- | --- | --- |
| **D-9** | D-1(a) 확장에 **읽기 경로**를 포함한다: `Rate.fraction` 을 공개 읽기(`val`)로, `Rate : Comparable<Rate>`. 생성자는 `internal` 그대로 — 1B 가 닫은 것은 **구성**이고 정규화된 값의 읽기는 불변식을 깨지 않는다. `Money` 의 `export()` 대응물 | 스카우트 §4.2 ①. 없으면 `floor-threshold` authoritative 둘을 실행할 방법이 없다. **운영자 사후 확인 대상**(D-1 (a) 의 「셋」 밖 넷째 항목) — **운영자 승인 2026-09-06(1D 종결 시)**. 근거는 `Rate.kt` KDoc 이 갖는다 |
| **D-10** | 임계 사정률 나눗셈의 자리수는 정책 데이터(`criticalRateScale`, legacy 6 = `legacy-behavior`) — 1B `divideForRate` 와 같은 반올림 경로를 쓴다. 십진 빈도는 내지 않는다(D-7). **정정 2026-09-06(verifier r1 F-1)**: `criticalAssessmentRate` 는 **파생값**이므로 decision 17(`data-dictionary.md` §9·§11.1 — 파생 율 `AssessmentRate`·`AwardRate`·`BidRate` 는 계산 정책 version 을 `DerivationRecord` 로 싣는다)대로 형제 함수 `assessmentRateAgainst` 와 같은 `Measurement<Derived<AssessmentRate>>` 를 돌려준다. D-1(a) 표의 「`criticalAssessmentRate(bid, floor): AssessmentRate`」 문면은 계약 초안의 오기이며 승인 명세가 우선한다. `AssessmentRate.observed` 는 **관측 표본의 재구성**에만 쓰고 파생값을 그 factory 로 만들지 않는다. floor 봉투는 `Derived<AssessmentRate>` 를 그대로 나르고(두 정책 version — 나눗셈 자리수 정책과 floor 정책 — 이 각자 자리에 남는다), 자리수는 `FloorShortfallPolicyData.criticalRateScale` 에서 온다(F-3) | decision 17 · §1.4.2 「파생값」 |
...
| **D-13** | 커널 출력 봉투 `ProvenanceJudgement(original: BaseAmount, classification, evidence: ProvenanceEvidence(firstMatchedRule, ruleOrder, policyVersion), recoveryEstimate: Fact<Money>)` · `FloorShortfallJudgement(tally: ShortfallTally, critical: Derived<AssessmentRate>, result: FloorShortfall)`(**정정 2026-09-06, verifier r2 N-2** — D-10 정정과 같은 갈래, 봉투는 `Derived` 를 나른다). 감사 boolean·`transitionedFrom`·평문 version·두 깊이의 `firstMatchedRule` 은 **runner 투영**이 봉투에서 낸다 — 입력·기대값에서 읽지 않는다 | 1C ⑥·⑦ 관례 |
