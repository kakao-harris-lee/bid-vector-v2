...
28. **ktlint 는 컴파일러 소스를 입력으로 받지 못했다.** `sizeGate`·`sourceLanguageGate`·detekt
    에는 `compileKotlin` 의 `sources` 를 더했으나, ktlint Gradle 플러그인은 source set 에서
    task 를 만들고 임의 파일 집합을 받는 자리가 없다. **커버리지 차이가 실질이 되려면 소스가
    source set 밖에 있어야 하는데**, 그런 소스에서 나온 class 는 `SourceFile` 대조가 잡는다
    (`E-53`~`E-55`). 그래도 「형식 검사만은 그 파일을 못 본다」는 사실은 남는다.
    **detekt 쪽 배선의 효과는 따로 재지 않았다** — 세 fixture 를 닫은 것은 원산지 층이다.
