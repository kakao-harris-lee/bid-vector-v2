...
3. **domain 경계는 네 층이고 각 층의 실패 방향이 고정돼 있다.** T-A 하위 허용(`bidvector`) ·
   ...
   **T-D 는 여전히 열려 있는 열거다**(숨기지 않는다) — 다만 대상이 허용된 62 종 안의 정적 멤버로
   좁고 T-C 가 그 밖을 이미 닫았다. `Integer.valueOf`(필수)와 `Integer.getInteger`(환경 읽기)가
   같은 클래스라 클래스 단위로 가를 수 없는 자리다.
...
7. **두 게이트의 사각이 서로 반대다.** ArchUnit 은 바이트코드를 보므로 **선언만 되고 참조가
   ...
   **이 자리에서 참이 아니다.**
8. **detekt 2.0 은 alpha 이고, 그것이 죽으면 함수 50줄 임계가 사라진다.** 파일 축은 도구 비의존
   `sizeGate` 라 남지만 **함수 축의 정본은 detekt 이다**(scope.md D-4 의 표). 인라인
   `@Suppress("LongMethod")` 로 끄는 것은 sizeGate 가 막지만(실측) **도구 자체가 죽는 경우는
   덮지 못한다.** 그 축의 도구 비의존 백스톱은 Kotlin 함수 경계를 파서 없이 재야 해서 1A 가
   만들지 않았다 — 필요해지면 별도 결정이다.
9. **`X::class.java` 클래스 리터럴은 게이트가 보지 못한다.** 상수 풀의 Class 엔트리는 ArchUnit
...
10. **Java source-set 봉쇄의 task 수준 음성이 자동화돼 있지 않다.** 구조적 봉쇄와
   `sourceLanguageGate` 는 실측으로 확인했으나(`E-25`), 그것을 **테스트로** 고정하려면 중첩
   빌드(GradleRunner/TestKit)가 필요하다. 도입하지 않았다 — 실행 시간 대가가 있고 이 빌드
   (included build + configuration cache)에서 도는지 재지 않았다. 지금 증거는 자동 테스트가
   아니라 기록된 실측이다.
11. **모듈 안의 패키지 순환은 아직 시험되지 않았다.** 규칙(`bidvector.(**)` slice)은 걸려 있으나
   각 모듈에 패키지가 하나뿐이라 모듈 **내부** 순환을 만들 자리가 없다. 위반 fixture 의 순환은
   모듈 사이다. 1B 가 패키지를 늘리면 그때 안쪽에서도 구속력이 생긴다.
12. **CI 워크플로가 실행된 적 없다.** 원격에 push 하지 않았으므로 `.github/workflows/ci.yml` 은
   **문법만 갖춘 골격**이고 GitHub 러너에서 도는 것을 확인하지 못했다.
13. **허용 목록의 과잉 도달 — `java.net` 이 허용 정확 패키지에 없어 `java.net.URI` 도 막힌다.**
   ...
   **함께 잰 것**: `java.math.BigDecimal` 과 `Charsets` 는 막히지 않는다 — 1B 의 금액 타입과
   인코딩은 안전하다.
14. **이 slice 의 `milestone-1.md` 편집이 `capability-map.md` 의 M0 작성 좌표를 깨뜨렸다 —
...
15. **비결정성(시계·난수)은 게이트 대상이 아니다 — 1B 인계.** `Instant.now()`·`Clock`·
