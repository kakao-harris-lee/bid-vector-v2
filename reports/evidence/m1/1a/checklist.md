...
26. **아카이브의 원산지 검사는 바이트 스캔이다 — 근거는 「도구를 못 믿어서」가 아니다.**
    상수 풀에서 `kotlin/Metadata` 를 찾는 방식을 쓰고, 그것이 **실제로 잡는 것을 `E-52` 가
    실측**했으며(검증 집합에 심어 포함 관계를 통과시킨 class 를 이 층이 잡는다) 그 층에
    라이브러리 의존이 없다. 틀리는 방향도 닫히는 쪽이다 — 그 문자열을 참조만 하는 Java
    클래스는 통과하지만 **포함 관계가 이미 잡는다**(1차가 포함 관계, 2차가 원산지).

    **처음 이 선택의 근거로 적었던 관측은 재현되지 않았다.** ArchUnit `importJar` 이 같은 jar 의
    Java 산출 클래스를 빠뜨리는 것을 **한 번 보았고**, 그 조건을 그대로 복원해 다시 재니
    3 회 전부 정상 임포트였다(verifier 도 세 구성에서 재현 실패). **1 회 관측 · 재현 실패**로
    적고 원인은 규명하지 못했다 — 층의 선택은 위 근거로 그대로 서지만, 그것을 정당화하려고
    재현되지 않는 관측을 들지는 않는다.
