...
3. **domain 경계는 allow-list 이고 허용은 정확 패키지다.** 금지 열거는 세 라운드 연속 새는
   좌표를 냈고 마지막(`kotlin.io`)은 **승인 문서가 이름으로 든 적이 없어 출처 대조로도 잡히지
   않았다.** 허용을 **뿌리**로 적었을 때는 같은 게임이 뿌리 안으로 이사했다(`java.util.logging`·
   `java.lang.reflect`·`kotlin.reflect`). 정확 패키지는 하위를 열지 않으므로 그 게임이 끝난다.
   **잔여 deny 는 허용된 정확 패키지 안에** 남는다 — 한 패키지가 순수 타입과 위험 타입을 함께
   담으면 패키지 단위로 가를 수 없기 때문이다(`java.lang` 의 프로세스·스레드 진입점, `java.util` 의
   `Timer`·`ServiceLoader`). **JDK 가 고정한 패키지의 원소라 유한하고**, 그래서 무한 열거가 아니다.
   **대가는 과잉 도달**이다 — 막히는 자리는 `ADR 0007` D-4 의 allowlist 형식으로 연다(제한 9).
   **컴파일러 산출물은 허용한다** — `@NotNull`·`Intrinsics`·`FunctionN` 은 사용자 코드가 아니고,
   막으면 `data class` 하나로 게이트가 깨진다(실측).
4. **게이트가 실제 도메인 형태로 시험된다.** 양성 fixture 가 `data class`·nullable·컬렉션·
...
5. **경계 판정의 근거는 산출물의 위치다.** 그 전에는 클래스의 자기 신고에 기댔고, 선언 없는
...
6. **두 게이트의 사각이 서로 반대다.** ArchUnit 은 바이트코드를 보므로 **선언만 되고 참조가
   ...
   **이 자리에서 참이 아니다.**
7. **detekt 2.0 은 alpha 다.** 규칙과 설정 키가 정식 출시 전에 바뀔 수 있다. 승인된 두 임계 중
   파일 축은 detekt 밖(`sizeGate`)에 있어 detekt 이 죽어도 남는다.
8. **모듈 안의 패키지 순환은 아직 시험되지 않았다.** 규칙(`bidvector.(**)` slice)은 걸려 있으나
   각 모듈에 패키지가 하나뿐이라 모듈 **내부** 순환을 만들 자리가 없다. 위반 fixture 의 순환은
   모듈 사이다. 1B 가 패키지를 늘리면 그때 안쪽에서도 구속력이 생긴다.
9. **CI 워크플로가 실행된 적 없다.** 원격에 push 하지 않았으므로 `.github/workflows/ci.yml` 은
   **문법만 갖춘 골격**이고 GitHub 러너에서 도는 것을 확인하지 못했다.
10. **허용 목록의 과잉 도달 — `java.net` 이 허용 정확 패키지에 없어 `java.net.URI` 도 막힌다.**
   ...
   **함께 잰 것**: `java.math.BigDecimal` 과 `Charsets` 는 막히지 않는다 — 1B 의 금액 타입과
   인코딩은 안전하다.
11. **이 slice 의 `milestone-1.md` 편집이 `capability-map.md` 의 M0 작성 좌표를 깨뜨렸다 —
...
12. **비결정성(시계·난수)은 게이트 대상이 아니다 — 1B 인계.** `Instant.now()`·`Clock`·
...
13. **RED/GREEN 커밋을 checkout 해서는 재현할 수 없다.** 빌드에 필요한 여덟 파일이 `90e46ff`
