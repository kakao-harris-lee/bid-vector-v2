...
47. **소문자 뿌리의 지역 선언 판별은 파일 단위다 — 같은 패키지 다른 파일의 top-level 선언은
    못 본다(verifier r19 M-2 잔여).** `SourceReferences.locallyDeclaredNames` 는 현재 파일의
    프로퍼티·파라미터·import 별칭만 모은다. `tree.Node()` 형태에서 `tree` 가 **다른 파일의
    top-level 프로퍼티**로 선언돼 있으면 그 이름은 이 집합에 없어 여전히 오탐(닫히는 방향)일
    수 있다 — 회피책은 제한 34①을 따른다(같은 파일로 선언을 옮기거나 이름을 바꾼다). 스코프를
    가리지 않고 파일 전체를 훑으므로 반대 방향(실제로는 다른 이름인데 우연히 겹쳐 놓친다)의
    위험은 없다 — 이름이 같으면 무조건 값 체인으로 건너뛰므로 더 자주 닫히는 쪽으로만 넓어진다.
