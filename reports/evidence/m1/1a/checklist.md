...
3. **domain 경계는 allow-list 다 — 판정 방향이 뒤집혔다.** 금지 열거는 세 라운드 연속 새는
   좌표를 냈고 마지막(`kotlin.io`)은 **승인 문서가 이름으로 든 적이 없어 출처 대조로도 잡히지
   않았다.** 이제 domain 이 볼 수 있는 것을 열거하고 나머지를 전부 막는다. **대가는 과잉 도달**
   이다 — 정당한 도메인 코드가 막히면 `ADR 0007` D-4 의 allowlist 형식으로 연다(제한 7 참조).
4. **경계 판정의 근거는 산출물의 위치다.** 그 전에는 클래스의 자기 신고에 기댔고, 선언 없는
...
5. **두 게이트의 사각이 서로 반대다.** ArchUnit 은 바이트코드를 보므로 **선언만 되고 참조가
   ...
   **이 자리에서 참이 아니다.**
6. **detekt 2.0 은 alpha 다.** 규칙과 설정 키가 정식 출시 전에 바뀔 수 있다. 승인된 두 임계 중
   파일 축은 detekt 밖(`sizeGate`)에 있어 detekt 이 죽어도 남는다.
7. **모듈 안의 패키지 순환은 아직 시험되지 않았다.** 규칙(`bidvector.(**)` slice)은 걸려 있으나
   각 모듈에 패키지가 하나뿐이라 모듈 **내부** 순환을 만들 자리가 없다. 위반 fixture 의 순환은
   모듈 사이다. 1B 가 패키지를 늘리면 그때 안쪽에서도 구속력이 생긴다.
8. **CI 워크플로가 실행된 적 없다.** 원격에 push 하지 않았으므로 `.github/workflows/ci.yml` 은
   **문법만 갖춘 골격**이고 GitHub 러너에서 도는 것을 확인하지 못했다.
9. **허용 목록의 과잉 도달 — `java.net` 이 허용 목록에 없어 `java.net.URI` 도 막힌다.** allow-list 의
   대가다 — 허용을 좁게 두면 정당한 타입도 함께 막힌다. 도메인이 URI 를 값으로
   ...
   **함께 잰 것**: `java.math.BigDecimal` 과 `Charsets` 는 막히지 않는다 — 1B 의 금액 타입과
   인코딩은 안전하다.
10. **이 slice 의 `milestone-1.md` 편집이 `capability-map.md` 의 M0 작성 좌표를 깨뜨렸다 —
...
11. **RED/GREEN 커밋을 checkout 해서는 재현할 수 없다.** 빌드에 필요한 여덟 파일이 `90e46ff`
