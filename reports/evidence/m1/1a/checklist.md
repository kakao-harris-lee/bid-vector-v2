...
47. **(정정, Codex 14차 #1) 소문자 뿌리의 지역 선언 판별은 조상 사슬이다 — 같은 패키지
    다른 파일의 top-level 선언, 그리고 같은 파일 안의 무관한 형제 함수·클래스의 선언은
    못 본다.** verifier r19 M-2 가 처음 쓴 파일 전체 이름 집합은 **미탐**을 냈다 — 다른
    함수의 동명 지역 변수(`val java = 1`) 하나가 그 파일 전체의 완전수식 참조를 지웠다
    (Codex 14차 #1). `SourceReferences.visibleLocalNames`(`KtElement` 확장, 조상 사슬만
    본다)로 판별을 좁혔다 — 게이트 술어의 원칙(오탐이 미탐보다 낫다)을 지키는 방향의 정정이다.
    `tree.Node()` 형태에서 `tree` 가 **감싸는 함수·람다·클래스가 아닌 자리**(다른 함수·다른
    파일의 top-level 선언)에 있으면 그 이름은 조상 사슬에 없어 여전히 오탐(닫히는 방향)일 수
    있다 — 회피책은 제한 34①을 따른다(같은 파일의 감싸는 스코프로 선언을 옮기거나 이름을
    바꾼다). 조상 사슬만 보므로 반대 방향(실제로는 다른 스코프의 이름인데 우연히 겹쳐 놓친다)
    의 위험은 이전보다 오히려 줄었다 — 무관한 형제 선언은 더 이상 섞이지 않는다.
