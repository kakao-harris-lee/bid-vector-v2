...
**`java.lang` 안에만 잔여 deny 가 남는다.** 그 패키지가 프로세스·스레드·환경·클래스 로딩의
진입점을 함께 담고 있어 패키지 단위로 가를 수 없다. **이것은 무한 열거가 아니다** — `java.lang`
은 JDK 가 고정한 패키지라 원소가 유한하고 새 좌표가 임의로 늘지 않는다. 그 유한성이 deny-list 를
버린 근거(좌표가 무한히 는다)와 이 잔여 deny 를 가르는 지점이다.
