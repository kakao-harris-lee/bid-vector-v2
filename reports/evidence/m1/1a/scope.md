...
2. 같은 절이 「함께 측정한다」로 든 여섯 축 중 **다섯**(함수/메서드 크기·복잡도, 파일 크기, 모듈
   fan-in/fan-out, public API 수, 순환 의존)과 `OPEN-ADR-06` 이 묻는 세 축(클래스/타입 크기 ·
   상속 깊이 · mixin 수)을 **재는 task** 를 만들고 그 실측을 `capability-map.md` §12 에 등재한다.
   **여섯째 축(`duplicate mechanical helper`)은 도메인 코드가 없어 측정이 공허하므로 이월한다** —
   추적은 `OPEN-ADR-16`(`ADR 0007` §5, Codex 12차 medium · 운영자 결정 2026-09-03), 도구 조사와
   확정은 1B. 나머지 다섯·세 축의 **결정은 운영자**다 (`capability-map.md` §14.2 의 `OPEN-ADR-06`
   행 — 결정 주체가 운영자, 시점이 1A 실측 뒤).
