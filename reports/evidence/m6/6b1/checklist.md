...
6. **`StrategyAdapterDependencyTest`(바이트코드 상수 풀 의존 게이트)는 정적 참조만 본다 — 이 층의 경계(verifier r3 LOW-6·r6 시정)** — 게이트는 컴파일 산출물에 남은 **정적 참조만** 본다 — 인라인된 값·본문(금지 모듈의 `public const val`·top-level `public inline fun`을 쓰면 컴파일 시점에 값·본문이 인라인돼 상수 풀에 금지 좌표가 안 남는다, verifier r3 실측: 게이트 초록·상수 풀 흔적 0)과 **런타임에 조립되는 이름**(완성된 좌표가 상수 풀에 단일 상수로 남지 않는 형태, verifier r6 실측)은 **그 밖**이다. **이 층에서는 못 잡는다 — 잡으려면 다른 층(예: 런타임 클래스로더 감시)이 필요하다**, 정규식을 넓혀 이 층에서 잡으려 하면 산문 문자열 오탐(아래 7)이 폭증한다. 지금은 잠재다 — 금지 루트(`procurement`·`decision`·`qualification`·`workflow.event`·`adapters.ml`)에 그런 인라인 표면도 조립 우회 실사용도 현재 0건이다. 처방은 게이트 신설·정규식 확장이 아니라 이 경계를 적어 두는 것 — 뒤 slice가 같은 자리에서 정규식 확장으로 재발명하지 않게 한다.
7. **`StrategyAdapterDependencyTest`의 넓힌 정규식(verifier r3 MEDIUM-5 수정)이 산문 문자열 상수도 본다(verifier r4 LOW-7)** — 술어가 상수 풀의 Utf8 문자열을 점·슬래시 표기 양쪽으로 훑으므로, `bidvector.adapters.strategy` 패키지의 **의존이 아닌** 산문 문자열 리터럴이 금지 루트(`procurement`·`decision`·`qualification`·`workflow.event`·`adapters.ml`)를 언급하면(예: 주석성 텍스트를 문자열 상수로 박아 두는 경우) 게이트가 거짓 양성으로 걸린다. verifier가 변이로 확인했다. 영향은 이 패키지 main 소스의 문자열 리터럴에 한정되고, 실패 메시지가 걸린 문자열을 그대로 찍어 줘 원인 파악이 즉시 된다 — 코드 수정 없이 등재만 한다.
...
| 전수 확인(열거 방어 금지) | ~~`EditSessionRow.kt`의 모든 디코드 접근자를 grep으로 전수 확인 — ... `BigDecimal(it.asText())` 넷(임계값·점수)은 정확히 왕복되거나 `NumberFormatException`으로 크게 실패해 같은 결함 계열이 아니다.~~ **verifier r7 HIGH-5 시정 — 이 주장은 사실이 아니었다.** `.asInt()`/`.asLong()`/`.asDouble()`/`.asBoolean()`은 `requireIntValue`/`requireLongValue` 안 둘뿐이라는 것과, `.asText()`로 읽는 enum·kind 문자열 필드가 `workflow/EditSessionRestore.kt`의 소진 `when`/`else -> error(...)`로 한 층 뒤에서 닫힌다는 것(D-6B1-7, (2b) 표의 「필드별 어휘는 검사」)은 그대로 맞다. 틀린 것은 `BigDecimal(it.asText())` 판정 — 아래 HIGH-5 참고 | 없음(BigDecimal 행 제외) — 코드 변경 0, 이 표가 실측 근거 |
...
## verifier r7 — HIGH-5 처분

| finding | 처분 | 근거 |
| --- | --- | --- |
| HIGH-5 — 십진 네 필드(`minimumMatchScore`·`minimumProbabilityScore`·`bidNowThreshold`·`reviewThreshold`)가 척도·정밀도를 잃고 조용히 다른 값으로 왕복한다(`0.70`→`0.7`) | **수정** — 코덱 전용 `ObjectMapper`에 두 설정을 함께 켠다: `DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS`(부동소수 토큰을 `DecimalNode`로) + `JsonNodeFeature.STRIP_TRAILING_BIGDECIMAL_ZEROES = false`(Jackson 내부가 그 값에서 끝자리 0 을 지우는 디폴트를 끔). 바이트코드 실측(`BaseNodeDeserializer._fromFloat`/`_fromBigDecimal`, jackson-databind 2.21.5)으로 **두 설정이 함께 필요함**을 확인 — 앞 설정만으로는 `getDecimalValue()`가 척도를 정확히 돌려준 뒤에도 끝자리 0 이 지워진다 | `JdbcEditSessionRepositoryTest`에 DB 행 기원 test 셋: 척도(`0.70`) 왕복 동일성, double 정밀도를 넘는 고정밀 값 왕복 동일성, **척도 있는 값으로 같은 command 재전달 시 효과 0 의 `Accepted`**(실제 깨진 계약) |
| 매퍼가 이 코덱 전용인가 | `grep -rn "ObjectMapper(" adapters/src/main/kotlin/` — `EditSessionRow.kt`·`SchemaValidation.kt` 2건, 서로 다른 `private val` 인스턴스(전자는 `internal object` 멤버라 그 object 밖에서 참조 자체가 불가). 이 변경은 다른 디코드 경로에 영향 없음 | 위 grep + 두 파일 각자 자기 `ObjectMapper()` 소유 확인 |
| 관찰(코드 변경 없음) — `duplicateOutcome`(`Transition.kt`)의 재전달 동등성 비교가 척도에 민감하다 | **적기만 함, 고치지 않음.** `EditCommand`/`StrategyDraft`의 데이터 클래스 동등성은 `BigDecimal.equals()`(척도까지 비교)를 쓴다 — 코덱을 고쳐 왕복이 정확해지면 "같은 입력 재전송"에서는 증상이 사라지지만, **왕복이 아니라 비교 그 자체가 척도에 민감하다는 성질은 남는다**(예: 다른 경로로 다른 척도의 "수학적으로 같은" 값이 들어오면 여전히 다른 값으로 판정될 수 있다). 코덱 고정으로 이 slice 의 실제 노출(DB 왕복)은 닫히므로 지금 코드를 더 건드리지 않는다 — 별도 위험 여부는 `Transition.kt`/도메인 동등성 정책을 다루는 slice 의 판단 몫 | `Transition.kt` 무편집(범위 밖, 재작업 상한 근접) |

**변이 실측(버릴 clone, `/tmp/6b1-high5-verify`, 실측 뒤 삭제)** — 수정 전(`ObjectMapper()` 기본값)으로 신설 test 셋을 실행: 척도 왕복·고정밀 왕복·재전달 계약 셋 다 FAILED(척도 `0.70`→`0.7`, 고정밀 끝자리 변형, `Rejected(IdempotencyConflict)`로 재현). 수정 후 원 worktree 재실행 20 tests 0 failed(기존 리터럴 `0.7` 왕복 test 포함 — 정상 값 회귀 없음).
