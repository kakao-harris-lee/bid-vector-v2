...
git rm -f \
  ...
  app/src/test/kotlin/bidvector/app/management/ManagementSurfaceLateSourceRefusalTest.kt \
...
`config/quality/gate-tests.properties`(이 slice 의 test 이름이 없는 판)와 되돌린 test 집합이
짝이 맞아야 통과한다 — ①과 ②를 함께 해야 성립한다(이 slice 의 test 네 이름이 없는 판과 test 네 파일이
...
| ② | 위 `git rm -f` | 0 | 신설 9개 삭제. 인덱스 집계 **D 9 · M 16** |
...
- **CI**: `container` job 의 새 step 다섯(앱 배포물·앱 이미지·앱 위생·거부 스모크·컨테이너 스모크)만
  지우면 6C 축은 그대로 돈다. 단 위생 게이트 step 둘은 정책 파일 인자를 요구하므로 함께 남긴다.
- **앱**: `java -jar app.jar` 로 띄우는 기존 경로는 **관리 포트가 하나 더 열리는 것** 말고는
  ...
  **접두사 거부(D-6A2a-10)와 늦은 재검사(D-6A2a-14)는 끌 수 없다** — 끄는 스위치를 만들지 않았다.
  그 기동 비호환의 범위와 배치 쪽 처방은 `checklist.md` 알려진 제한 15·16 과 「6E runbook 입력」이
  정본이다(운영자가 보는 자리는 되돌림 문서가 아니다). 여기서는 그것이 이 slice 가 만든 **유일한
  기동 비호환**이라는 사실만 적는다.
