...
| 표면 | 값을 얻는가 / 새로 낳는가 | 판정 |
|---|---|---|
| `BusinessDivision`(enum) — 상수 넷 · `fromLabel` · `entries`/`values`/`valueOf` · 앞으로 생길 멤버 | 문자열·어휘 → 대분류 | 닫는다 — 값 획득 축 셋(위 1행). **멤버 목록 없음**: 허용은 (호출자→멤버)·(호출자→소유자#멤버) 쌍이고 관측과 같다. 허용의 근거는 영속 라벨 복원 · 정책 어휘 파생 · 설정 기본 표 한 행 · 운반 슬롯 읽기뿐이다. 클래스 객체 참조자는 0 |
...
`workflow/.../evaluation` test 하나. r1 변경(M): 위 게이트 파일 셋과 정책 둘(`architecture-policy` 는 키 교체 — `division-parse.*` → `division-value.*`) · `gate-tests`(등재 둘 추가) · procurement 관측·분류 main·test · adapters koneps/persistence main·test · app wiring·collection·conformance · workflow test 둘.
...
11. **오퍼레이션 간 대분류는 last-writer-wins 다**(r1 신설) — 공고 목록 관측의 대분류는 절대 `null` 이 아니라(오퍼레이션이 정한다) 존재 가드가 이 축에 대해서는 「유입이 항상 이긴다」로 작동한다. 같은 공고번호·차수가
    ...
    측정으로 잠갔다). 충돌을 세는 자리는 없다(R1 은 응답 라벨 불일치만 다룬다). 오늘은 **공고번호가 오퍼레이션 간 배타**라는 전제에 의존하고, 그 전제의 실측은 7항 ⑨다. 병합 의미는 이 slice 가 바꾸지 않았다.
...
-- ⑨ 오퍼레이션 간 겹침 (알려진 제한 11 의 전제 실측) — 두 질의 다 0 이면 「공고번호는 오퍼레이션 간 배타」가 성립한다.
--    (a) 정확: 용역 전용 칸과 공사 전용 칸이 **둘 다** 찬 행 = 두 오퍼레이션이 같은 공고를 채운 것(존재 가드가 둘을 남긴다).
SELECT COUNT(*) FROM notice WHERE service_division IS NOT NULL AND main_construction_type IS NOT NULL;
--    (b) 넓은 대리 측정: 원문 행에는 오퍼레이션 정보가 없다(제한 2) — 오퍼레이션마다 자기에게만 있는 키의 존재로 가른다.
--        주공종이 공사 응답의 약 33% 에만 있어 **과소 계수**다(0 이 배타의 증명은 아니고, 0 이 아니면 확실한 겹침이다).
SELECT COUNT(*) FROM (
  ...
  GROUP BY 1, 2
  HAVING bool_or(payload_fields ? 'srvceDivNm') AND bool_or(payload_fields ? 'mainCnsttyNm')
) overlapping;
```
