...
- **D-6F4-6 — 배선이 올 때 공고명 키를 어댑터에 하드코딩하지 않는다.** KONEPS 원시 키는 정책
  ...
  된다.
- **D-6F4-7 — 마이그레이션 번호는 V14 다(2026-09-19 갱신, 착수 시 V11).** 착수 시점의 main 은 V9
  까지였고 V10 을 6A-1 계약이 선점해 V11 을 잡았다. 그 뒤 운영자 지시로 병합 순서가 뒤집혀
  **6F-6(V12)이 먼저 병합**됐고(PR #38), V13(PR #39)도 대기 중이다.
  **그래서 V14 로 비켜선다.** 새 DB 에서는 V11 이든 V14 든 무해하고 영속 인스턴스도 0 이라 **오늘의
