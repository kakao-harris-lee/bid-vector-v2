...
## 수정 라운드 3 — verifier r3 처분(r2 여덟 중 일곱 닫힘, R3-1 한 줄로 마무리)

r3 판정: r2 findings 여덟 중 일곱이 닫혔다(다이제스트 바꿔치기가 이제 exit 1, CI 에
통과 불가 step 이 더 없음, root 자식 검출이 전 행을 읽음 등). **차단은 R3-1 하나뿐**이고
팀장 지시대로 **한 줄로 닫고 그 이상 모양 분기를 늘리지 않는다.**

| ID | severity | 처분 | 근거 |
| --- | --- | --- | --- |
| R3-1 | HIGH | **수정**(한 줄) — `list` kind 원소를 절삭한 뒤 빈 원소 거부 | 공백/탭만인 원소가 `[ -z ]`를 통과해 그 축 검사가 꺼졌다. 변이 셋(공백만·탭·혼합) 전부 exit 2 로 시정 확인, 정본 정책은 그대로 통과 |
| R3-2 | LOW | **부분 수정 + 등재** — postgres 서비스에 `no-new-privileges` 추가(무해 확인). `RealServerIntegrationTest`의 Testcontainers 컨테이너는 알려진 제한으로 등재 | 판정이 "이 이미지는 안전하다"가 아니라 "이 플래그 아래서 안전하다"로 좁혀졌다는 지적 — 미적용 경로 둘 중 하나(compose)는 닫고, 나머지 하나(test 하네스, 배포 경로 아님)는 등재 |
| R3-3 | LOW | **수정**(문면만) — 실패 메시지 둘에 `OPEN-6C-MULTIARCH`·플랫폼 불일치 가능성을 함께 적음. 정책 파일 주석에도 포인터 복원 | `linux/amd64` 하드코딩이 fail-closed 는 맞지만 사유가 "FROM 이 다르다"로만 읽혀 플랫폼 불일치 가능성이 가려졌다 |
| R3-4 | LOW | **수정**(장부) — rollback.md 기준 커밋을 `bbdfba3`→`29e0b40`으로 갱신, 재실행은 verifier r3 가 직전 태그에서 한 결과를 참조로 명시(직접 재실행하지 않음) | 실측 기준이 3 커밋 낡았다(파일 셋 불변이라 목록 자체는 유효했음) |
| R3-5 | LOW | **등재만**(수정 안 함, 팀장 지시 — step 순서를 또 건드리지 않는다) — `check` job 이 `check`·`qualityBaseline`을 이중 실행하는 비용 | R2-2 처방(job 이동)이 job 간 중복을 job 안 중복으로 옮긴 결과. Gradle 증분 덕에 실비용은 작음(로컬 실측 5~6초) |
| R3-6 | LOW | **수정**(표시만) — 위생 게이트 요약이 관측한 프로세스 uid 전부를 보여줌(`실프로세스-uid전체=[...]`) | 판정은 R2-5 부터 전 행을 봤지만 요약은 첫 행만 보여줘 검사 범위를 못 드러냈다 |

### 재작업 패턴에 대한 처분(verifier r3 진단 수용)

verifier r3 는 r1 F-3·r2 R2-1·r3 R3-1 이 **같은 함수**(`_policy_value`)에서 매 라운드
다른 미검증 모양이 통과한 「열거 방어의 서명」이라고 진단했다(신설 검증 분기가 매번
하나씩 늘고 매번 다른 구멍이 남는 패턴). **이 진단을 그대로 받아들인다** — 이번
...
- (수정 라운드 3 신설, R3-2 승계) 능력 차단(`no-new-privileges`)이 위생 게이트·compose
  둘 다에 걸려 있지만 `RealServerIntegrationTest`가 Testcontainers 로 띄우는 컨테이너에는
  없다 — 그 test 소스에 관련 설정이 0건이다. 배포 경로가 아니고(D-6C-1, 운영 배포 자체가
  이 slice 밖) 판정 목적이 "Kotlin 소비자 규칙이 실 응답에서 성립하는가"라 능력 차단까지
  그 test 의 책임으로 보지 않는다 — 다만 이 이미지를 다른 자리에서 재사용할 때는 이 플래그
  없이 뜬 사실을 알아야 한다.
- (수정 라운드 3 신설, R3-3 승계) 베이스 layer 파생이 `linux/amd64` 를 하드코딩해 고른다
  (`OPEN-6C-MULTIARCH`). 이 개발 환경은 호스트가 arm64 인데도 빌드 산출물이 amd64 로
  나와(실측) 지금은 맞아떨어지지만, 다른 플랫폼으로 빌드하면 이 축이 fail-closed 로
  떨어진다 — 통과가 아니라 실패이므로 안전하지만, 실패 사유 문면이 플랫폼 불일치
  가능성을 함께 적도록 이번 라운드에서 갱신했다.
- (수정 라운드 3 신설, R3-5) `check` job 이 자기 자신 안에서 `./gradlew --no-daemon
