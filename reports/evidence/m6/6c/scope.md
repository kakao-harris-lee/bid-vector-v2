...
| **D-6C-9** (계약 갱신 (4), verifier r1 HIGH 둘) | 위생 게이트의 두 술어는 **선언이 아니라 실행·빌드 산출물**을 읽는다 — ① non-root: `--entrypoint` 로 덮어쓴 `id -u`(선언된 `USER` 만 잼)가 아니라 **실 ENTRYPOINT 로 띄운 컨테이너의 pid 1 사용자**(`docker top -o user` 류)를 정책 값과 대조 ② 베이스 고정: Dockerfile 이 손으로 적는 라벨 문자열이 아니라 **빌드가 실제로 해석한 베이스 layer 체인**을 대조(초판은 BuildKit provenance attestation 을 앞세웠으나 **채택된 구현은 D-6C-10 ② 의 다이제스트 파생**이다 — 계약 갱신 (7) 정정, verifier r4 LOW ②). 라벨·선언은 보조 근거로만 쓰고 단독 근거로 쓰지 않는다 | verifier r1 실측: `USER` 를 유지한 채 setuid 로 root 를 얻는 entrypoint 이미지에서 게이트 exit 0(그냥 실행하면 EUID 0) · `FROM` 을 떠 있는 태그로 바꾸고 라벨만 남긴 이미지에서 게이트 exit 0. **출하 이미지 자신은 안전**(pid 1 uid 10001, root 잠김)이고 게이트의 맹점이다. 우회 (1)·위협 모델 (b)(c) 가 한 토큰 편집으로 무력화됐다 |
| **D-6C-10** (계약 갱신 (5), verifier r2 HIGH 둘) | ① 위생 게이트의 정책 파서는 **키 모양뿐 아니라 값 모양**을 검증한다 — 빈 값 거부(직전 판에 있었고 F-3 시정이 지웠다)·수치 키 `^[0-9]+$`·열거 키 원소 ≥1·CRLF 절삭, 위반은 비-0. 수치 비교를 `elif` 조건에 두어 `set -e` 를 면제시키지 않는다. ② **베이스 고정의 정책 값은 layer 목록이 아니라 고정 다이제스트 하나**로 두고, 게이트가 그 다이제스트를 pull 해 layer 체인을 **파생**해 대조한다(값 바꿔치기 표면을 목록에서 다이제스트 하나로 줄인다. **채택된 구현은 pull 불가·판정 불가를 fail-closed 로 끊는다** — 초판 괄호가 허용한 「목록 유지」 대안은 쓰지 않았고 구현이 계약보다 엄격하다, 계약 갱신 (7) 정정, verifier r4 LOW ②). ③ non-root 는 점 표집에 기대지 않고 **능력 차단**을 함께 요구한다(`--security-opt no-new-privileges` 로 상승 자체를 막고, 출하 이미지가 그 조건에서 READY 로 서는 것을 실측). ④ CI 에서 6C 게이트를 돌리는 step 은 **그 job 의 전제(도구·이력 깊이)가 이미 충족된 job**에 둔다 — 전제를 새 job 에 복제하지 않는다 | verifier r2 실측: F-3 시정이 빈 값 거부를 함께 지워 `forbidden.packages=`·`base.image.layers=`·`size.cap.bytes=` 를 비우면 게이트가 통과하고, 특히 layer 목록을 비우면 **`base-layer-접두-일치=false` 를 찍으면서 exit 0** 이었다 · F-6 시정이 넣은 S-20 step 이 `buf`·전체 이력 부재로 통과 불가이고 그것이 step 6 이라 **S-21~S-25(6C 게이트 전부)가 CI 에서 한 번도 실행되지 않았다** · 점 표집(`sleep 1`)은 표집 뒤 상승하는 entrypoint 를 놓쳤다(t=2s 비특권 → t=8s root) |
...
| 2026-09-17 승인 전 일괄(7) | D-6C-9·D-6C-10 ② 문면을 **채택된 구현**과 일치시킴(provenance 를 앞세운 초판 서술 → 다이제스트 파생 · 「pull 불가 시 목록 유지」 대안 → fail-closed, 구현이 계약보다 엄격) | verifier r4 LOW ② — 계약이 채택하지 않은 경로를 서술하고 있었다. 구현·게이트 무편집, 문면만 |
