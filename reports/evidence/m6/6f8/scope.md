...
## 계약 갱신 (3) — 실수집 1차가 드러낸 결함 하나와 공백 하나 (2026-09-24, 팀장)

verifier r2(판정 SHA `38510674`) **ready-for-review**(HIGH 0 · MEDIUM 2 · LOW 3). 그 SHA 의 배포물 jar 로 D-6F8-5 실수집을 돌렸다(최근 30일 KST,
공사 + 용역, V2 전용 개발 DB). **코드는 끝까지 돌았다** — 슬롯 62 전부 완료·절단 0·쿼터 멈춤 0·exit 0, 수신 = 정규화 + 탈락 등식 성립,
정규화된 공고 전부가 공고명을 가짐, 로그에 키 흔적 0, 평가 dry-run 이 수집 데이터로 끝까지 돌고 키워드 규칙이 공고명으로 정확히
매칭(「도로」 마감 전 공고 수 == review 수). 수치는 `commands.md` 실수집 절이 정본(건수·비율만).

**D-6F8-11 — 빈 문자열인 OPTIONAL 일시는 부재다. 공고를 버리지 않는다(실데이터 결함, 수정한다).** 탈락의 거의 전부가 입찰마감일시
(`bidClseDt`, 계약 `OPTIONAL`)가 **빈 문자열**인 공고였다 — 빈 값을 「일시 해석 실패」(`DATE_TIME`)로 다뤄 공고 전체를 탈락시켰다
(대부분 용역). D-6F4-8 「값이 없으면 없다」와 계약 `OPTIONAL` 문면에 따르면 **부재(`null`)** 다. 합성 fixture 가 빈 일시를 담지 않아
어떤 test 도 이 형태를 재지 않았다 — **실데이터를 붙여야만 드러나는 결함이 실제로 나왔다.**
- 수정 지점은 `canonicalize` 의 일시 해석 한 곳: 공백·빈 값 → 그 필드 `null`. 형태가 있는데 해석이 안 되는 값만 `DATE_TIME` 탈락.
  같은 규칙을 **모든 OPTIONAL 필드의 빈 값**에 일관되게 적용하는지 전수(금액·율·코드는 이미 빈 값 → 부재인지 D-6F8-7 표와 대조).
- 필수(`REQUIRED`) 필드의 빈 값은 지금처럼 탈락 — 사유가 「해석 실패」가 아니라 「필수 필드 부재」로 가는 것이 맞는지도 판단해 적는다.
- 나머지 몇 건은 일시 형태가 정상인데도 `DATE_TIME` 으로 탈락했다 — 원인을 원문 형태(값이 아니라 형태)로 진단해 고치거나 사유를 적는다.
- 잠금: 빈 `bidClseDt` 항목 → 정규화되고 마감 `null` · E2E 표본에 빈 일시 항목 · 수정 뒤 실수집 3차로 탈락 수가 줄어드는지 확인(팀장).
- 마감이 `null` 인 공고가 평가 후보(마감 전)에 어떻게 들어가는지는 기존 후보 원천 규칙(`JdbcCandidateSource` 의 마감 조건)을 따른다 —
  바꾸지 않는다. 그 거동을 checklist 에 한 줄로 적는다.

**D-6F8-12 — 업무구분 코드가 목록 응답에 없다 → `OPEN-6F8-BUSINESS-CATEGORY-SOURCE` 신설(데이터 공백, 이 slice 에서 고치지 않는다).**
실수집 공고 **0건**에 업무구분 코드가 채워졌다 — 필드 계약이 가리키는 원시 키가 공사·용역 목록 응답에 **아예 없다**(키 존재율 0).
응답에 있는 후보 키(공공조달분류 번호·이름)는 용역에만 채워진다. 그래서 전략의 「관심 업종」 규칙은 **지금 어떤 공고와도 맞지 않는다.**
어느 키를 업종 축으로 쓸지는 정책 데이터(운영자 승인) 결정이다 — 선택지와 추천을 OPEN 에 적어 운영자에게 올린다.

**D-6F8-13 — verifier r2 MEDIUM·LOW 처분(같은 라운드).** F2-1 NUL 문자: 실수집에서 `22P05|22021` 0회 — MEDIUM 유지·알려진 제한 22 를
「원문 저장 단계 `22P05`, 원문도 잃음」으로 정정하고 `OPEN-6F8-NUL-CHARACTER` 신설. F2-2 리플렉션 우회: `workflow`·`app` production 에
리플렉션 API 참조 금지 규칙(허용 집합 ∅) — 게이트 술어 변경이므로 표적 재검증. F2-3·F2-4·F2-5: 문면 정정 또는 등재.

**실수집 사실(이 slice 의 알려진 제한에 등재)**: 원문 행 **전부**에 KONEPS 담당자 키가 있다(`OPEN-6F8-RAW-PII-RETENTION` 가 그대로
실측됐다 — 6B-3). 필드 계약 밖 키가 항목마다 백여 개(미지 필드 계수) — 계약은 필요한 필드만 나른다(설계대로).
