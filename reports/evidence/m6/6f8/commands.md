...
## 실수집 (D-6F8-5, 팀장 실행 — 건수·비율만, 원문·키 없음)

KONEPS 실 호출(읽기 전용), 범위 최근 30일(KST, 조회일 31 × 공사·용역 = 슬롯 62), V2 전용 개발 DB(docker postgres:16.4, 127.0.0.1 만).
키는 legacy `.env` 원문형 변수를 서브셸에서 읽어 그 프로세스 환경에만 넘겼다(checklist 6항 형태, 명령 문자열·argv 에 값 없음).

| 실행 | jar(SHA) | exit | 슬롯 · 절단 · 멈춤 | 페이지 | 수신 | 정규화 | 중복 | 탈락 | 새 notice |
|---|---|---|---|---|---|---|---|---|---|
| 1차 | `38510674` | 0 | 62 · 0 · 없음 | 265 | 22,639 | 20,869 | 0 | 1,770(전부 `DATE_TIME`) | 20,869 |
| 2차(멱등) | `38510674` | 0 | 62 · 0 · 없음 | 265 | 22,639 | 0 | 20,869 | 1,770 | 0 |
| 3차(D-6F8-11 수정 뒤) | `bf7ea5ca` | 0 | 62 · 0 · 없음 | 265 | 22,639 | 1,770 | 20,869 | **0** | 1,770 |

- 회계 등식(수신 = 정규화 + 중복 + 탈락)은 세 실행 · 슬롯 62 전부에서 성립. 2차의 슬롯별 멱등 등식(`normalized = 0` · `duplicate = 1차
  normalized + 1차 duplicate` · `unchanged = 1차 inserted + updated`) 위반 슬롯 0, 두 실행 사이 수신 변화 슬롯 0.
- 1차 탈락의 1,765건은 빈 입찰마감일시(OPTIONAL) — D-6F8-11 로 부재 처리, 3차에서 탈락 0(정체를 모르던 5건도 함께 사라졌다 —
  checklist 5d 의 분류 질의가 잴 대상이 남지 않았다).
- 최종 DB: `notice` 22,639(= 수신) · 공고명 비어 있지 않음 **100%** · 마감 `null` 1,765(후보 조건상 평가에 오르지 않음 — 알려진 제한 26) ·
  업무구분 코드 채움 **0%**(`OPEN-6F8-BUSINESS-CATEGORY-SOURCE`) · `collection_run` 186(= 62 × 3) · `raw_observation` 67,917(관측마다 추가 전용).
- 담당자 키가 있는 원문 행: **전부**(`OPEN-6F8-RAW-PII-RETENTION`).
- 로그의 키 흔적(64자 16진 패턴) 0 · `22P05|22021`·`StructureFailure`·`QuotaExhausted` 0 — 세 실행 모두.
- **평가 dry-run**(1차 뒤, 같은 DB, `bf7ea5ca` 이전 jar `38510674`, 전략: 키워드 「도로」 · 여력 상한 5 · 현재 2 · 임계 0.5/0.8):
  HTTP 200 · 후보 4,912(= 마감이 미래인 공고 수) · review 163(= 마감 전이면서 공고명에 「도로」가 있는 공고 수) · notReached 4,749 ·
  bidNow 0 · wouldNotify 0(ML 자리지킴) · 배열 합 = 후보 · 서로소 · wouldNotify == bidNow · 무인증 401.
- 개발 DB 폐기: `docker rm -f -v bid-vector-v2-dev`(볼륨 `bid-vector-v2-dev-data` 포함) — 운영 데이터 아님.
