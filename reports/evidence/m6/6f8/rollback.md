...
`실측 HEAD: 66589eda`(이 slice 의 마지막 산출물 커밋 — 이후 커밋은 evidence 전용). 앞 라운드 실측을 옮기지 않는다.
...
신규(A) 30 · 변경(M) 7 = 37개. **라운드마다 이 명령을 다시 돌려 목록을 재산출한다**(목록이 낡는 것이 이 결함의 실제 원인이다).
...
|---|---|
| `config/quality/gate-tests.properties` | `c5c13fde` · `09e13abc` · `7becb41a` · `defcc09d` · `4504483a` |
| `config/quality/architecture-policy.properties` | `09e13abc` |
...
## 임시 worktree 실측 (①~⑥, 버릴 worktree — `git worktree add --detach <scratch> 66589eda`)

| 축 | 결과 |
|---|---|
| ① `git restore` exit / hunk 격리 exit | 0 / 일곱 번 전부 0 |
| ② D/M 수 | D 30 · M 7 (위 목록과 일치) + 공유 파일 셋 hunk 되돌림(충돌 0) |
| ③ 되돌린 경로 전체(목록 37 + 공유 셋)의 `git diff <base>` | 0줄(완전 일치) |
...
「내 줄이 사라졌다」와 「남의 줄이 남았다」를 **둘 다** 쟀다: 공유 셋을 되돌린 트리에서 이 slice 의 주석·등재 항목 grep 은 세 파일 모두 0건, `gate-tests.properties` 의 등재 항목 수는 base 와 같다
...
```
git diff --name-only 66589eda..<판정 SHA> -- <위 목록 37개 경로> config/quality/gate-tests.properties config/quality/architecture-policy.properties docs/discovery/data-dictionary.md
```
