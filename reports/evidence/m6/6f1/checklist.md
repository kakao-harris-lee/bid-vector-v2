...
**verifier r4 기록(팀장 지시)**: r4 는 이 20/20 축을 **재판정하지 않았다** — 코드를
읽고 r3 의 실측(MUT-8·MUT-9 재현)을 그대로 인정했을 뿐, 이 라운드에서 새 변이를 다시
돌리지는 않았다. **읽기로 확인된 축이지 이 라운드의 실행으로 재확인된 축이 아니다** —
확인 불가를 확인한 것처럼 적지 않는다.
