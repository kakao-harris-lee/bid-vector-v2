...
in_scope:
  ...
  - adapters/src/test/kotlin/bidvector/adapters/persistence/JdbcStrategyRepositoryCodexRegressionTest.kt   # 신설(계약 갱신 (9)): Codex 심판 HIGH·MEDIUM 회귀 중 원시 SQL 로 도메인 경로를 우회하는 자리 — sizeGate(파일 500줄)로 위 파일에서 분리
...
| **D-6F1-9** (계약 갱신 (8), Codex 심판 HIGH) | 예산 `won`(`NUMERIC(20,0)`, 20자리)을 `Long`(19자리 상한)으로 복원할 때 `toLong()`이 범위 초과 시 예외 없이 하위 64비트만 남겨 다른 금액이 됐다(그 상태로 검증을 통과). **심층 방어 둘**: ① V9 두 표의 `min/max_budget_won`에 범위 `CHECK(0..Long.MAX_VALUE)` 추가(새 쓰기를 DB 층이 막는다) ② `StrategyRow.toExactStrategyWon()`이 `longValueExact()`로 정확히 변환하고 실패하면 원인 예외를 실어 `IllegalStateException`을 던진다(CHECK 이전에 저장된 값·CHECK 우회를 코드 층이 막는다 — CHECK 하나만 믿지 않는다) | CHECK 만으로는 **이미 저장된 값**(대역 밖 쓰기·CHECK 신설 이전 데이터)을 막지 못한다 — DB 와 코드 양쪽이 독립으로 막아야 한다(팀장 지시). 버릴 clone 변이로 셋을 각각 실측했다: CHECK 를 런타임에 드롭해도 코드가 막음 / 코드 방어만 원복하면 Codex 재현값(`18446744073709551621`)이 조용히 `won=5`로 통과함(원 결함 재현) / 두 방어 모두 있으면 어느 쪽을 빼도 회귀 test 가 붉어짐 |
| **D-6F1-10** (계약 갱신 (8), Codex 심판 MEDIUM 처분 변경) | `getTextList()`가 `TEXT[]` 원소의 NULL 을 `filterNotNull()`로 조용히 걸렀다 — 컬럼의 `NOT NULL`은 원소를 막지 않아 축소된 감시 규칙이 유효 전략처럼 통과했다. **처분을 「알려진 제한 등재」에서 「닫는다」로 변경**(Codex 심판이 r2/r3 verifier와 다르게 판정) — 원소에 NULL 이 있으면 `check()`로 크게 실패한다(빈 문자열의 `BlankTerm`과 같은 자리, 같은 방향). 표 제약(`array_position(...) IS NULL`류 CHECK, 배열 다섯 컬럼 × 두 표 = 열 개)은 **더하지 않았다** — `getTextList()`가 이 값을 읽는 유일한 production 경로라 코드 층 방어만으로 도달 불가능이 보장되고, 열 개 CHECK 는 이번 라운드의 스키마 축 변경(D-6F1-9 의 won 범위 CHECK 둘)에 비해 침습이 크다(판단은 팀장 재량으로 위임됨, 완료 보고에 근거 상술) | `getTextList()`는 `StrategyRow.kt` 다섯 호출이 유일한 production 소비자(grep 확인) — 표 제약을 더해도 코드 층 검사가 이미 같은 실패를 내므로 중복 방어이고, 열 개 CHECK 신설이 주는 한계 효용이 침습 비용보다 낮다고 판단했다 |
| **D-6F1-11** (계약 갱신 (9), verifier r4 MEDIUM-3) | D-6F1-9 의 「둘 다 있어야 닫힌다」에서 **코드 층이 실제로는 무방비였다** — CHECK 가 있는 한 범위 밖 값이 코드 층까지 갈 길이 없어, 그 층만 겨누는 회귀 test 가 없으면 정확 변환을 되돌려도(`toLong()`) 전건이 초록이었다(D-6F1-9 의 실측 서술은 커밋된 test 가 아니라 버릴 clone 의 일회성 실험이었다). **닫는다** — `JdbcStrategyRepositoryCodexRegressionTest`에 CHECK 를 런타임에 떼는 test 를 신설(신설 파일, sizeGate 로 `JdbcStrategyRepositoryTest.kt`에서 분리). `@AfterEach`로 그 CHECK 를 되건다 — 컨테이너가 class 간 공유돼 `@BeforeEach` 의 TRUNCATE 만으로는 DDL 이 복구되지 않는다(되걸기 전에 위반 행도 비워야 한다, 실측으로 확인) | 게이트 술어를 더하는 자리라 severity 무관 표적 재검증 대상(팀장 지시). 정확 변환을 되돌리는 변이로 그 test 만 붉어지고 나머지는 CHECK 가 여전히 있어 초록임을 실측, `@AfterEach` 복구 뒤 클래스 전체를 3회 연속 재실행해 오염 없음을 확인 |
...
| `OPEN-6F1-WON-OVERFLOW-REPO-WIDE`(신설, verifier r4 등재) | 같은 `NUMERIC(20,0)` ↔ `Long` 접힘 패턴이 이 slice 밖 **파일 셋에 아홉 자리** 더 있다(공고 복원 넷·개찰 결과 저장소 넷·개찰완료 축 코덱 하나 — 저장소 전역, 좌표는 다른 slice 가 그 파일을 만지면 낡으므로 여기 적지 않는다). **이 slice 밖, 고치지 않는다.** 이번 slice(D-6F1-9)가 닫은 처방을 그 아홉에 적용할 때의 본으로 남긴다: **DB CHECK(범위) + 코드 `longValueExact()`(정확 변환, 원인 예외 포함) 이중 방어**, 그리고 그 코드 층을 실제로 겨누려면 **CHECK 를 런타임에 떼는 회귀 test 가 따로 필요하다**(D-6F1-11 — CHECK 가 있으면 코드 층까지 값이 못 간다) | 저장소 전역 부채이고 이 slice 의 in_scope 파일이 아니다. 처방 한 줄만 남겨 다음 slice/라운드가 아홉을 닫을 때 재발견 비용을 줄인다 |
...
### rebase 실측 — 예측 대조 (2026-09-18, 계약 갱신 (6))

위 「절차」대로 `origin/main`(6B-1 의 `V8` 포함) 위로 실제 rebase 를 수행했다. **예측을 지우지 않고** 그
예측이 실측에서 어떻게 확인됐는지를 아래에 덧붙인다 — **이번엔 예측이 전부 맞았다**(충돌 위치·해소 형태·
커밋 수·최종 해시까지). 다음 slice 를 위해 이 사실 자체를 남긴다: 「병행 레인이 같은 파일군을 만지면 병합
전 대조를 버릴 clone 에서 먼저 해 둔다」 절차의 근거가 이번 실측이다.

- **커밋 수**: rebase 재생 중 위 위험 ①의 「먼저 병합되는 레인 소유」가 실무적으로 실현됐다 — 신설 커밋이
  6B-1 판 의존 게이트 파일을 add/add 로 채택하고 그 뒤의 **철회 커밋을 거부**해 **20 → 18**로 줄었다(받아들
  였다면 6B-1 의 게이트가 지워졌을 것이다. **이 자리는 merge 가 아니라 rebase 만 만든다**)
- **멈춤 넷** — 위 위험 ①이 예고한 파일 셋 + 예고하지 않았던 test 정리 목록 하나가 같은 범주(exact-match
  목록의 위치 충돌)로 추가 확인됐다:
  1. test 정리 목록 — 두 레인이 각자 표를 추가해 충돌 → **합집합**(한 호출에 세 표 전부)으로 해소
  2. clean-migration 세 파일(표 목록·PK·CHECK) 동시 충돌 → 표 목록·PK 집합·CHECK 개수는 **합집합**. 컬럼
     축 합성식은 위 위험 ①의 「양쪽이 같은 이름의 property 를 각자 추가해 중복 선언」 그대로 **양쪽이 같은
     식의 끝을 각자 고쳐** 수동 병합이 필요했다(세 항이 전부 살아야 한다 — 하나라도 빠지면 그 표의 컬럼 축
     검사가 사라진다)
  3. 의존 게이트 파일 **add/add** — 위 위험 ①의 「먼저 병합되는 레인 소유」대로 **6B-1 판 채택**. 최종 파일이
     `origin/main` 판과 **diff 0줄**, md5 가 임시 clone 리허설 실측값과 일치
  4. 뒤따르는 **철회 커밋**이 `UD`(자기 수정/상대 삭제)로 멈춤 — **거부(skip)**. 받아들이면 6B-1 의 게이트가
     지워진다 — 위 3과 한 벌
- **rebase 뒤 마무리 둘**(절 제목 「rebase 마무리」커밋) — 컬럼 축 기본값 표기 두 행 · 세션 fake 반환 타입,
  위 위험 ②가 예고하고 임시 clone 리허설이 실측한 그대로
- **rebase 직후 전건**: `check` → **563 tests, 1 failed** — 그 1건이 계약 갱신 (5)·D-6F1-8 에서 고친 의존
  위반 하나뿐이었다(이 라운드에서 수정 뒤 재실측 exit 0, 상세는 완료 보고). 나머지 축은 전부 통과했고, V8·
  V9 의 표가 같은 게이트에 **공존**한다

## 계약 갱신 이력

| 일자 | 갱신 | 사유 |
| --- | --- | --- |
| 2026-09-18 verifier r4 뒤(9) | **D-6F1-11 신설**(MEDIUM-3, 이중 방어의 코드 층 무방비) — `JdbcStrategyRepositoryCodexRegressionTest.kt` 신설(in_scope 등재, sizeGate 로 분리) 하여 CHECK 를 런타임에 떼는 회귀 test·`@AfterEach` DDL 원복을 더함. `OPEN-6F1-WON-OVERFLOW-REPO-WIDE` 신설(이 slice 밖 아홉 자리, 좌표 없이 처방만 등재) | verifier r4 판정 ready-for-review, 승인 전 배치: 신설 회귀 test 가 CHECK 층만 재고 있어 정확 변환을 되돌리거나 그 함수를 통째로 지워도 전건이 초록이었다(D-6F1-9 의 「둘 다 있어야 닫힌다」를 지키는 것은 하나뿐이었다) |
| 2026-09-18 Codex 심판 뒤(8) | **request_changes 처분** — D-6F1-9(HIGH, 예산 won Long 접힘) 신설, D-6F1-10(MEDIUM, 배열 NULL 원소 — 처분을 「등재만」에서 「닫는다」로 변경) 신설. `commands.md` 낡는 base 문자열 제거(LOW-B), `checklist.md` 읽기 순서 정정(LOW-C) | Codex 독립 심판 verdict `request_changes`(BLOCKER 0·HIGH 1·MEDIUM 1) — 완료 보고에 실측 상술 |
| 2026-09-18 verifier r2 뒤(7) | **HIGH-1 수정** — `base_sha` 를 `c4d09cc` → `8652893`(rebase 뒤 값)로 갱신, `rollback:` 블록·「하네스 레인 변경」 절의 명령 base 를 같게 갱신, `rollback.md` 의 되돌릴 대상 목록을 새 base 기준으로 재산출. LOW-1(`ProvenanceCodec.kt` KDoc 의 결정 ID `D-6F1-5`→`D-6F1-8`)·LOW-3(in_scope 의 `StrategyAdapterDependencyTest.kt` 중복 행 병합)·MEDIUM-2(이력 표 payload 컬럼 왕복 test 신설, `JdbcStrategyRepositoryTest`)도 이 라운드에서 처리. MEDIUM-1 은 알려진 제한으로 등재(checklist.md) | verifier r2 판정 **not-ready**(HIGH-1). **팀장 오류**로 기록: rebase 지시(계약 갱신 (6))에 `base_sha`·rollback 재산출을 함께 지시하지 않아, 문서대로 rollback 을 실행하면 6B-1 이 병합한 의존 게이트 파일(이번 라운드 D-6F1-8 결함을 잡은 그 파일)이 삭제되고 컴파일이 깨짐을 verifier 가 버릴 clone 에서 실행 실측했다 |
