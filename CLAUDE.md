...
| 2026-09-12 | **부분 게이트를 전건 대신 돌리지 않는다** — 리뷰·push 전 정본은 CI 가 돌리는 명령 그대로(`./gradlew --no-daemon check`)이고, 관련 게이트만 돌린 것은 **안 돌린 것과 같게** 취급 | evidence-pack | M4 PR #7 — 주석 전용 계약 변경이라 `contractGate` 만 돌려 exit 0 을 보고 push 했는데 CI 를 떨어뜨린 것은 `:leakPatternGate` 였다. 2026-09-09 「상시 붉은 게이트는 아무것도 막지 못한다」의 뒷면 — **안 돌린 게이트도 아무것도 막지 못한다** |
