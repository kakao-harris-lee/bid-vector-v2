...
| 2026-09-16 | 공유 파일 rollback 에 **「같은 slice 의 자기 이력은 착수 경계 기준 단일 역적용」** 추가 — 다른 slice 의 줄이 없는 파일(계약·정책 표)은 커밋별 hunk 격리 대신 `git diff <착수 커밋> -- <파일> \| git apply -R` 한 번, 판별은 author 가 아니라 내용 | evidence-pack | M5/5C-2 실측 — `scope.md` 를 구현 레인 2·팀장 3 커밋이 같은 줄을 이어 만져 커밋별 역적용이 conflict 로 깨짐(verifier r2 LOW → 구현 레인 정정 → r3 「규격과 어긋나지 않는다」 판정). 2026-09-09·10 의 hunk 격리 규정은 **두 slice** 의 겹침만 다뤘고 **한 slice 안의 두 레인** 겹침을 안 다뤘다 |
