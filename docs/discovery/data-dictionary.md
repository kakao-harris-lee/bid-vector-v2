...
이 규칙이 적용되는 legacy 자리(전수 목록은 `commands.md` **C-7.3**이 낸다): 정산 금액·율,
기초금액, 투찰율, 시공능력평가금액, 도급한도, 역량 점수, 페이퍼 투찰 금액·율.
...
> 흩어져 있다(상태 컬럼의 전수는 `commands.md` **C-7.4**가 낸다). `v2-지침서.md` §4.5가
...
`OPEN-QUAL-11`은 이 등재로 **해소**된다(§11). **`OPEN-QUAL-09`(시공능력 요건 미달을 즉시
실격으로 볼 것인가, 적격심사 감점으로 볼 것인가)는 별개로 유지**된다 — 그것은 결합 규칙이
아니라 판정 결과의 문제다.
...
| **`OPEN-QUAL-11`** (§12) | `permsnIndstrytyList`를 포함한 요건의 결합 규칙 | **U-8**(그룹 간 OR / 그룹 내 AND, 결측 행은 단일 그룹 AND) | **§3.2.2** |
