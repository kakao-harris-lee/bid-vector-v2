...
>   provenance를 읽는 자리를 이 문서가 두지 않기 때문이다. 사정률 축이 값으로 서는 자리는
>   §1.4.3의 **값 밴드 필터**(**B1**·**B2**·**B3**)와 §7의 **행 단위 오염 측정**, 그리고
>   §3.3의 `criticalAssessmentRate`인데, 앞 둘은 **값과 행**을 읽고 마지막은 **두 `Money`의
>   몫이 아니라 추천 투찰율 ÷ 낙찰하한율**이다. **나를 수 없는 요구가 아니라 자리를 잘못
>   지목한 요구였다.**
>
...
**일곱을 등록했고 일곱 다 활성이다.** **그중 셋은 이 slice가 등재한 운영자 결정의
미완이 직접 만든 것**이며, 아래 표의 **「어느 결정의 잔여인가」** 칸이 그것을 적는다.
**결정을 물어 답을 받은 것과 그 축에 남은 것이 없는 것은 다르다** — 같은 U-묶음이
활성 `OPEN`을 일곱 개 만들었다.
