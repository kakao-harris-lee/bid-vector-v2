...
| **`OPEN-QUAL-10`의 운영자 보유액 축만** (§12) | 운영자가 보유한 시공능력평가금액의 unit·basis·과세 처리·기준 시점 | **과세 처리 = U-2**(부가세 포함) · **기준 시점 = U-2b**(직전 연도 공시값) + 0C의 기준일 규칙(공고일) · **basis = 개념 축**(§1.2) · **unit = 두 결정이 정하지 않는다 — 아래 문단이 근거를 댄다** | **§1.2 · §1.2.4** |
