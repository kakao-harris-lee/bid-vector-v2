...
§12.1이 적는 legacy 반올림 자리수 가운데 **금액 축의
것은 위 정의 ①이 닫았고**(그래서 V2가 그 자리수를 쓰지 않는다), **금액 축 밖의 것은 값이
미결이라 이 `OPEN`이 소유한다** — 승계할지 여부가 그 미결 안에 있다. **경계가 그 미결에 든
것은 legacy에 「집계」이면서 「금액」인 자리가 실재하기 때문이다** —
`app/services/opportunity_analysis/market.py:91`이 금액을 평균 내며 자리수 `2`로 반올림한다.
**그래서 「집계」가 곧 금액 축 밖은 아니다.**
