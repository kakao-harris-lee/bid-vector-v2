...
| **시공능력평가금액** | `ConstructionCapacityAmount` | 원 / 시공능력평가액 | **`Inclusive`** | `authoritative` — 운영자 결정 (**U-2**) |
...
한 축의 율을 다른 축에 대입할 수 없게 타입을 나눈다.

| 타입 | 분자 / 분모 | 단위 |
| --- | --- | --- |
| `AssessmentRate` (사정률) | 예정가 / 기초금액 | fraction |
| `AwardRate` (낙찰률) | 낙찰가 / 기초금액 | fraction |
| `FloorRate` (낙찰하한율) | **예정가격 기준** 율 | fraction |
| `BidRate` (투찰율) | 투찰가 / 기초금액 | fraction |
...
**이 결정이 닫는 `OPEN`은 없다 — 이 절이 그 결정을 소유한다.**
...
> 닫는가뿐이며, 이 결합 축을 소유한 `OPEN`은 따로 없었다. 정정의 근거와 운영자 판정은
> §11이 적는다.
...
- **수집 신설은 M1 이후로 인계**하고, 그때까지 금액 capacity 판정은 비교 대상 부재로
...
`OPEN-QUAL-09` · `OPEN-QUAL-11` · `OPEN-ML-05` · `OPEN-SET-04` · `OPEN-SET-05` ·
`OPEN-SET-06` · `OPEN-SET-10` · `OPEN-DEC-07` · `OPEN-DEC-10` · `OPEN-OPS-10` ·
`OPEN-NUM-01` · `OPEN-REG-04` — 에 **정의를 붙이지 않았다.**
**`OPEN-QUAL-11`은 앞서 이 목록 밖에 있었다** — §11이 U-8로 닫혔다고 적었기 때문이며,
그 귀속이 틀렸다(§11의 ⚠ 정정). **어느 id가 어느 상태인지의 판정은 이 산문이 아니라
...
| **`OPEN-QUAL-10`** (§12) | 시공능력평가금액의 unit·basis·과세 처리·기준 시점 | **U-2**(부가세 포함) + **U-2b**(직전 연도 공시값) + 0C의 기준일 규칙(공고일) | **§1.2 · §1.2.4** |
| **`OPEN-QUAL-06`** (§12) | `Uncertain` 판정의 분해와 사유 목록 | **U-5**(`UncertainReason` 네 값) | **§3.2.3** |

> **⚠ 정정 (2026-08-30, Codex 3차 high #1)** — 이 표에 **`OPEN-QUAL-11` 행이 있었고
...
> **U-8 자체는 유효하고 §3.2.2가 그 결정을 소유한다** — 바뀐 것은 **그 결정이 어느
> `OPEN`을 닫는가**뿐이고, **U-8이 닫는 `OPEN`은 없다.**
>
...
> **§7.1이 요구한 분자도 그 분자가 딛는 분모도 갖지 않은 채** `C-4.2`가 `PASS`를 냈다
