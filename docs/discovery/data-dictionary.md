...
> 그 행의 질문 전부에 답하지 않았다. `docs/adr/0002:185`가 `OPEN-REG-05`를 **이 ADR이
> 선점하지 않는 활성 `OPEN`**으로 들고 `:202`가 *"닫힌 뒤에 만든다"*로 적는 것과 이제
> 한목소리다. **U-1·U-1b의 결정 내용은 바뀌지 않는다** — 바뀐 것은 **그 결정이 어느
