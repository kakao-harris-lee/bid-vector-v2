...
### D-8. 재활용 출처를 기록한다 — 위치는 ADR 0009가 소유한다

`v2-지침서.md` §3.2가 *"각 모듈에 재활용 출처(원본 파일 경로와 기준 commit)와 수행한
튜닝·수정 내용을 기록한다"*를 요구한다. **기록 의무는 이 ADR이 확정**하고,
**기록 위치는 ADR 0009**가 소유한다 — 그 ADR이 **운영자 결정 2026-08-29**로 (d) 조합을
확정했다. **이 ADR은 위치를 주장하지 않는다.**
...
### ~~`OPEN-ADR-01`~~ · Spring Boot 세대 — **해소**

- **결정**: **(b) Boot 4.x로 이동.** 아래 선택지 셋 중 (b)다.
- **결정자**: **운영자.** **시점**: **2026-08-29.** **전달 경로**: 팀 리드 레인 →
  spec-writer 지시 메시지(**Q1**). `ADR 0009` §5 `OPEN-ADR-10`과 같은 형태로,
  `_workspace/`에 대응 원본이 없어 **커밋된 산출물이 저장소 안 기록**이다 —
  `v2-지침서.md` §5와 `reports/evidence/m0/0e/scope.md`.
- **종료 조건과 그 충족**: 종료 조건은 **`v2-지침서.md` §5 문면 개정**이었다. 그 문면이
  `3.x`인 한 1A의 version catalog가 무엇을 써도 승인된 기준 문서와 어긋났기 때문이다.
  **M0 종료 slice 0E가 그 개정을 집행했다** — 같은 절이 이제 *"Kotlin 2.x와 현재 지원되는
  Spring Boot **4.x** 조합을 M1에서 버전 고정한다"*라 적는다. **결정과 종료 조건이 둘 다
  충족됐으므로 이 항목은 활성 registry에서 내린다.**
- **남는 것 — 이 해소가 담지 않는 것**: **나머지 라이브러리의 4.x 호환 실측 재확인**이
  남는다. `OPEN-OPS-07` 조사와 §3의 판정이 **3.x 전제 위에서** 이뤄졌기 때문이다.
  **그것은 이 `OPEN`의 답이 아니며 새 `OPEN`을 만들지도 않는다** — 소유는
  **`v2-지침서.md` §5가 이 개정과 함께 신설한 항목**(*"나머지 라이브러리의 4.x 호환은
  M1 slice 1A가 스모크 빌드로 실측 재확인한다"*)이다. **새 `OPEN`을 만들지 않는 근거 셋**:
  ① `OPEN`은 **결정 필요 사항**인데 여기에는 고를 것이 없다 — 할 일과 주체가 이미 정해진
  **측정**이다. ② 같은 형태의 선례가 이 ADR 묶음 안에 있다 — `ADR 0004`가 Flyway 호환을
  *"`OPEN-ADR-01`(Boot 세대)과 함께 **M1 slice 1A의 스모크 빌드**에서 확인"*으로 넘기며
  `OPEN`을 만들지 않았다. ③ 그 축에 **실제로 남은 결정은 이미 등록돼 있다** —
  `OPEN-ADR-08`(Detekt 버전 경로, 선택지 (i)~(iv), **소유: M1 버전 고정**)이 그것이며,
  Boot 4.x가 Kotlin 하한을 올리므로 그 선택지 판정을 1A가 실측과 함께 내린다.
  **측정을 `OPEN`으로 올리면 활성 registry가 결정 없는 항목으로 부풀고, 그것이
  `capability-map.md` §12.1이 경계한 계상 오류의 반대 방향이다.**

**아래는 미결 당시의 기록이며 지우지 않는다.**

- **결정 필요 사항**: V2가 어느 Spring Boot 라인으로 간다고 전제하는가.
- **왜 미결이었는가**: `v2-지침서.md` §5는 *"Kotlin 2.x와 **현재 지원되는** Spring Boot 3.x
