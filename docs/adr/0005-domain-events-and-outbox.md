...
덮는다고 확인되지 않았다.** **ShedLock에 대해서는 이번 라운드에 「덮지 않는다」가 공식
문서로 확인**됐고(요구 ② 불충족), 나머지 둘은 여전히 **미조사·미확인**이다. 아래 표는
...
- **db-scheduler의 락 해제 기제를 공식 문서로 확인하지 않았다**(§3.2 요구 ②).
  ...
  **ShedLock 쪽은 이 라운드에 공식 README로 확인했다** — 더 이상 이 목록에 있지 않다.
