...
| **domain** | ~~`bidding`~~ | 투찰 계획과 상태 — **보류. 아래 D-2.1** |
...
### D-2.1. `bidding`은 소유 capability가 정본에 설 때 추가한다 (운영자 결정 2026-09-02)

**결정**: `bidding`을 **지금 만들지 않는다.** `docs/discovery/capability-map.md`에 「투찰 계획과
상태」를 **소유하는 capability가 서면** 그때 D-2 표에 되살린다. 추적은 **`OPEN-ADR-14`**다.

- **결정자**: **운영자.** **시점**: **2026-09-02.** 계기는 M1 slice 1A의 구현과 그에 대한
  Codex 1차 리뷰 finding #1이다.
- **왜 이 개정이 필요했는가**: 1A가 아홉 모듈만 세우고 그 근거를 evidence와 `OPEN-ADR-14`에만
  적었다. **그것은 discovery 문서가 승인된 ADR을 대체한 상태**이고, 이 표가 열을 요구하는 한
  구현·정책·테스트가 승인 문서와 어긋난 채로 남는다. **정본을 고치는 것이 옳은 순서다.**
- **근거**: `capability-map.md`의 capability 가운데 「투찰 계획과 상태」를 **소유하는 항목이
  없다.** 투찰이 붙은 제목은 STR-16(요청 경로) · DEC-02/05(산출 로직) · SET-03/04(**둘 다
  `근거 부족`**) · SET-10(설계 입력)이고 소유는 각각 다른 축에 있다. 반면 `V2 필수`인 SET-01·
  SET-02는 「실투찰 레코드」를 **전제**하며 SET-02의 분류 줄이 *"투찰 capability와 함께여야
  의미가 있다 — 의존 명시 필요"*라 스스로 적는다. **전제는 있고 소유자가 없다.**
- **`milestone-1.md` 1A 항목과 정합한다** — *"M0 필수 capability에 없는 모듈은 만들지 않는다"*.
  같은 문서의 1A 모듈 목록도 이 결정에 맞춰 함께 개정했다.
- **D-5(목록 밖 신설 금지)의 예외가 아니다.** D-5가 막는 것은 **열한 번째 모듈을 더하는 것**이고
  이 결정은 목록에서 **빼는 것**이다. 되살릴 때도 D-5를 거치지 않는다 — 이 표의 원소이기 때문이다.
- **되살리는 조건**: `OPEN-ADR-14`의 선택지 (c)가 답이 되어 소유 capability가 등재되고 그
  분류(`V2 필수` 등)를 받는 것. 그때 이 절을 지우고 D-2 표의 취소선을 걷는다.

**아래 D-3 이후의 층·의존 규칙은 이 결정으로 바뀌지 않는다** — `bidding`은 domain 층이었고,
domain 층의 규칙은 나머지 여섯 domain 모듈에 그대로 적용된다.
