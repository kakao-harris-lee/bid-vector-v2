...
**이 ADR이 정하지 않는 것**: **어떤 값이 정책 데이터이고 어떤 값이 환경 설정인지의
분류**는 정하지 않는다. 활성 **`OPEN-ML-05`**가 그 축을 들고 있다 — *"predictor 정책 값
33개 중 어디까지가 versioned policy이고 어디까지가 환경 설정인가"*. 여기서 정하는 것은
**분류가 끝난 뒤 그 데이터가 어느 모듈에 속하는가**뿐이다.
