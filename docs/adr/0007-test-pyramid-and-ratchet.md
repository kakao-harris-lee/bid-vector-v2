...
### ~~`OPEN-ADR-06`~~ · 래칫 축에 클래스/타입 크기를 추가하는가 — **해소**

- **결정**: **(a) 클래스/타입 크기 축을 추가한다** — 운영자 결정 **2026-09-05, M1 1B 종료
  시점**(약속대로 `OPEN-ADR-16`과 함께). 형태 둘: ① `sizeGate`에 **타입 멤버 수 상한 30**을
  더한다 — 함수 50·파일 500과 같은 자리, Kotlin PSI **소스** 기준이라 `qualityBaseline`의
  바이트코드 한계(`internal`이 public으로 보임)를 물려받지 않는다. ② **상속 깊이·구현
  인터페이스 수는 임계가 아니라 래칫**이다 — 첫 도메인 baseline(shared-kernel: 깊이 2 ·
  인터페이스 1) 대비 증가를 실패로 내고, 증가가 필요하면 이 ADR을 개정한다.
- **근거 실측**: 1B 종료 시점 `qualityBaseline`(shared-kernel) — 타입 64 · 최대 멤버 20 ·
  최대 깊이 2 · 최대 인터페이스 1. 상한 30은 최대 20의 1.5배로 운영자가 정했다(1A D-6의
...
- **소유**: M1 래칫 구현.
- **결정자·시점**: 운영자, **`OPEN-ADR-16`과 함께 M1 1B 종료 시점에 결정**(운영자 결정
  2026-09-04, M1/1B 계약 갱신). 1A `qualityBaseline`이 이미 실측(아홉 축)을 냈고, 1B가
  첫 도메인 코드로 그 실측에 값을 채운다 — 두 `OPEN`을 같은 시점에 묶는 이유는 둘 다
  「도메인 코드가 있어야 판단할 수 있는 축」이기 때문이다.

### ~~`OPEN-ADR-16`~~ · `duplicate mechanical helper` 축의 측정 정의 — **해소**

- **결정**: **(a) PMD CPD를 채택한다** — 운영자 결정 **2026-09-05, M1 1B 종료 시점**. 배선은
  `de.aaschmid.cpd` Gradle 플러그인(`language = kotlin`), 엔진은 `toolVersion`으로 플러그인
  수명과 독립해 올린다 — 이 ADR의 「게이트를 alpha·정체 도구에 종속시키지 않는다」
  (`OPEN-ADR-08` 해소문) 기준을 그렇게 통과한다. **시작 모드는 관찰**(리포트만, `check`를
  실패시키지 않음) — 첫 도메인 baseline을 본 뒤 **1C 종료 시점**에 실패 모드 전환을
  결정한다. `minimumTokenCount`는 **도구 기본값 50 유지**(수치를 지어내지 않는다 — baseline
  실측 뒤 조정). **첫 배선의 acceptance에 Gradle 9.6.1 스모크**를 둔다(플러그인 마지막
  릴리스 2024-11, Gradle 9 호환 공식 확인 없음) — 실패하면 (b) 자체 판정으로 후퇴한다.
- **도구 조사**(1B 종료 시점, `_workspace/m1-1b/10_tool-survey_duplicate-detection.md`):
  detekt 2.0 alpha에는 중복 탐지 규칙이 없다(issue #789). jscpd·Simian·SonarQube·IntelliJ
  CLI는 각각 Node 런타임·불명 라이선스·상시 서버·IDE 기동이라는 새 표면을 연다. **(b)의
  대가 정정**: 조사서는 「Kotlin 렉서 종속을 새로 들인다」고 적었으나 `build-logic`은 이미
...
- **결정자·시점**: 운영자, **1B가 CPD 등 기존 Kotlin 중복 탐지 도구 조사를 내고 첫 도메인
  baseline과 함께 1B 종료 시점에 `OPEN-ADR-06`과 함께 결정**(운영자 결정 2026-09-04, M1/1B
  계약 갱신 — 시점만 확정하고 선택지 (a)/(b)/(c)는 아직 고르지 않는다). (2026-09-03 신설 ·
  Codex 12차 medium)
