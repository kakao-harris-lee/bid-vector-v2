...
6B-1 병합 이후의 `main` 이고 **전건을 그 시점에 다시 재었다**(`check` exit 0, 모듈 아홉 1,530
tests·실패 0) — 「M4 당시 통과」를 근거로 쓰지 않았다.

충족 셋은 ①전이표와 그 여집합 ②use case 격리(여기서 **구조가 test 보다 강하다** — application
층의 허용 project 의존이 domain 뿐이라 DB·network 구현은 링크 자체가 불가능하다) ③순서 반전·중복
수렴(순수 property + 실 DB 저장 층 양쪽)이다. 좁혀진 넷은 **전부 받는 자리가 있다** — ④고갈 축은
부하 test 가 없어 6D · ⑤**조건이 공허하게 참이다**(발송 port 의 어댑터 구현이 0건, port KDoc 이
스스로 「실 sender 가 없어 fake 로만 증명된다」고 적는다, `OPEN-STR-12`) · ⑥조합 지점에 correlation
id 를 관측하는 test 가 0건이고 사유는 M4 의 결함이 아니라 **배선 부재**(6F 군·6A 뒤 6D) · ⑦구조
축은 컴파일이 강제해 충족이나 **직접 라이브러리 축은 소스 텍스트 스캔 하나**가 막는다.

**알려진 제한 일곱 중 ②(세션 영속·낙관적 동시성 미검증)는 M6/6B-1 이 닫았다.** ③④⑤⑥ 은 열려
있고 ①⑦ 은 이관이 문면에 있다. **제한 ⑥ 이 이 판정의 가장 값있는 발견이다** — M4 는 그 텍스트
술어가 특정 이름 형태를 놓친다고만 선언했는데, 같은 술어 계열이 M6 배선 slice 에서 **반환형으로만
스치는 좌표**를 구조적으로 보지 못해 **실제 위반**을 놓친 것이 실측됐다(바이트코드 상수 풀 게이트가
그것을 잡았다 — 심은 변이가 아니다). 완료 조건 ⑦ 의 약한 층이 그 계열이고, 처방도 같은 곳에
실측이 있다(좌표 8 → 0).

`OPEN` 은 **115건을 배타 넷으로** 갈랐다(종결 36·이월 59·미결 12·상충 7·참조 1, 합 = 전수).
상충 7 은 대개 **정본 표의 갱신 누락**이고 판정이 갈리는 것이 아니다. **종결 승인에는 운영자 판단
묶음 넷이 선행한다** — 상충 문면 정정의 소유, `OPEN-NOTI-06` 의 소유자 공백, 이월 문면이 빈 둘,
미결 12 의 이월처. `checklist.md` 「운영자 판단이 필요한 것」이 정본이다.
