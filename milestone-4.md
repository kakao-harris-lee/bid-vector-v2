...
`OPEN` 은 **115건을 배타 넷으로** 갈랐다(종결 36·이월 59·미결 12·상충 7·참조 1, 합 = 전수).
상충 7 은 **갱신 누락 4 · 그 밖 3** 이다(초판의 「대개 갱신 누락」 단정은 3/7 에서 틀렸다) — 정본
자신이 두 값을 말하는 것 하나, 소유자 공백 하나, 닫는 주체가 셋으로 갈리는 것 하나가 남는다.
**종결 승인에는 운영자 판단 묶음 넷이 선행한다** — 상충 문면 정정의 소유, `OPEN-NOTI-06` 의
소유자 공백, 이월 문면이 빈 둘, 미결 12 의 이월처. `checklist.md` 「운영자 판단이 필요한 것」이
정본이다.
