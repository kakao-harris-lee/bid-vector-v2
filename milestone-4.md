...
**운영자 결정 2026-09-18 — 넷 다 추천안.** ① **최소 종결** — 판정만 확정하고 상충 문면 정정·
미결 이월처 지정·이월 빈 둘의 재분류는 **각 계열 소유 slice** 로 넘긴다(M5 가 이월 14건을 남기고
종결한 전례). 그래서 `docs/discovery/capability-map.md` 와 사전은 이 slice 가 편집하지 않고,
**의도적으로 남는 것 넷을 `checklist.md` 가 선언으로 적는다.** ② **`OPEN-NOTI-06` → 6F-7 이월**
(알림 요청을 outbox 로 잇는 slice 가 그 축의 유일한 실제 소비자다) — 배타 분류가 **종결 36·이월
**60**·미결 12·상충 **6**·참조 1, 합 115** 로 갱신된다. ③ **게이트 공백은 후속 slice 등재만** —
조건 ⑦ 을 미충족으로 내리지 않고 **`OPEN-M4-APP-LAYER-GROUP-GATE`** 를 신설해 추적한다(M4 는
port 경계까지가 범위였고 이 공백은 새로 생긴 것이 아니다). ④ **재검증 통과 뒤 PR.**
