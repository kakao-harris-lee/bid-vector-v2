...
**종결 판정 2026-09-18 — 충족 3 · 좁혀진 4 · 미측정 0 · 미충족 0.** 정본
`reports/evidence/m4/closure/`(`scope.md`·`checklist.md`·`open-inventory.md`). 판정 base 는
6B-1 병합 이후의 `main` 이고 **전건을 그 시점에 다시 재었다** — 버릴 clone 의 깨끗한 실행이
정본이고 `check` **exit 0 · 1,781 tests · 실패 0 · 건너뜀 4** 다. 「M4 당시 통과」를 근거로
쓰지 않았다.

충족 셋은 ①전이표와 그 여집합 ②use case 격리 ③순서 반전·중복 수렴이다. **셋 다 「재는 게이트가
있고 초록」이라는 뜻일 뿐**이고, 초판이 ② 의 근거로 든 「구조가 강제해 DB·network 구현은 링크
자체가 불가능하다」는 **verifier r1 이 변이로 뒤집었다** — 게이트가 막는 것은 `adapters`
**project 링크** 하나이고, JDK `java.sql`·`java.net.http` 를 use case 본문에 인라인하면 의존
선언 없이 전건이 통과한다. 좁혀진 넷은 **전부 받는 자리가 있다** — ④고갈 축은 부하 test 가 없어
6D · ⑤**조건이 공허하게 참이다**(발송 port 의 어댑터 구현이 0건, port KDoc 이 스스로 「실 sender
가 없어 fake 로만 증명된다」고 적는다, `OPEN-STR-12`) · ⑥조합 지점에 correlation id 를 관측하는
test 가 0건이고 사유는 M4 의 결함이 아니라 **배선 부재**(6F 군·6A 뒤 6D) · ⑦**project 의존 축만**
충족이고 직접 라이브러리·JDK 축은 소스 텍스트 스캔 하나가 막는데 그 술어가 `java` 루트를 설계상
허용한다.

**이 판정이 실측으로 배운 것**: `group.forbidden` 과 domain 외부 allow-list 가 **domain 층에만**
걸려 **application 층에는 금지 group 게이트가 없다.** 완료 조건 ⑦ 이 문면으로 금지한 형태의
인라인 변종이 게이트 전건을 통과한다 — 후속 slice 로 등재한다(이 slice 는 판정이라 고치지 않는다).

**알려진 제한 일곱 중 ②(세션 영속·낙관적 동시성 미검증)는 M6/6B-1 이 닫았다.** ③④⑤⑥ 은 열려
있고 ①⑦ 은 이관이 문면에 있다. **제한 ⑥ 은 열려 있고 그 술어 계열이 뚫린 기록이 base 에 있다** —
6B-1 에서 **`import` 없이 전체 한정 좌표로 직접 참조**하면 소스 텍스트 술어를 통과한다는 실측이며
그것은 **verifier 가 심은 변이**다. 그 slice 는 술어를 **바이트코드 상수 풀**로 옮겨 닫았고, 같은
처방이 조건 ⑦ 의 약한 층에 적용된다. 초판은 여기에 **아직 병합되지 않은 다른 레인의 워크스페이스
사실**을 「가장 값있는 발견」으로 실었는데 그 근거가 이 base 에 0건이어서 **확인 불가로 내렸다** —
그 레인이 병합된 뒤 evidence 좌표를 달아 다시 등재한다.

`OPEN` 은 **115건을 배타 넷으로** 갈랐다. 초판의 「대개 갱신 누락」 단정은 3/7 에서 틀렸고
(정본 자신이 두 값을 말하는 것 하나, 소유자 공백 하나, 닫는 주체가 갈리는 것 하나) **갱신 누락
4 · 그 밖 3** 으로 정정했다.

**운영자 결정 2026-09-18 — 넷 다 추천안.**

① **최소 종결** — 판정만 확정하고 상충 문면 정정·미결 이월처 지정·이월 빈 둘의 재분류는 각 계열
소유 slice 로 넘긴다(M5 가 이월 14건을 남기고 종결한 전례). 그래서 `docs/discovery/capability-map.md`
와 사전은 이 slice 가 편집하지 않고, 의도적으로 남는 것 넷을 `checklist.md` 가 선언으로 적는다.

② **`OPEN-NOTI-06` → 6F-7 이월** — 알림 요청을 outbox 로 잇는 slice 가 그 축의 유일한 실제
소비자다. 배타 분류는 종결 36 · 이월 60 · 미결 12 · 상충 6 · 참조 1, 합 115 로 갱신된다.
**단 6F-7 은 이 base 의 승인 문서에 없다** — 이 문서의 `milestone-6.md` 는 6A~6E 를 정의하고
6F 군은 아직 병합되지 않은 레인에서 정의된다. 그래서 이 이월은 **조건부**이고, 같은 전제를 조건
⑤ 의 받는 자리와 `OPEN-STR-12` 의 전망이 함께 쓴다(`checklist.md` 「받는 자리의 지위」).

③ **게이트 공백은 후속 slice 등재만** — 조건 ⑦ 을 미충족으로 내리지 않고
`OPEN-M4-APP-LAYER-GROUP-GATE` 를 신설해 추적한다. M4 는 port 경계까지가 범위였고 이 공백은
새로 생긴 것이 아니다.

④ **재검증 통과 뒤 PR.**

정본은 `checklist.md` 「운영자 결정」 절이다.
