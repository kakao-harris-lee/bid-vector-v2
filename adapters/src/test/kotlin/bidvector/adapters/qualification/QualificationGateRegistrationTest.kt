...
/**
 * `EvaluationGateRegistrationTest`·`MlGateRegistrationTest` 관례(D-6F5-6) —
...
class QualificationGateRegistrationTest {
    @Test
    fun `adapters qualification 패키지의 모든 Test class 는 gate-tests properties 에 등재된다`() {
        val discovered = discoverAdaptersQualificationTestClasses()
        val registered = registeredAdaptersQualificationTests()
    ...
    fun `등재 집합에 없는 이름이 discovered 에 있으면 차집합이 비지 않는다 — 양성 대조`() {
        val discovered = setOf("bidvector.adapters.qualification.FakeUnregisteredTest")
        val registered = setOf("bidvector.adapters.qualification.SomeOtherTest")

        (discovered - registered) shouldBe discovered
    }
}

private fun discoverAdaptersQualificationTestClasses(): Set<String> {
    val sourceRoot = File("src/test/kotlin/bidvector/adapters/qualification")
    check(sourceRoot.isDirectory) { "소스 루트를 찾지 못했다: ${sourceRoot.absolutePath}" }

    return sourceRoot
        .listFiles { file -> file.isFile && file.name.endsWith("Test.kt") }
        .orEmpty()
        .map { file -> "bidvector.adapters.qualification.${file.nameWithoutExtension}" }
        .toSet()
}

private fun registeredAdaptersQualificationTests(): Set<String> {
    ...
    return required
        ...
        .filter { fqcn -> fqcn.startsWith("bidvector.adapters.qualification.") }
        .toSet()
}
