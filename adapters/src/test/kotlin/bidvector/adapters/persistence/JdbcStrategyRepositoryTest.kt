...
class JdbcStrategyRepositoryTest : PersistenceTestSupport() {
    ...
    fun `Codex 심판 HIGH 회귀 — 예산 won 의 Long 상한 경계값은 정상 왕복한다`() {
        val repository = JdbcStrategyRepository(dataSource(), testPolicy())
        val workflow = workflowWith(repository)
        val boundary = BaseAmount(Long.MAX_VALUE, Currency.KRW, VatTreatment.INCLUSIVE, Provenance.OperatorDeclared)
        // maxBudget 을 비운다 — checkBudgetOrder(min<=max)와 충돌하지 않게(경계값 자체를 겨눈다).
        val draft = fullDraft().copy(minBudget = boundary, maxBudget = null)

        applyDraft(workflow, EditSessionId("s-won-boundary"), draft, StrategyRevision(0))
