...
/**
 * 운영 정책 인스턴스(D-4D-7) — 초기값은 착수 placeholder(정본 `reports/evidence/m4/4d/
 * policy-values.md`, 승인 대기 `OPEN-4D-POLICY-VALUES`). 근거는 실측이 아니라 3C
 * `EXTRACTION_POLICY`와 2D `retry.sample.max-attempts=3`의 보수적 상한이다(ADR 0010 D-1
 * 「보수적 상한 + 측정 의무」). test 는 이 인스턴스를 쓰지 않는다 — 좁은 값의 별도 인스턴스
...
val ML_CALL_POLICY: EffectiveDatedPolicy<MlCallPolicyData> =
    EffectiveDatedPolicy(
        source =
            "reports/evidence/m4/4d/policy-values.md D-4D-7 — 착수 placeholder(3C EXTRACTION_POLICY·" +
                "2D retry.sample.max-attempts=3 의 보수적 상한 근거, 실측 근거 없음 — 관측 뒤 version 갱신, " +
                "OPEN-4D-POLICY-VALUES)",
        entries =
            listOf(
