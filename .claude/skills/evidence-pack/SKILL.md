...
결과도 그 HEAD 에서 다시 낸다. **verifier 는 그 SHA 가 자기 판정 대상 SHA 와 같은지를 rollback 절에서
가장 먼저 본다. 다르면 그 절은 통과가 아니라 미검증이다.** base 가 움직였으면(rebase·흡수 병합)
목록 재산출의 기준도 함께 움직인다 — 옛 base 로 재산출한 목록은 명령이 exit 0 을 내면서 틀린다.
