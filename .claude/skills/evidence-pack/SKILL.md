...
- [ ] test/lint/type/architecture/contract 관련 명령 통과 — **부분 게이트를 전건 대신 돌리지
      않는다. CI 가 돌리는 명령 그대로(`./gradlew --no-daemon check`)가 정본이다.** 「이 변경은
      계약 파일만 건드리니 `contractGate` 면 된다」 같은 축약은, 정작 막는 게이트가 다른 것일 때
      로컬 초록과 CI 붉음을 동시에 만든다. 관련 게이트만 돌려 본 것은 **돌려 보지 않은 것과
      같게 취급**하고, 전건 exit 0 을 commands.md 에 남긴 뒤에만 이 항목을 체크한다.
