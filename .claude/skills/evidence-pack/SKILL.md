...
실제 PR 32건 재생에서 **FAIL 23(72%) · 확인된 진짜 결함 0 · 오탐 1**. 닫히지 않은 이유 셋 —
(a) 브랜치가 main 을 흡수하면 merge-base 가 전진해 문서의 착수 지점과 **구조적으로** 어긋난다
...
사라져 게이트가 붉는다 (c) 다중 slice PR·2단 복원 절차·문서를 먼저 올리는 순서 바꾸기에 각각
초록인 형태가 없거나 커밋 하나짜리 우회가 있다. 술어를 좁히는 수정마다 같은 계열의 새 구멍이 열렸다
(「열거 방어의 서명」). **되돌리기 목록의 정합은 문자열 대조가 아니라 실행으로만 닫힌다** — 그래서
이 자리의 규율은 게이트가 아니라 위 「실측 HEAD」 한 줄이다.
