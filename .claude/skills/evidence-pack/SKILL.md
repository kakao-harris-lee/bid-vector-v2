...
- **낡는 좌표 — 같은 사실을 두 자리에, 값을 박아 적기.** 개수·목록·SHA·줄 번호는 산출물이
  ...
  **게이트의 허용 목록(baseline·allowlist)도 이 규격 아래에 있다 — 키에 `file:line` 을 쓰지
  않는다.** 좌표를 키로 잡으면 위쪽에 줄을 넣는 편집이 (a) 밀린 자리를 「새 매치」로 만들어
  게이트를 붉히고 (b) 원래 항목을 아무것도 가리키지 않는 죽은 줄로 남긴다 — **편집 하나가
  오탐과 미탐을 동시에 만든다.** 키는 내용에서 뽑는다(매치한 줄의 정규화 해시, 파일+어휘+건수
  같은 형태). 기존 목록이 좌표 키면 죽은 항목을 보고하는 축(`stale_*`)을 함께 낸다.
  (M4 PR #7 실측 2026-09-12: `leak-pattern-baseline.txt` 가 `파일:줄` 키라 4줄 삽입이 같은
  인용을 `:64 → :68` 로 밀어 CI 가 붉어졌다. 「낡는 좌표」를 evidence 문서에는 금지하면서
  **게이트 자신의 키 설계로 상설화**한 것이다 — `OPEN-LEAK-BASELINE-COORD`.)
...
**hunk 역적용은 자동으로 해소되지 않는다 (2026-09-10).** 두 slice 의 삽입 지점이 **인접**하면
(같은 프로퍼티 키의 같은 목록, 같은 문서의 이웃 문단) `git apply -R` 은 물론 **`--3way` 도 자동
해소에 실패하고 conflict marker 를 낸다.** 그러므로 ② 의 명령만 적어 두는 것으로는 rollback 이
서지 않는다 — **수동 해소 절차를 rollback.md 에 미리 적고**(무엇을 지우고 무엇을 남기는지 문단·
블록 이름으로), **임시 clone 에서 끝까지 실제로 실행**해 「내 줄 grep 0」과 「남의 줄 남음」을 둘 다
실측한다. M4/4C-2 실측: `config/quality/gate-tests.properties` 에서 4C-2 와 4B-3 의 주석 블록이
같은 삽입 지점에 붙어 `--3way` 가 conflict 를 냈다.

**목록은 자기를 담은 커밋을 가리킬 수 없다 — 공유 파일을 만지는 문서 커밋은 목록 갱신과
나눈다 (2026-09-10).** rollback.md 를 쓰는 커밋이 **공유 파일도 함께** 만지면 그 커밋은 자기
목록에 들어갈 수 없다(커밋 전에 자기 해시를 모른다). 그러면 문서대로 실행할 때 **역적용
시퀀스가 중간에서 끊긴다** — 앞 hunk 가 conflict 로 걸리고 다음 명령이 「인덱스에 없습니다」로
죽는다(1A 16차 「명령이 아예 실패」 계열). 그래서 **두 커밋으로 나눈다**: ① 공유 파일을 만지는
문서 편집(구현 기록·종결 등재) ② 그 해시를 목록에 더하는 커밋 — **②는 evidence 경로만 만진다.**
②가 공유 파일을 건드리면 목록이 또 자기만큼 낡아 결함이 자기를 재생산한다.
M4/4C-2 실측: 목록이 evidence 커밋 하나만큼 낡아 verifier 가 문서대로 실행했을 때 exit 1 이
났고, 앞 라운드에서는 그 커밋이 우연히 공유 파일을 안 건드려 **우연히 맞았을 뿐**이었다.
...
**⑥ 그 트리에서 게이트가 초록인지도 잰다 (2026-09-12).** ④⑤ 는 「코드가 서는가」만 재고 **게이트가
받아들이는가**는 재지 않는다. 이 저장소의 품질 장치는 통째로 게이트이므로 그 둘이 갈리면
**rollback 문서는 초록인데 CI 는 붉다.** 되돌린 트리에서 **그 slice 가 닿은 게이트**(없으면 전건
`check`)를 돌려 종료 코드를 rollback.md 에 적는다. **초록이 아니면 그것이 rollback 절차의 미완이지
게이트의 결함이 아니다** — 무엇을 더 되돌려야 초록이 되는지를 찾아 절차에 적고, 그 보완 경로도
**실제로 실행해** 초록을 확인한다(적어 두기만 한 복구 경로는 복구 경로가 아니다). 되돌리지 않기로
한 것(예: 이 slice 의 evidence 디렉터리)이 게이트를 붉히면, **그 사실과 조건을 알려진 제한으로
등재**한다.

근거: M4/leak-baseline-coord 실측 — in_scope 를 base 로 되돌리면 `compileTestKotlin` exit 0 ·
단위 test exit 0 인데 **`leakPatternGate` 는 exit 1**(되돌리지 않은 evidence 문서의 자기매치 9건이
base 의 baseline 에 없다). 첫 evidence 커밋부터 있던 구조적 한계인데 **세 라운드 동안 rollback.md 에
「게이트 전부 정상 통과」로 적혀 있었다** — ④⑤ 만 요구하는 규격이 그 거짓을 통과시켰다. 2026-09-08 이
「명령이 성공하면서 결과가 미달」을 잡았고, 이것은 **그 한 칸 안쪽**이다.

**같은 slice 안에 층층이 쌓인 자기 이력은 착수 경계 기준 단일 역적용으로 걷는다 (2026-09-16).**
hunk 격리(커밋 해시별 `git apply -R`)의 목적은 **다른 slice 의 줄**을 파괴하지 않는 것이다. 그 파일에
다른 slice 의 줄이 없고 보존할 것이 **이 slice 의 착수 커밋 하나**(계약 초판·정책 표 초판)뿐이면,
그 뒤 커밋 전부(구현 레인·팀장 문서 레인 가리지 않고 — 전부 착수 결정의 사후 기록이다)를
`git diff <착수 커밋> -- <파일> | git apply -R` 한 번으로 걷는다. 커밋 단위 격리는 같은 줄을 두
레인이 이어 만지는 순간 컨텍스트 불일치로 깨지고, 단일 역적용은 커밋이 더 붙어도 안 깨진다.
판별은 git author 가 아니라 **내용**(착수 결정 자체인가 / 그 뒤의 사후 기록인가)이다.
**단일 역적용은 하네스 레인 절까지 지운다** — 되돌린 scope.md 가 「착수 시 없음」인데 range 에는
하네스 커밋이 남아 복구 상태의 승인 문서가 거짓 서술을 갖는다. 그래서 역적용 뒤 **하네스 레인
절을 현재 값(`git log --oneline <base>..HEAD -- CLAUDE.md .claude/`)으로 재등재**하는 단계를
rollback.md 에 두고 임시 clone 실측에 포함한다(커밋별 격리 대비 단일 역적용이 잃는 유일한 것).
M5/5C-2 실측: `scope.md` 를 구현 레인 2·팀장 3 커밋이 이어 만져 커밋별 역적용 셋이 conflict 로
깨졌고, 착수 커밋 경계 단일 역적용은 그 뒤 두 커밋이 더 붙은 HEAD 에서도 exit 0·conflict 0 이었다.
...
**①~⑥ 실측은 그 slice 의 마지막 산출물 커밋에서 돌고, 그 SHA 를 문서에 적는다 (2026-09-18).**
rollback.md 첫머리에 `실측 HEAD: <sha>` 한 줄을 둔다. 앞 라운드의 실측을 옮기지 않는다 — 목록도
결과도 그 HEAD 에서 다시 낸다. base 가 움직였으면(rebase·흡수 병합) 목록 재산출의 기준도 함께
움직인다 — 옛 base 로 재산출한 목록은 명령이 exit 0 을 내면서 틀린다.

**verifier 가 대조하는 것은 「실측 HEAD == 판정 SHA」가 아니다 (2026-09-19 정정).** 처음 쓸 때 그렇게
적었는데 **구조적으로 만족할 수 없는 문장**이었다 — evidence 커밋은 언제나 마지막 산출물 커밋 **뒤에**
오므로 둘은 영원히 다르다(M6/6F-4 verifier 가 「문면 그대로면 미검증인데 같은 규율의 다른 절과 동시
만족이 불가능하다」로 지적). 대조할 것은 **그 사이에 산출물이 움직이지 않았는가**다:

```
git diff --name-only <실측 HEAD>..<판정 SHA> -- <rollback.md 가 되돌리는 경로들>
```

**빈 출력이면 실측이 유효하다.** 한 줄이라도 나오면 그 뒤 커밋이 되돌림 대상을 바꾼 것이므로 실측은
낡았고 그 절은 통과가 아니라 미검증이다. 실측 HEAD 가 판정 SHA 의 조상이 아닌 경우(다른 가지)도 미검증이다.

**대조 경로는 `rollback.md` 자신의 목록이다 (2026-09-19 두 번째 정정).** 처음에는 `':!reports/evidence'`
하나만 제외하도록 적었는데, 그러면 **되돌림 대상이 아닌 팀장 레인 문서**(`CLAUDE.md`·`.claude/**`·
마일스톤 문서)의 편집이 잡혀 멀쩡한 실측이 「미검증」이 된다(M6/6F-4 에서 `milestone-6.md` 한 줄로
실제로 걸렸다). 되돌림 절차가 덮는 집합과 이 술어의 집합은 **같아야 한다** — 그래서 제외 목록을 따로
관리하지 않고 목록 자신을 쓴다. 이 규율은 세운 날 **측정으로 두 번 교정됐다**(자기모순 → 제외 집합).

근거: M6/6F-1 — rebase 로 base 가 바뀌었는데 목록을 옛 base 기준으로 재산출해, 문서대로 실행하면
**exit 0 인데 다른 slice 가 세운 게이트 파일이 사라지고 트리가 컴파일되지 않는** 상태가 됐다
(verifier r2 HIGH-1). 6B-1 도 병합 뒤 같은 이유로 낡았다. 「라운드마다 재산출」은 2026-09-10 부터
규정돼 있었으나 **어느 HEAD 에서 재산출하는가**가 없어 세 번 재발했다.

**이 확인을 CI 텍스트 게이트로 대체하려는 시도는 측정으로 실패했다 (2026-09-18).** 「문서의 복원 인자
집합 = 그 범위의 diff 집합」을 CI 에서 대조하는 하네스 slice 를 네 라운드 돌려 폐기했다. 이 저장소의
실제 PR 32건 재생에서 **FAIL 23(72%) · 확인된 진짜 결함 0**. 최근 관례로 연 PR 7건 중 5건이 붉고
**전부 오탐**이었다. 닫히지 않은 이유 넷 —

(a) **`restore` 인자는 절차의 전부가 아니다.** 신설 파일은 되돌릴 base 내용이 없어 `git rm` 으로
지우고 `git restore` 는 수정 파일만 받는다(이 저장소의 표준 형식, `m5/5e` 문서가 근거까지 적어 뒀다).
「복원 인자 집합 = 범위 diff 집합」이라는 등식은 애초에 성립하지 않는다 — 인자 불일치로 잡힌 누락
경로 219개 중 **170개(78%)가 신설 파일**이었다. 이 하나가 FAIL 대부분을 설명한다.
(b) 되돌려야 할 집합은 diff 집합이 **아니다** — evidence 를 남기는 관례와 누출 baseline 의 allowlist
(293 항목 중 279 가 evidence 경로)가 짝이라, diff 를 전부 되돌리면 evidence 는 남고 allowlist 만
사라져 게이트가 붉는다.
(c) 브랜치가 main 을 흡수하면 merge-base 가 전진해 문서의 착수 지점과 어긋난다. 여러 slice 가
로컬 병합으로 쌓인 뒤 PR 하나로 올라오면 그 PR 하나가 slice 문서 여럿을 **단일 merge-base** 로
판정한다(실측: PR 하나가 문서 12개를 그렇게 판정했고 12건 전부 붉었다).
(d) 2단 복원 절차·문서를 먼저 올리는 순서 바꾸기에 각각 초록인 형태가 없거나 커밋 하나짜리
우회가 있다.

술어를 좁히는 수정마다 같은 계열의 새 구멍이 열렸다(「열거 방어의 서명」).
**되돌리기 목록의 정합은 문자열 대조가 아니라 실행으로만 닫힌다** — 그래서 이 자리의 규율은
게이트가 아니라 위 「실측 HEAD」 한 줄이다.
...
- [ ] test/lint/type/architecture/contract 관련 명령 통과 — **부분 게이트를 전건 대신 돌리지
      않는다. 정본은 CI 워크플로의 job 이 돌리는 명령 그대로다.** 「이 변경은 계약 파일만
      건드리니 `contractGate` 면 된다」 같은 축약은, 정작 막는 게이트가 다른 것일 때 로컬
      초록과 CI 붉음을 동시에 만든다. 관련 게이트만 돌려 본 것은 **돌려 보지 않은 것과 같게
      취급**하고, 전건 exit 0 을 commands.md 에 남긴 뒤에만 이 항목을 체크한다.
      **줄일 수 있는 단위는 게이트가 아니라 job 이다** — 이 저장소의 `.github/workflows/ci.yml`
      은 Kotlin `check` 와 Python `ml-engine` 두 job 이고 **소스가 겹치지 않는다고 워크플로
      자신이 선언**한다. 변경이 닿은 job 만 돌리는 것은 허용되고(닿은 job 이 둘이면 둘 다),
      **한 job 안에서 「이 게이트만」으로 좁히는 것이 금지**다. job 경계를 근거로 쓸 때는
      워크플로의 독립 선언을 확인하고 쓴다 — 선언이 없으면 전부 돌린다.
      (M4 PR #7 실측 2026-09-12: 주석 전용 계약 변경이라 `contractGate` 만 돌려 exit 0 을 보고
      push 했는데, CI 를 떨어뜨린 것은 `:leakPatternGate` 였다 — 2026-09-09 「상시 붉은 게이트는
...
- [ ] 비밀값 스캔 통과 — evidence 디렉토리와 산출물 경로에 대해 **패턴 파일을 참조하는 형태**로
      ...
      **패턴 어휘를 evidence 문서에 축어로 적지 않는다 (2026-09-16).** 루트 `leakPatternGate`
      ...
      그 기록은 거짓이다.
      **단, 마지막 커밋의 결과 정본은 evidence 가 아니라 verifier 와 PR 조치 코멘트다 (2026-09-16).**
      evidence 는 **자기 마지막 커밋의 post-state 를 담을 수 없다** — 그 줄을 적으려면 커밋이 하나 더
      필요하고 그 커밋이 다시 같은 줄을 요구한다. 그래서 마지막 HEAD 의 게이트 결과는 **verifier 가 그
      HEAD 에서 직접 재서** 리포트에 남기고, 저작 레인은 **PR 조치 코멘트**에 적는다(PR 이 없으면 완료
      보고). evidence 의 명령 표는 그 직전까지만 담고 「마지막 HEAD 는 verifier·조치 코멘트가 정본」을
      한 줄로 밝힌다. 근거: M5/5F-1 verifier r1 — range 22 커밋 중 16 이 산출물 무접촉이고 8 이
      `commands.md` 전용이었다(자기급식 루프). 규율 자체는 유지한다 — 바뀌는 것은 **마지막 한 칸의
      기록 자리**뿐이다.
