...
```
git diff --name-only <실측 HEAD>..<판정 SHA> -- . ':!reports/evidence'
```

**빈 출력이면 실측이 유효하다.** 한 줄이라도 나오면 그 뒤 커밋이 산출물을 바꾼 것이므로 실측은 낡았고
그 절은 통과가 아니라 미검증이다. 실측 HEAD 가 판정 SHA 의 조상이 아닌 경우(다른 가지)도 미검증이다.
