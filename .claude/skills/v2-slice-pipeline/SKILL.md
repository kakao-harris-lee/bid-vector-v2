...
4. **값 slice(정책 값·정책 데이터 한 줄)도 test 두 축을 계약에 고정한다 (2026-09-16).** 「값만 바꾸고
   `src/**` 무편집」인 slice 는 게이트가 통째로 초록인 채 두 자리가 비어 있기 쉽다 — ① **그 값이 wire
   (요청·응답 proto, 외부 계약 표면)에 닿는가**: 실 정책으로 만든 요청을 대조하는 test 가 없으면 운반
   단계를 뒤집어도 전건이 초록이다 ② **그 값이 만드는 거동 변화**: 경계가 이동하면 다른 입력 구간의
   결과가 바뀐다 — fixture 를 옮겨 피하지 말고 그 거동을 test 로 고정하고(계약·소비자 위반인지 먼저
   실측) 알려진 제한에 등재한다. 값 slice 의 acceptance 에 이 둘을 넣고 verifier 표적으로 준다.
   근거: M5/5F-2 verifier r1 변이 B(Kotlin 정책 값 한 줄 — 요청 매핑을 뒤집어도 `check` exit 0, 값이
   wire 에 실림을 재는 test 가 저장소에 없었다) · M5/5F-1(clamp 상한을 내리자 경계 위 입력에서 후보
   둘이 같은 값으로 접혔다 — Kotlin 소비자·도메인 타입이 비엄격임을 실측하고 D-5F1-5 로 고정).
...
(2) 우회 경로를 최소 다섯 고안해 설계가 각각을 어떻게 막는지, (2b) **값 획득 축** — 이 slice 가
새로 public 으로 내놓는 타입·함수·반환값을 전수하고 각각이 **밖에 허락하는 것**을 적는지,
(3) 승인 문서가 요구하지 않는 과잉·미달을 판정해 `_workspace/{slice}/NN_design-review.md` 에 남긴다. 구현 레인은 그
...
**(1) 의 판정에는 「게이트 술어가 무엇에 걸려 있는가」가 들어간다 — 문자열이 아니라 구조여야 한다
(2026-09-16).** 술어가 **호출 텍스트**(`grep 'yaml.safe_load('`)나 **주석 문자열**(반환 annotation 에
`Rejected` 가 있는지)에 걸려 있으면, 규약을 형식적으로 다 지키는 저자가 **스타일 하나**로 통과한다 —
별칭 import(`import yaml as _y`), 타입 별칭 반환 주석, from-import, `full_load`·`safe_load_all`,
`getattr`. 열거를 늘리는 것은 답이 아니다(형태 집합이 무한하다). **저자가 우회하려면 게이트 자신이나
빌드 설정을 바꿔야 하는 층**에 건다: import-linter 계약(import 단계에서 별칭·from-import 전부 잡힘) ·
AST(이름 바인딩 집합으로 속성·호출을 셈) · 타입·컴파일(명명 인자 호출이 시그니처를 잠금) · 린트 규칙
(미사용 파라미터). 수집·열거도 같다 — 「손 목록 금지, 기계 수집」에서 한 칸 더 가서 **하한 단언
(`>= 4`)이 아니라 집합 등식**(수집 집합 == 열거 집합)을 쓰고, **검사 대상 목록을 게이트 자신이 아니라
계약 파일에서 끌어온다**(예외를 늘리면 검사도 함께 늘어난다).
근거: M5/5E-3 verifier r1 HIGH — 정책 로더 fail-closed 게이트 넷이 초록인 채 다섯째 로더가 raw 파서
예외를 누출했고(우회 두 축이 독립), import 계약 + 모듈 규약 수집으로 바꾸자 변이 다섯이 전부 잡혔다.
5E-1 「collection 선택에 따라 답이 갈리는 게이트」에 이은 **게이트 결함 클래스 네 번째**다.

**(2b) 값 획득 축은 값 위조 축과 다른 물음이다 (운영자 채택 2026-09-09).** 「이 값을 밖에서
**만들 수 있는가**」와 「이 값을 밖에서 **얻을 수 있는가**」는 답이 다르고, 후자를 세지 않으면
전자를 닫아도 게이트가 서지 않는다. 검토자는 **공개 표면을 전수**한다 — 새로 public 인 타입·
최상위 함수·데이터 클래스 프로퍼티·use case 반환값 각각에 대해 「이걸 손에 넣은 주체가 무엇을
할 수 있는가」를 한 줄씩 적고, 그 권한이 위협 모델 경계 안인지 판정한다. 필요하면 `javap` 로
공개 시그니처를 전수해 눈으로 세지 않는다.

**(2b) 표의 고정 항목 — 「`object` 커널을 세야 할 때 무엇을 주입하는가」 (2026-09-10).**
순수 커널을 `object`(port 아님)로 두면 **호출 횟수를 셀 수 없다.** 세려면 주입해야 하고, **주입 자리는 곧 공개 표면**이다 —
「테스트 가능성」과 「표면 최소화」가 정면으로 충돌하는 자리다. 설계 검토는 이 충돌을 **미리** 표에 올리고 처분을 정한다:
주입하지 않고 간접 관측할 수 있는가 · 주입한다면 **주 생성자를 `internal` 로 내리고 public 보조 생성자를 두는가**(그러면
같은 모듈 test 는 세고 밖은 못 꽂는다) · 그 자리가 대체되면 **무엇이 무너지는가**.

**근거 — M4/4B-2 세 라운드의 계보.** 「판정 1회에 회귀 보호 없음」(low) → 시정이 판정 함수를 생성자 인자로 뽑음 → **high**
(밖에서 함수를 갈아끼워 알림 요청 생성) → 주 생성자 `internal` + public 보조 생성자로 폐쇄. 산출물이 부실해서가 아니라
이 충돌을 설계 단계에서 안 짚어서 세 라운드가 들었다. **같은 축이 4D(ML port 계수)·4C-2(배달 계수)에서 곧 다시 온다.**
판정 기준도 그 slice 가 세웠다 — 「그 이음매가 없으면 조합 근처의 누구도 그 값을 만들 수 없는가」가 참이면 **없던 권한**이라
high 이고, 배선 주체가 port 로 이미 가진 것과 동치면 등재다. 「결과가 쓴 값을 나르는가」(조작의 자기서술성)도 함께 본다.

**(2b) 표는 착수 시 한 번이 아니라 새 public 표면을 만드는 모든 수정 라운드에서 갱신한다 (2026-09-10).**
설계 검토는 구현 **전에** 한 번 돌지만, finding 을 고치는 커밋이 **새 공개 시그니처를 만들면** 그 표면은 어떤 축 검사도
받지 않고 산출물에 들어간다. **테스트 가능성을 얻으려 연 자리가 가장 흔한 경로다.** 그래서 수정 라운드 보고에
「이번 수정이 새 public 표면을 만들었는가, 만들었다면 그것이 밖에 무엇을 허락하는가」를 **항목으로** 넣고, verifier 는
그 항목을 표적으로 받는다.

**근거 — M4/4B-2 실측.** 검증이 「판정 1회에 회귀 보호가 없다」(low)를 지적했고, 시정이 판정 함수를 **생성자 인자**로
뽑았다. `private val` 은 프로퍼티 읽기만 막고 **생성자 매개변수는 공개 시그니처**라, 다른 모듈이 자기 함수를 꽂아
후보와 무관한 입력으로 진짜 사다리를 불러 정당한 `BidNow` 를 만들어 넣을 수 있게 됐다(실측: 정직한 배선 알림 0건 →
주입 배선 알림 2건). **없던 권한이 생겼다** — 그 이음매가 없으면 조합 근처의 누구도 알림 요청을 만들 수 없다.
low 를 닫은 커밋이 high 를 낳았고, 시정은 주 생성자를 `internal` 로 내리고 public 보조 생성자를 두는 한 수였다.
**축을 착수 시점에만 걸면 이 경로가 항상 열려 있다.**

**「경계로 처리」 행도 실측 목록에 넣는다 (2026-09-09).** (2b) 표의 각 행은 「닫는다」·「연다」·「경계로 처리」 중 하나로 판정된다.
구현·검증 레인의 실측 목록이 **「닫는다」 행만** 담으면 그 목록은 **정의상 자기가 세운 경계를 검사하지 않는다** — 경계 논증이 틀렸을 때
그것을 잡을 장치가 없다. **「경계로 처리」 행마다 「그 권한이 정말로 경계 안 주체에게만 가는가」를 컴파일·실행으로 재고 결과를 남긴다.**
M4/4C-1 실측: 값 획득 축 표의 여섯 행 중 다섯(「닫는다」)은 착수 전에 닫혀 실측도 통과했는데, **유일한 「경계로 처리」 행**(저장소 복원 진입점)만
실측 목록에서 빠졌고 거기서 high 가 나왔다 — public 복원 함수가 「persistence 어댑터의 권한」이 아니라 **아무 모듈에나** 위조 이벤트 주입을 허락했다.
경계 논증은 **주체를 한정할 때만** 서고, 가시성이 그 한정을 강제하지 않으면 논증이 아니라 희망이다.

**근거 — M4/4A 실측.** 4A 설계 검토가 우회 후보 일곱을 세웠으나 **전부 위조 축**이었다(생성자
직조·`copy`·필수 필드 우회). 구현이 통로 타입(`internal constructor` + `@ConsistentCopyVisibility`)
으로 위조를 닫자, 다음 라운드에 **획득** 경로가 그대로 남아 있었다 — 커널 함수가 public 이라
밖에서 직접 몰면 토큰을 정당하게 얻어 port 로 **이벤트 0 인 write** 가 실행됐고(STR-07 이
`폐기`로 못 박은 형태), 그 다음 라운드에도 use case **반환값**으로 같은 토큰이 샜다. 세 라운드가
전부 「use case 가 막지만 타입이 열어 둔다」 한 계열이었고, 이 표가 설계 단계에 있었으면 한 번에
나왔다. 반복 자체는 발산이 아니라 수렴이었다(우회에 필요한 권한이 라운드마다 상승) — 그러나
수렴을 라운드로 사지 말고 검토로 사라는 것이 이 항목이다.
...
**진행 중 slice 의 브랜치는 종결 뒤에만 병합한다 (2026-09-10).** 레인이 별도 브랜치·worktree 로
갈라져 살면 「병합 전 검증용」이라는 이유로 **아직 검증되지 않은 남의 slice 를 자기 브랜치로
끌어오는** 일이 생긴다. 그 병합 커밋이 공유 브랜치(`main`·마일스톤 브랜치)로 흘러가면 **verifier 가
`not-ready` 를 낸 코드가 승인 없이 공유 브랜치에 실린다** — 그리고 그 코드에는 보통 **아직 참이
아닌 문면**(승인 문서가 「닫혔다」고 진술하는 자리)이 함께 실린다. 다른 레인의 브랜치는 **그
slice 가 종결(verifier `ready-for-review` + 사용자 승인)된 뒤에만** 병합한다. 병합 전 대조가
필요하면 자기 브랜치를 상대 브랜치에 얹지 말고 **버릴 clone/worktree 에서** 한다.
M4 실측: 4C-2 가 verifier high 둘로 `not-ready` 인 동안 다른 레인이 그 진행분을 자기 브랜치에
병합했고, 그 커밋으로 마일스톤 브랜치와 `main` 이 함께 이동해 오케스트레이터가 「검증 전 코드를
`main` 에 넣지 않는다」로 박아 둔 핀이 무효가 됐다. **이력은 되쓰지 않는다** — 위 혼입 규율대로
evidence 에 사실로 선언하고 다음 라운드가 닫는다(되쓰기가 더 큰 위험이다).
...
**변이가 실제로 적용됐는지 먼저 재라 (2026-09-19).** 「변이를 넣었는데 초록이다」는 **두 가지**를 뜻할
수 있다 — 게이트가 못 잡거나, **변이가 애초에 안 들어갔거나**. 둘을 가르지 않으면 후자가 「게이트 구멍
발견」으로 보고된다. 그래서 변이를 넣은 직후 `git diff --numstat` 으로 **줄이 실제로 바뀌었는지** 확인한
뒤에 명령을 돌린다.

근거: M6/6F-4 에서 **같은 함정에 두 레인이 각각 빠졌다.** 셸 heredoc 이 `\u0009` 같은 이스케이프 표기를
**실제 제어문자로 치환**해, 구현 레인은 의도보다 좁은 문자 클래스를 커밋했고(공백 금지 CHECK 가 VT·FF·CR
을 놓쳤다) verifier 는 **변이가 적용되지 않은 채 `BUILD SUCCESSFUL`** 을 받았다. 파일에 이스케이프 표기를
넣어야 하면 **손으로 다시 타이핑하지 말고 선례 파일의 바이트를 프로그램으로 읽어 재사용**하고, 결과를
바이트로 대조한다(6F-4 가 그렇게 닫았다).
...
Phase 2.5 설계 검토도 별도 에이전트가 아니라 세션 모델이 직접 수행한다.
