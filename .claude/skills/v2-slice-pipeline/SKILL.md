...
이고, 종결 여부는 운영자가 그때마다 명시 결정한다 — 닫는 결정이 나면 「Codex approve
없는 종결」의 예외로 그 결정을 기록한다. **상한 도달도 같다** — finding 의 재분류가
아니라 미승인 상태의 정지·보고다. 반루프 취지는 유지된다: M0/0E 실측에서 B4 이후 다수
finding 이 수정 커밋 자신이 낡게 만든 장부층이었고, 「approve = 장부의 완벽성」으로 두면
루프가 정의상 끝나지 않는다. 이 정지선은 자동 라운드를 멈출 뿐 리뷰의 차단력을 건드리지
않는다 — `milestone-0.md` 의 「Codex approve 와 사용자 승인」 요건과 agent-workflow.md
의 finding 임의 축소 금지는 그대로 정본이다. 도메인·계약층 finding 이 하나라도 있으면
층 구분 없이 일반 절차를 따른다.
