...
**장부층 종결선 (운영자 채택 2026-09-01).** finding 을 두 층으로 갈라 읽는다 —
**도메인·계약층**(명세 의미, fixture 기대값, 계약이 잠그는 것)과 **장부층**(evidence 문서
사이의 정합성: 개수·좌표·현재형 서술·상호 포인터). 재리뷰 verdict 가 도메인·계약층
finding 없이 **장부층 non-blocker 만** 담으면, 수정 라운드를 돌리지 않고 그 finding 들을
「알려진 제한」으로 등재한 뒤 사용자 승인으로 slice 를 닫는다. 이유: M0/0E 실측에서
B4 이후의 모든 finding 이 수정 커밋 자신이 낡게 만든 장부층이었다 — 수정이 다음 finding
을 생산하는 구조에서 「approve = 장부의 완벽성」으로 두면 루프가 정의상 끝나지 않는다.
심판이 틀렸다는 뜻이 아니다(각 finding 은 실재하는 모순이다) — 등재는 은폐가 아니라
기록이고, 최종 승인권은 어차피 사용자에게 있다. 도메인·계약층 finding 이 하나라도 있으면
이 종결선은 적용되지 않는다.
