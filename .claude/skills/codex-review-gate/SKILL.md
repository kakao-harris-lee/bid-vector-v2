...
- **저장된 verdict 가 raw output 의 종말 메시지와 일치하고 `commands_run` 이 실질적**
  ...
  **codex 가 뜬 텍스트는 방출이 아니다.** 그리고 **`tokens used` 블록이 앞서면 CLI
  ...
  세 라운드 전부 실측). 이 절이 없으면 라운드마다 +1 과대계수된다. **판별자는 앞머리
  키가 아니라 리뷰 verdict 의 형태다** — `reviewed_base`·`reviewed_head`·`findings` 가
  함께 있는 객체만 센다. 도메인 출력 스키마도 `verdict` 를 필드명으로 쓰므로(`license-*`
  기대값 `{"verdict":"Eligible",…}`) 앞머리 `{"verdict":"` 앵커는 fixture 출력을 방출로
  오탐한다(B14 실측 8건 — 세 번째 false-positive 표면). 캡처 의미가 바뀌거나
