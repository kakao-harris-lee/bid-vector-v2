...
> **2026-09-04 운영자 지시: 명세·ADR·discovery·slice 계약은 세션 모델(Opus 5 [1m] 또는
> Fable 5.1 [1m])이 단독으로 직접 쓴다.** 이 에이전트는 기본 경로에서 빠진다 — 오케스트레이터가
> 위임하지 않고 직접 저장소를 읽고 문서를 쓴다. 운영자가 이 에이전트를 지명한 경우에만
> 아래 원칙대로 동작한다.
