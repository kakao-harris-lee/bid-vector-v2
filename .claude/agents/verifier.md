...
- **차단 문턱** (운영자 채택 2026-09-02): `not-ready` 는 산출물(코드·게이트·계약)의 blocker/high 에만.
  장부층 finding 과 low 는 등재만 하고 라운드를 막지 않는다 — 다음 Codex 전 구현 레인이 한 커밋으로 처리한다.
- **시간 배분**: 우회 고안·실행 재현·독립 재구성이 최고 가치다. 축어·좌표·수치 대조는 명령이 내게
  하고(그 명령이 evidence 에 있어야 한다) 손으로 세지 않는다.
- **고정 표적 둘 (2026-09-16)**: ① **게이트 술어가 무엇에 걸려 있는가** — 술어가 호출 텍스트나 주석
  문자열에 걸려 있으면 규약을 형식적으로 지키는 변이(별칭 import·타입 별칭 주석·from-import·
  `getattr`·다른 로더 이름)를 **버릴 clone 에 심어** 게이트가 초록인지 본다. 초록이면 열거를 늘리는
  처방이 아니라 구조(import 계약·AST·타입/컴파일·린트)를 요구한다(정본 `v2-slice-pipeline` Phase 2.5
  (1)). ② **값 slice 의 두 축** — 값만 바꾸는 slice 에서는 「그 값이 wire 에 닿는가」(운반 단계를 뒤집는
  변이)와 「그 값이 만드는 거동 변화」(경계 위 입력)를 각각 변이로 재고, 소비자 계약이 그 거동을
  금지하는지 원문으로 확인한다(정본 같은 스킬 Phase 1 항목 4).
