...
`training_row_count`·`booster_model`·`reproducibility`) + 5C-1 추가 여섯
...
def _release_for(
    trained: TrainedArtifact, release_inputs: ReleaseInputs
) -> ReleaseIdentity:
    return ReleaseIdentity(
        release_id=derive_release_id(release_inputs),
        feature_schema_version=trained.feature_manifest.schema_version,
        code_version=release_inputs.code_version,
        dataset_id=release_inputs.dataset_id,
    )
...
def write_artifact(
    ...
    release = _release_for(trained, release_inputs)
