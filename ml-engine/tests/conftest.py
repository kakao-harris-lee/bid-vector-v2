...
(1) module 레벨에서 **즉시**(pytest collection 이전) `ml_engine/contracts/_generated/`
(VCS 밖, gitignore)에 `contracts/proto` 전체(6개)를 생성한다 — `ml_engine.contracts` 재수출
(`src/ml_engine/contracts/__init__.py`)이 이 자리를 import 하므로, 어느 test 파일이든 module
최상단에서 `ml_engine.contracts`를 import할 수 있고 pytest 는 conftest.py 를 그 형제 test
파일들보다 항상 먼저 로드하므로 여기서 미리 채운다.
...
_CONTRACTS_GENERATED_DIR = (
    _ML_ENGINE_ROOT / "src" / "ml_engine" / "contracts" / "_generated"
)
_PROTO_FILES_2A = (
...
def generated_stub_path() -> Iterator[Path]:
    ...
    그대로 — 이 fixture 를 쓰는 round-trip test 는 `contracts/_generated`가 아니라 이 임시
