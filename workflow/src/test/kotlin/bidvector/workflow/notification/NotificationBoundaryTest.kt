...
/**
 * S-5(scope.md, 4A `EditSessionImportBoundaryTest`·4C-1 `EventBoundaryTest` 관례 계승) —
 * `notification` 패키지도 `workflow`(application 층)라 `moduleDependencyGate`·도메인 소스
 * 참조 게이트가 걸리지 않는다. 술어는 `disallowedQualifiedReferences`(4A `strategy`
 * 패키지, `internal`)를 그대로 재사용한다 — 같은 모듈이라 복제하지 않는다(§5 중복 금지,
 * 우회 (9)의 test 증거).
 */
class NotificationBoundaryTest {
    @Test
    fun `notification 패키지는 kotlin·kotlinx·java·shared-kernel·strategy·workflow 밖의 좌표를 참조하지 않는다`() {
        val sourceRoot = File("src/main/kotlin/bidvector/workflow/notification")
    ...
    /** 양성 대조(4A·4C-1 관례 계승) — 술어가 늘 통과만 하는 회귀를 막는다. */
    @Test
