 ...
 *
 * **원산지 판정의 앵커다.** 앞선 판은 `kotlin.Metadata` 애노테이션을 앵커로 썼는데 그것은
 * **소스에 한 줄 붙여 위조된다** — Codex 8차가 `@kotlin.Metadata` 를 단 Java 클래스로 그 층을
 * 통과시켰다. `SourceFile` 은 컴파일러가 쓰는 속성이라 위조하려면 바이트코드 편집기가 필요하고,
