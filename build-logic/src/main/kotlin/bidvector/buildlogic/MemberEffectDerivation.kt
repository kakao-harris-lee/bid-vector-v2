 ...
 *
 * 왜 도출인가 — 「어떤 멤버가 위험한가」를 손으로 열거하면 생각해 내지 못한 멤버가 조용히
 * 열린다. 반대로 **여집합**(허용 목록 밖에 닿으면 불순)은 무너진다 — `ArrayList#add` 가
 * `System#arraycopy` 를, `String#substring` 이 `jdk.internal` 을 부르므로 컬렉션·문자열이 전부
 * 불순이 된다(실측).
