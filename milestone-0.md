...
| 대상 | 실측 | 정본 |
| --- | --- | --- |
| 활성 `OPEN`을 M1 이후로 이월 | **69건** = (B) 14 · (C) 55 · (A) 0 | `docs/discovery/capability-map.md` §14 |
| fixture가 덮지 못한 축 | **18축** — 각각 소유 `OPEN`과 함께 등재 | `fixtures/manifest.yaml` → `uncovered_axes` |
| `milestone-1.md:12`의 「`authoritative` case 준비」가 **부분 만족**인 채로 M1 진입 | 위 18축이 그 부분이다 | `milestone-1.md` 선행 조건 |
