...
> **2026-09-04 운영자 결정 (M0 종결 뒤 개정):** M0 는 위 조건대로 Codex `approve` 와 사용자 승인
> 둘 다 갖춰 닫혔고 그 기록은 그대로 둔다. **M1 이후 코드 slice 는 Codex 리뷰를 기본 경로에서
> 뺀다** — 완료 조건은 `verifier` `ready-for-review` 와 사용자 승인이다. 명세·ADR 같은 기획
> 문서는 세션 모델이 단독 저작하고 운영자가 검토하며, Codex 계획 검토는 운영자가 필요할 때
> 명시 요청하면 건다. 이유: 외부 유료 호출 과다(0E 15라운드·1A 16라운드)와 저작 지연.
